...
use super::{BUILD_GRAMPARSER_FILENAME, GRAMLEXER_FILENAME, GRAMLEXER_LEXICON, GRAMLEXER_TAG, GRAM_SYM_T_TAG};

fn gramlexer_source(lexicon_filename: &str, indent: usize, verbose: bool) -> Result<(String, String), ParserError> {
    ...
    if verbose {
        // terminals to replace in src/gram/gramparser.rs (copy/paste)
        println!("Terminals:\n{sym_src}");
    }
...
pub fn write_gramlexer() {
    let (result_sym, result_src) = gramlexer_source(GRAMLEXER_LEXICON, 4, true)
...
mod tests {
    ...
    fn test_source() {
        ...
        let (_result_sym, _result_src) = gramlexer_source(GRAMLEXER_LEXICON, 4, VERBOSE)
            .inspect_err(|e| eprintln!("Failed to parse lexicon: {e:?}"))
            .unwrap();
        if !cfg!(miri) {
            let expected_sym = get_tagged_source(BUILD_GRAMPARSER_FILENAME, GRAM_SYM_T_TAG).unwrap_or(String::new());
