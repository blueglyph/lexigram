...
* Non-recursive LL(1) parser or LALR(1) parser (for now).
* LL(1): automatic transformation of left-recursive/ambiguous rules and left factorization.
* LL/LR: conflict resolution in parsing tables for ambiguous rules according to priority and associativity.
* Grammar-driven listener pattern for better flexibility and to avoid mixing grammar with target (Rust) source code.
