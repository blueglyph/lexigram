...
impl<'l, 'ls: 'l> PanDemo<'l, '_, 'ls> {
    ...
    fn parse(&mut self, text: &'ls str) -> Result<PanDemoResult, BufLog> {
        ...
        let tokens = self.lexer.tokens().split_channel0(|(_tok, ch, text, pos_span)|
            ...
            .inspect(|(token, txt, span)| println!("** token: {token}, txt: {txt:?}, {span}"));
...
impl PandemoniumListener for PanDemoListener<'_> {
    ...
    fn exit_l_rrec_i(&mut self, ctx: CtxLRrecI, spans: Vec<PosSpan>) -> SynLRrecI {
        ...
        SynLRrecI(match ctx {
            ...
            CtxLRrecI::V1 { l_rrec_i: SynLRrecI(mut values), num } => {
                values.push(num);
                values
            }
            // l_rrec_i -> ";"
    ...
    fn exit_nv_l_rrec_i(&mut self, ctx: CtxNvLRrecI, spans: Vec<PosSpan>) -> SynNvLRrecI {
        self.spans.push(format!("exit_nv_l_rrec_i({})", spans.into_iter().map(|s| format!("{:?}", self.extract_text(&s))).join(", ")));
        SynNvLRrecI(match ctx {
            // nv_l_rrec_i -> nv_l_rrec_i "," "*"
            CtxNvLRrecI::V1 { nv_l_rrec_i: SynNvLRrecI(mut values) } => {
                values.push(false);
                values
            }
            // nv_l_rrec_i -> ";"
            CtxNvLRrecI::V2 => vec![true],
        })
    }
...
pub mod pandemonium_parser {
    ...
    impl<T: PandemoniumListener> Wrapper<T> {
        ...
        fn exit_nv_l_rrec_i(&mut self, alt_id: AltId) {
            ...
            let spans = self.stack_span.drain(self.stack_span.len() - n ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_nv_l_rrec_i(ctx, spans);
