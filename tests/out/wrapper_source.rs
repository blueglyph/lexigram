...
pub(crate) mod rules_123_1 {
    /// User-defined type for `a`
    #[derive(Debug, PartialEq)]
    pub struct SynA();

    // ------------------------------------------------------------
    // [wrapper source for rule 123 #1, start a]
    ...
    pub enum CtxA {
        /// `a -> A B* C+ D`
        V1 { a: String, star: SynA1, plus: SynA2, d: String },
    }

    /// Computed `B*` array in `a -> A  ►► B* ◄◄  C+ D`
    #[derive(Debug, PartialEq)]
    pub struct SynA1(pub Vec<String>);
    /// Computed `C+` array in `a -> A B*  ►► C+ ◄◄  D`
    #[derive(Debug, PartialEq)]
    pub struct SynA2(pub Vec<String>);

    #[derive(Debug)]
    enum EnumSynValue { A(SynA), A1(SynA1), A2(SynA2) }
    ...
    impl<T: TestListener> ListenerWrapper for Wrapper<T> {
        fn switch(&mut self, call: Call, nt: VarId, alt_id: AltId, t_data: Option<Vec<String>>) {
            ...
            match call {
                ...
                Call::Exit => {
                    match alt_id {
                        0 => self.exit_a(),                         // a -> A a_1 a_2 D
                        1 => self.exit_a1(),                        // a_1 -> B a_1
                        2 => {}                                     // a_1 -> ε
                        4 |                                         // a_3 -> a_2
                        5 => self.exit_a2(),                        // a_3 -> ε
                     /* 3 */                                        // a_2 -> C a_3 (never called)
                        _ => panic!("unexpected exit alternative id: {alt_id}")
                    }
                }
                Call::End(terminate) => {
    ...
    impl<T: TestListener> Wrapper<T> {
        ...
        fn exit_a2(&mut self) {
            let c = self.stack_t.pop().unwrap();
            let Some(EnumSynValue::A2(SynA2(plus_acc))) = self.stack.last_mut() else {
                panic!("expected SynA2 item on wrapper stack");
            };
            plus_acc.push(c);
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
...
pub(crate) mod rules_153_1 {
    ...
    impl From<TokenId> for Term {
        fn from(value: TokenId) -> Self {
            match value {
                ...
                _ if value == Term::E as TokenId => Term::E,
                _ if value == Term::F as TokenId => Term::F,
    ...
    pub enum NTerm {
        ...
        #[doc = "`a_2`, parent: `a_1`"] A2 = 3,
        #[doc = "`a_3`, parent: `a_1`"] A3 = 4,
        #[doc = "`a_4`, parent: `a_1`"] A4 = 5,
    }
    ...
    impl TryFrom<TokenId> for NTerm {
        ...
        fn try_from(value: VarId) -> Result<Self, Self::Error> {
            match value {
                ...
                _ if value == NTerm::A2 as VarId => Ok(NTerm::A2),
                _ if value == NTerm::A3 as VarId => Ok(NTerm::A3),
                _ if value == NTerm::A4 as VarId => Ok(NTerm::A4),
