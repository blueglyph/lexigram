// Copyright (c) 2026 Redglyph (@gmail.com). All Rights Reserved.

#![cfg(test)]
#![allow(dead_code)]

// Most of the code below is generated by parsergen::tests::wrapper_source::build_items.
// Some of those wrappers are tested in crate::integration::wrappers.

// ================================================================================

pub(crate) mod rules_980_1 {
    /// User-defined type for `a`
    #[derive(Debug, PartialEq)] pub struct SynA();

    /// User-defined type for `s`
    #[derive(Debug, PartialEq)] pub struct SynS();

    /// User-defined type for `p`
    #[derive(Debug, PartialEq)] pub struct SynP();

    /// User-defined type for `vs`
    #[derive(Debug, PartialEq)] pub struct SynVs();

    /// User-defined type for `ns`
    #[derive(Debug, PartialEq)] pub struct SynNs();

    /// User-defined type for `xs`
    #[derive(Debug, PartialEq)] pub struct SynXs();

    /// User-defined type for `vp`
    #[derive(Debug, PartialEq)] pub struct SynVp();

    /// User-defined type for `np`
    #[derive(Debug, PartialEq)] pub struct SynNp();

    /// User-defined type for `xp`
    #[derive(Debug, PartialEq)] pub struct SynXp();
    // ------------------------------------------------------------
    // [wrapper source for rule 980 #1, start a]

    use lexigram_lib::{AltId, TokenId, VarId, lexer::PosSpan, log::{LogMsg, Logger}, parser::{Call, ListenerWrapper, Terminate}};

    #[derive(Debug)]
    pub enum CtxA {
        /// `a -> s p`
        V1 { s: SynS, p: SynP },
    }
    #[derive(Debug)]
    pub enum CtxS {
        /// `s -> vs ns xs`
        V1 { vs: SynVs, ns: SynNs, xs: SynXs },
    }
    #[derive(Debug)]
    pub enum CtxP {
        /// `p -> vp np xp`
        V1 { vp: SynVp, np: SynNp, xp: SynXp },
    }
    #[derive(Debug)]
    pub enum CtxVs {
        /// `vs -> A B*`
        V1 { a: String, star: SynVs1 },
    }
    #[derive(Debug)]
    pub enum CtxNs {
        /// `ns -> "A" "B"*`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxXs {
        /// `xs -> A x*`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxVp {
        /// `vp -> A B+`
        V1 { a: String, plus: SynVp1 },
    }
    #[derive(Debug)]
    pub enum CtxNp {
        /// `np -> "A" "B"+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxXp {
        /// `xp -> A x+`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxX {
        /// `x -> "X"`
        V1,
    }

    /// Computed `B*` array in `vs -> A  ►► B* ◄◄ `
    #[derive(Debug, PartialEq)]
    pub struct SynVs1(pub Vec<String>);
    /// Computed `B+` array in `vp -> A  ►► B+ ◄◄ `
    #[derive(Debug, PartialEq)]
    pub struct SynVp1(pub Vec<String>);

    #[derive(Debug)]
    enum EnumSynValue { A(SynA), S(SynS), P(SynP), Vs(SynVs), Ns(SynNs), Xs(SynXs), Vp(SynVp), Np(SynNp), Xp(SynXp), Vs1(SynVs1), Vp1(SynVp1) }

    impl EnumSynValue {
        fn get_a(self) -> SynA {
            if let EnumSynValue::A(val) = self { val } else { panic!() }
        }
        fn get_s(self) -> SynS {
            if let EnumSynValue::S(val) = self { val } else { panic!() }
        }
        fn get_p(self) -> SynP {
            if let EnumSynValue::P(val) = self { val } else { panic!() }
        }
        fn get_vs(self) -> SynVs {
            if let EnumSynValue::Vs(val) = self { val } else { panic!() }
        }
        fn get_ns(self) -> SynNs {
            if let EnumSynValue::Ns(val) = self { val } else { panic!() }
        }
        fn get_xs(self) -> SynXs {
            if let EnumSynValue::Xs(val) = self { val } else { panic!() }
        }
        fn get_vp(self) -> SynVp {
            if let EnumSynValue::Vp(val) = self { val } else { panic!() }
        }
        fn get_np(self) -> SynNp {
            if let EnumSynValue::Np(val) = self { val } else { panic!() }
        }
        fn get_xp(self) -> SynXp {
            if let EnumSynValue::Xp(val) = self { val } else { panic!() }
        }
        fn get_vs1(self) -> SynVs1 {
            if let EnumSynValue::Vs1(val) = self { val } else { panic!() }
        }
        fn get_vp1(self) -> SynVp1 {
            if let EnumSynValue::Vp1(val) = self { val } else { panic!() }
        }
    }

    pub trait TestListener {
        /// Checks if the listener requests an abort. This happens if an error is too difficult to recover from
        /// and may corrupt the stack content. In that case, the parser immediately stops and returns `ParserError::AbortRequest`.
        fn check_abort_request(&self) -> Terminate { Terminate::None }
        fn get_log_mut(&mut self) -> &mut impl Logger;
        #[allow(unused_variables)]
        fn handle_msg(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.get_log_mut().add(msg);
        }
        #[allow(unused_variables)]
        fn intercept_token(&mut self, token: TokenId, text: &str, span: &PosSpan) -> TokenId { token }
        #[allow(unused_variables)]
        fn exit(&mut self, a: SynA, span: PosSpan) {}
        #[allow(unused_variables)]
        fn abort(&mut self, terminate: Terminate) {}
        fn init_a(&mut self) {}
        fn exit_a(&mut self, ctx: CtxA, spans: Vec<PosSpan>) -> SynA;
        fn init_s(&mut self) {}
        fn exit_s(&mut self, ctx: CtxS, spans: Vec<PosSpan>) -> SynS;
        fn init_p(&mut self) {}
        fn exit_p(&mut self, ctx: CtxP, spans: Vec<PosSpan>) -> SynP;
        fn init_vs(&mut self) {}
        fn exit_vs(&mut self, ctx: CtxVs, spans: Vec<PosSpan>) -> SynVs;
        fn init_ns(&mut self) {}
        fn exit_ns(&mut self, ctx: CtxNs, spans: Vec<PosSpan>) -> SynNs;
        fn init_xs(&mut self) {}
        fn exit_xs(&mut self, ctx: CtxXs, spans: Vec<PosSpan>) -> SynXs;
        fn init_vp(&mut self) {}
        fn exit_vp(&mut self, ctx: CtxVp, spans: Vec<PosSpan>) -> SynVp;
        fn init_np(&mut self) {}
        fn exit_np(&mut self, ctx: CtxNp, spans: Vec<PosSpan>) -> SynNp;
        fn init_xp(&mut self) {}
        fn exit_xp(&mut self, ctx: CtxXp, spans: Vec<PosSpan>) -> SynXp;
        fn init_x(&mut self) {}
        #[allow(unused_variables)]
        fn exit_x(&mut self, ctx: CtxX, spans: Vec<PosSpan>) {}
    }

    pub struct Wrapper<T> {
        verbose: bool,
        listener: T,
        stack: Vec<EnumSynValue>,
        max_stack: usize,
        stack_t: Vec<String>,
        stack_span: Vec<PosSpan>,
    }

    impl<T: TestListener> ListenerWrapper for Wrapper<T> {
        fn switch(&mut self, call: Call, nt: VarId, alt_id: AltId, t_data: Option<Vec<String>>) {
            if self.verbose {
                println!("switch: call={call:?}, nt={nt}, alt={alt_id}, t_data={t_data:?}");
            }
            if let Some(mut t_data) = t_data {
                self.stack_t.append(&mut t_data);
            }
            match call {
                Call::Enter => {
                    if matches!(nt, 10 ..= 15) {
                        self.stack_span.push(PosSpan::empty());
                    }
                    match nt {
                        0 => self.listener.init_a(),                // a
                        1 => self.listener.init_s(),                // s
                        2 => self.listener.init_p(),                // p
                        3 => self.listener.init_vs(),               // vs
                        10 => self.init_vs1(),                      // vs_1
                        4 => self.listener.init_ns(),               // ns
                        11 => {}                                    // ns_1
                        5 => self.listener.init_xs(),               // xs
                        12 => {}                                    // xs_1
                        6 => self.listener.init_vp(),               // vp
                        13 => self.init_vp1(),                      // vp_1
                        16 => {}                                    // vp_2
                        7 => self.listener.init_np(),               // np
                        14 => {}                                    // np_1
                        17 => {}                                    // np_2
                        8 => self.listener.init_xp(),               // xp
                        15 => {}                                    // xp_1
                        18 => {}                                    // xp_2
                        9 => self.listener.init_x(),                // x
                        _ => panic!("unexpected enter nonterminal id: {nt}")
                    }
                }
                Call::Loop => {}
                Call::Exit => {
                    match alt_id {
                        0 => self.exit_a(),                         // a -> s p
                        1 => self.exit_s(),                         // s -> vs ns xs
                        2 => self.exit_p(),                         // p -> vp np xp
                        3 => self.exit_vs(),                        // vs -> A vs_1
                        10 => self.exit_vs1(),                      // vs_1 -> B vs_1
                        11 => {}                                    // vs_1 -> ε
                        4 => self.exit_ns(),                        // ns -> "A" ns_1
                        12 => self.exit_ns1(),                      // ns_1 -> "B" ns_1
                        13 => {}                                    // ns_1 -> ε
                        5 => self.exit_xs(),                        // xs -> A xs_1
                        14 => self.exit_xs1(),                      // xs_1 -> x xs_1
                        15 => {}                                    // xs_1 -> ε
                        6 => self.exit_vp(),                        // vp -> A vp_1
                        19 |                                        // vp_2 -> vp_1
                        20 => self.exit_vp1(),                      // vp_2 -> ε
                     /* 16 */                                       // vp_1 -> B vp_2 (never called)
                        7 => self.exit_np(),                        // np -> "A" np_1
                        21 |                                        // np_2 -> np_1
                        22 => self.exit_np1(),                      // np_2 -> ε
                     /* 17 */                                       // np_1 -> "B" np_2 (never called)
                        8 => self.exit_xp(),                        // xp -> A xp_1
                        23 |                                        // xp_2 -> xp_1
                        24 => self.exit_xp1(),                      // xp_2 -> ε
                     /* 18 */                                       // xp_1 -> x xp_2 (never called)
                        9 => self.exit_x(),                         // x -> "X"
                        _ => panic!("unexpected exit alternative id: {alt_id}")
                    }
                }
                Call::End(terminate) => {
                    match terminate {
                        Terminate::None => {
                            let val = self.stack.pop().unwrap().get_a();
                            let span = self.stack_span.pop().unwrap();
                            self.listener.exit(val, span);
                        }
                        Terminate::Abort | Terminate::Conclude => self.listener.abort(terminate),
                    }
                }
            }
            self.max_stack = std::cmp::max(self.max_stack, self.stack.len());
            if self.verbose {
                println!("> stack_t:   {}", self.stack_t.join(", "));
                println!("> stack:     {}", self.stack.iter().map(|it| format!("{it:?}")).collect::<Vec<_>>().join(", "));
            }
        }

        fn check_abort_request(&self) -> Terminate {
            self.listener.check_abort_request()
        }

        fn abort(&mut self) {
            self.stack.clear();
            self.stack_span.clear();
            self.stack_t.clear();
        }

        fn get_log_mut(&mut self) -> &mut impl Logger {
            self.listener.get_log_mut()
        }

        fn report(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.listener.handle_msg(span_opt, msg);
        }

        fn push_span(&mut self, span: PosSpan) {
            self.stack_span.push(span);
        }

        fn is_stack_empty(&self) -> bool {
            self.stack.is_empty()
        }

        fn is_stack_t_empty(&self) -> bool {
            self.stack_t.is_empty()
        }

        fn is_stack_span_empty(&self) -> bool {
            self.stack_span.is_empty()
        }

        fn intercept_token(&mut self, token: TokenId, text: &str, span: &PosSpan) -> TokenId {
            self.listener.intercept_token(token, text, span)
        }
    }

    impl<T: TestListener> Wrapper<T> {
        pub fn new(listener: T, verbose: bool) -> Self {
            Wrapper { verbose, listener, stack: Vec::new(), max_stack: 0, stack_t: Vec::new(), stack_span: Vec::new() }
        }

        pub fn get_listener(&self) -> &T {
            &self.listener
        }

        pub fn get_listener_mut(&mut self) -> &mut T {
            &mut self.listener
        }

        pub fn give_listener(self) -> T {
            self.listener
        }

        pub fn set_verbose(&mut self, verbose: bool) {
            self.verbose = verbose;
        }

        fn exit_a(&mut self) {
            let p = self.stack.pop().unwrap().get_p();
            let s = self.stack.pop().unwrap().get_s();
            let ctx = CtxA::V1 { s, p };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_a(ctx, spans);
            self.stack.push(EnumSynValue::A(val));
        }

        fn exit_s(&mut self) {
            let xs = self.stack.pop().unwrap().get_xs();
            let ns = self.stack.pop().unwrap().get_ns();
            let vs = self.stack.pop().unwrap().get_vs();
            let ctx = CtxS::V1 { vs, ns, xs };
            let spans = self.stack_span.drain(self.stack_span.len() - 3 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_s(ctx, spans);
            self.stack.push(EnumSynValue::S(val));
        }

        fn exit_p(&mut self) {
            let xp = self.stack.pop().unwrap().get_xp();
            let np = self.stack.pop().unwrap().get_np();
            let vp = self.stack.pop().unwrap().get_vp();
            let ctx = CtxP::V1 { vp, np, xp };
            let spans = self.stack_span.drain(self.stack_span.len() - 3 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_p(ctx, spans);
            self.stack.push(EnumSynValue::P(val));
        }

        fn exit_vs(&mut self) {
            let star = self.stack.pop().unwrap().get_vs1();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVs::V1 { a, star };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_vs(ctx, spans);
            self.stack.push(EnumSynValue::Vs(val));
        }

        fn init_vs1(&mut self) {
            let val = SynVs1(Vec::new());
            self.stack.push(EnumSynValue::Vs1(val));
        }

        fn exit_vs1(&mut self) {
            let b = self.stack_t.pop().unwrap();
            let Some(EnumSynValue::Vs1(SynVs1(star_acc))) = self.stack.last_mut() else {
                panic!("expected SynVs1 item on wrapper stack");
            };
            star_acc.push(b);
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_ns(&mut self) {
            let ctx = CtxNs::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_ns(ctx, spans);
            self.stack.push(EnumSynValue::Ns(val));
        }

        fn exit_ns1(&mut self) {
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_xs(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXs::V1 { a };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_xs(ctx, spans);
            self.stack.push(EnumSynValue::Xs(val));
        }

        fn exit_xs1(&mut self) {
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_vp(&mut self) {
            let plus = self.stack.pop().unwrap().get_vp1();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVp::V1 { a, plus };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_vp(ctx, spans);
            self.stack.push(EnumSynValue::Vp(val));
        }

        fn init_vp1(&mut self) {
            let val = SynVp1(Vec::new());
            self.stack.push(EnumSynValue::Vp1(val));
        }

        fn exit_vp1(&mut self) {
            let b = self.stack_t.pop().unwrap();
            let Some(EnumSynValue::Vp1(SynVp1(plus_acc))) = self.stack.last_mut() else {
                panic!("expected SynVp1 item on wrapper stack");
            };
            plus_acc.push(b);
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_np(&mut self) {
            let ctx = CtxNp::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_np(ctx, spans);
            self.stack.push(EnumSynValue::Np(val));
        }

        fn exit_np1(&mut self) {
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_xp(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXp::V1 { a };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_xp(ctx, spans);
            self.stack.push(EnumSynValue::Xp(val));
        }

        fn exit_xp1(&mut self) {
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_x(&mut self) {
            let ctx = CtxX::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 1 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_x(ctx, spans);
        }
    }

    // [wrapper source for rule 980 #1, start a]
    // ------------------------------------------------------------
}

// ================================================================================

pub(crate) mod rules_980_2 {
    /// User-defined type for `a`
    #[derive(Debug, PartialEq)] pub struct SynA();

    /// User-defined type for `s`
    #[derive(Debug, PartialEq)] pub struct SynS();

    /// User-defined type for `p`
    #[derive(Debug, PartialEq)] pub struct SynP();

    /// User-defined type for `vs`
    #[derive(Debug, PartialEq)] pub struct SynVs();

    /// User-defined type for `ns`
    #[derive(Debug, PartialEq)] pub struct SynNs();

    /// User-defined type for `xs`
    #[derive(Debug, PartialEq)] pub struct SynXs();

    /// User-defined type for `vp`
    #[derive(Debug, PartialEq)] pub struct SynVp();

    /// User-defined type for `np`
    #[derive(Debug, PartialEq)] pub struct SynNp();

    /// User-defined type for `xp`
    #[derive(Debug, PartialEq)] pub struct SynXp();
    // ------------------------------------------------------------
    // [wrapper source for rule 980 #2, start a]

    use lexigram_lib::{AltId, TokenId, VarId, lexer::PosSpan, log::{LogMsg, Logger}, parser::{Call, ListenerWrapper, Terminate}};

    #[derive(Debug)]
    pub enum CtxA {
        /// `a -> s p`
        V1 { s: SynS, p: SynP },
    }
    #[derive(Debug)]
    pub enum CtxS {
        /// `s -> vs ns xs`
        V1 { vs: SynVs, ns: SynNs, xs: SynXs },
    }
    #[derive(Debug)]
    pub enum CtxP {
        /// `p -> vp np xp`
        V1 { vp: SynVp, np: SynNp, xp: SynXp },
    }
    #[derive(Debug)]
    pub enum CtxVs {
        /// `vs -> A B*`
        V1 { a: String, star: SynVs1 },
    }
    #[derive(Debug)]
    pub enum CtxNs {
        /// `ns -> "A" "B"*`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxXs {
        /// `xs -> A x*`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxVp {
        /// `vp -> A B+`
        V1 { a: String, plus: SynVp1 },
    }
    #[derive(Debug)]
    pub enum CtxNp {
        /// `np -> "A" "B"+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxXp {
        /// `xp -> A x+`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxX {
        /// `x -> "X"`
        V1,
    }

    /// Computed `B*` array in `vs -> A  ►► B* ◄◄ `
    #[derive(Debug, PartialEq)]
    pub struct SynVs1(pub Vec<String>);
    /// Computed `B+` array in `vp -> A  ►► B+ ◄◄ `
    #[derive(Debug, PartialEq)]
    pub struct SynVp1(pub Vec<String>);

    #[derive(Debug)]
    enum EnumSynValue { A(SynA), S(SynS), P(SynP), Vs(SynVs), Ns(SynNs), Xs(SynXs), Vp(SynVp), Np(SynNp), Xp(SynXp), Vs1(SynVs1), Vp1(SynVp1) }

    impl EnumSynValue {
        fn get_a(self) -> SynA {
            if let EnumSynValue::A(val) = self { val } else { panic!() }
        }
        fn get_s(self) -> SynS {
            if let EnumSynValue::S(val) = self { val } else { panic!() }
        }
        fn get_p(self) -> SynP {
            if let EnumSynValue::P(val) = self { val } else { panic!() }
        }
        fn get_vs(self) -> SynVs {
            if let EnumSynValue::Vs(val) = self { val } else { panic!() }
        }
        fn get_ns(self) -> SynNs {
            if let EnumSynValue::Ns(val) = self { val } else { panic!() }
        }
        fn get_xs(self) -> SynXs {
            if let EnumSynValue::Xs(val) = self { val } else { panic!() }
        }
        fn get_vp(self) -> SynVp {
            if let EnumSynValue::Vp(val) = self { val } else { panic!() }
        }
        fn get_np(self) -> SynNp {
            if let EnumSynValue::Np(val) = self { val } else { panic!() }
        }
        fn get_xp(self) -> SynXp {
            if let EnumSynValue::Xp(val) = self { val } else { panic!() }
        }
        fn get_vs1(self) -> SynVs1 {
            if let EnumSynValue::Vs1(val) = self { val } else { panic!() }
        }
        fn get_vp1(self) -> SynVp1 {
            if let EnumSynValue::Vp1(val) = self { val } else { panic!() }
        }
    }

    pub trait TestListener {
        /// Checks if the listener requests an abort. This happens if an error is too difficult to recover from
        /// and may corrupt the stack content. In that case, the parser immediately stops and returns `ParserError::AbortRequest`.
        fn check_abort_request(&self) -> Terminate { Terminate::None }
        fn get_log_mut(&mut self) -> &mut impl Logger;
        #[allow(unused_variables)]
        fn handle_msg(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.get_log_mut().add(msg);
        }
        #[allow(unused_variables)]
        fn intercept_token(&mut self, token: TokenId, text: &str) -> TokenId { token }
        #[allow(unused_variables)]
        fn exit(&mut self, a: SynA) {}
        #[allow(unused_variables)]
        fn abort(&mut self, terminate: Terminate) {}
        fn init_a(&mut self) {}
        fn exit_a(&mut self, ctx: CtxA) -> SynA;
        fn init_s(&mut self) {}
        fn exit_s(&mut self, ctx: CtxS) -> SynS;
        fn init_p(&mut self) {}
        fn exit_p(&mut self, ctx: CtxP) -> SynP;
        fn init_vs(&mut self) {}
        fn exit_vs(&mut self, ctx: CtxVs) -> SynVs;
        fn init_ns(&mut self) {}
        fn exit_ns(&mut self, ctx: CtxNs) -> SynNs;
        fn init_xs(&mut self) {}
        fn exit_xs(&mut self, ctx: CtxXs) -> SynXs;
        fn init_vp(&mut self) {}
        fn exit_vp(&mut self, ctx: CtxVp) -> SynVp;
        fn init_np(&mut self) {}
        fn exit_np(&mut self, ctx: CtxNp) -> SynNp;
        fn init_xp(&mut self) {}
        fn exit_xp(&mut self, ctx: CtxXp) -> SynXp;
        fn init_x(&mut self) {}
        #[allow(unused_variables)]
        fn exit_x(&mut self, ctx: CtxX) {}
    }

    pub struct Wrapper<T> {
        verbose: bool,
        listener: T,
        stack: Vec<EnumSynValue>,
        max_stack: usize,
        stack_t: Vec<String>,
    }

    impl<T: TestListener> ListenerWrapper for Wrapper<T> {
        fn switch(&mut self, call: Call, nt: VarId, alt_id: AltId, t_data: Option<Vec<String>>) {
            if self.verbose {
                println!("switch: call={call:?}, nt={nt}, alt={alt_id}, t_data={t_data:?}");
            }
            if let Some(mut t_data) = t_data {
                self.stack_t.append(&mut t_data);
            }
            match call {
                Call::Enter => {
                    match nt {
                        0 => self.listener.init_a(),                // a
                        1 => self.listener.init_s(),                // s
                        2 => self.listener.init_p(),                // p
                        3 => self.listener.init_vs(),               // vs
                        10 => self.init_vs1(),                      // vs_1
                        4 => self.listener.init_ns(),               // ns
                        11 => {}                                    // ns_1
                        5 => self.listener.init_xs(),               // xs
                        12 => {}                                    // xs_1
                        6 => self.listener.init_vp(),               // vp
                        13 => self.init_vp1(),                      // vp_1
                        16 => {}                                    // vp_2
                        7 => self.listener.init_np(),               // np
                        14 => {}                                    // np_1
                        17 => {}                                    // np_2
                        8 => self.listener.init_xp(),               // xp
                        15 => {}                                    // xp_1
                        18 => {}                                    // xp_2
                        9 => self.listener.init_x(),                // x
                        _ => panic!("unexpected enter nonterminal id: {nt}")
                    }
                }
                Call::Loop => {}
                Call::Exit => {
                    match alt_id {
                        0 => self.exit_a(),                         // a -> s p
                        1 => self.exit_s(),                         // s -> vs ns xs
                        2 => self.exit_p(),                         // p -> vp np xp
                        3 => self.exit_vs(),                        // vs -> A vs_1
                        10 => self.exit_vs1(),                      // vs_1 -> B vs_1
                        11 => {}                                    // vs_1 -> ε
                        4 => self.exit_ns(),                        // ns -> "A" ns_1
                        12 |                                        // ns_1 -> "B" ns_1
                        13 => {}                                    // ns_1 -> ε
                        5 => self.exit_xs(),                        // xs -> A xs_1
                        14 |                                        // xs_1 -> x xs_1
                        15 => {}                                    // xs_1 -> ε
                        6 => self.exit_vp(),                        // vp -> A vp_1
                        19 |                                        // vp_2 -> vp_1
                        20 => self.exit_vp1(),                      // vp_2 -> ε
                     /* 16 */                                       // vp_1 -> B vp_2 (never called)
                        7 => self.exit_np(),                        // np -> "A" np_1
                        21 |                                        // np_2 -> np_1
                        22 => {}                                    // np_2 -> ε
                     /* 17 */                                       // np_1 -> "B" np_2 (never called)
                        8 => self.exit_xp(),                        // xp -> A xp_1
                        23 |                                        // xp_2 -> xp_1
                        24 => {}                                    // xp_2 -> ε
                     /* 18 */                                       // xp_1 -> x xp_2 (never called)
                        9 => self.exit_x(),                         // x -> "X"
                        _ => panic!("unexpected exit alternative id: {alt_id}")
                    }
                }
                Call::End(terminate) => {
                    match terminate {
                        Terminate::None => {
                            let val = self.stack.pop().unwrap().get_a();
                            self.listener.exit(val);
                        }
                        Terminate::Abort | Terminate::Conclude => self.listener.abort(terminate),
                    }
                }
            }
            self.max_stack = std::cmp::max(self.max_stack, self.stack.len());
            if self.verbose {
                println!("> stack_t:   {}", self.stack_t.join(", "));
                println!("> stack:     {}", self.stack.iter().map(|it| format!("{it:?}")).collect::<Vec<_>>().join(", "));
            }
        }

        fn check_abort_request(&self) -> Terminate {
            self.listener.check_abort_request()
        }

        fn abort(&mut self) {
            self.stack.clear();
            self.stack_t.clear();
        }

        fn get_log_mut(&mut self) -> &mut impl Logger {
            self.listener.get_log_mut()
        }

        fn report(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.listener.handle_msg(span_opt, msg);
        }

        fn is_stack_empty(&self) -> bool {
            self.stack.is_empty()
        }

        fn is_stack_t_empty(&self) -> bool {
            self.stack_t.is_empty()
        }

        fn intercept_token(&mut self, token: TokenId, text: &str, _span: &PosSpan) -> TokenId {
            self.listener.intercept_token(token, text)
        }
    }

    impl<T: TestListener> Wrapper<T> {
        pub fn new(listener: T, verbose: bool) -> Self {
            Wrapper { verbose, listener, stack: Vec::new(), max_stack: 0, stack_t: Vec::new() }
        }

        pub fn get_listener(&self) -> &T {
            &self.listener
        }

        pub fn get_listener_mut(&mut self) -> &mut T {
            &mut self.listener
        }

        pub fn give_listener(self) -> T {
            self.listener
        }

        pub fn set_verbose(&mut self, verbose: bool) {
            self.verbose = verbose;
        }

        fn exit_a(&mut self) {
            let p = self.stack.pop().unwrap().get_p();
            let s = self.stack.pop().unwrap().get_s();
            let ctx = CtxA::V1 { s, p };
            let val = self.listener.exit_a(ctx);
            self.stack.push(EnumSynValue::A(val));
        }

        fn exit_s(&mut self) {
            let xs = self.stack.pop().unwrap().get_xs();
            let ns = self.stack.pop().unwrap().get_ns();
            let vs = self.stack.pop().unwrap().get_vs();
            let ctx = CtxS::V1 { vs, ns, xs };
            let val = self.listener.exit_s(ctx);
            self.stack.push(EnumSynValue::S(val));
        }

        fn exit_p(&mut self) {
            let xp = self.stack.pop().unwrap().get_xp();
            let np = self.stack.pop().unwrap().get_np();
            let vp = self.stack.pop().unwrap().get_vp();
            let ctx = CtxP::V1 { vp, np, xp };
            let val = self.listener.exit_p(ctx);
            self.stack.push(EnumSynValue::P(val));
        }

        fn exit_vs(&mut self) {
            let star = self.stack.pop().unwrap().get_vs1();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVs::V1 { a, star };
            let val = self.listener.exit_vs(ctx);
            self.stack.push(EnumSynValue::Vs(val));
        }

        fn init_vs1(&mut self) {
            let val = SynVs1(Vec::new());
            self.stack.push(EnumSynValue::Vs1(val));
        }

        fn exit_vs1(&mut self) {
            let b = self.stack_t.pop().unwrap();
            let Some(EnumSynValue::Vs1(SynVs1(star_acc))) = self.stack.last_mut() else {
                panic!("expected SynVs1 item on wrapper stack");
            };
            star_acc.push(b);
        }

        fn exit_ns(&mut self) {
            let ctx = CtxNs::V1;
            let val = self.listener.exit_ns(ctx);
            self.stack.push(EnumSynValue::Ns(val));
        }

        fn exit_xs(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXs::V1 { a };
            let val = self.listener.exit_xs(ctx);
            self.stack.push(EnumSynValue::Xs(val));
        }

        fn exit_vp(&mut self) {
            let plus = self.stack.pop().unwrap().get_vp1();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVp::V1 { a, plus };
            let val = self.listener.exit_vp(ctx);
            self.stack.push(EnumSynValue::Vp(val));
        }

        fn init_vp1(&mut self) {
            let val = SynVp1(Vec::new());
            self.stack.push(EnumSynValue::Vp1(val));
        }

        fn exit_vp1(&mut self) {
            let b = self.stack_t.pop().unwrap();
            let Some(EnumSynValue::Vp1(SynVp1(plus_acc))) = self.stack.last_mut() else {
                panic!("expected SynVp1 item on wrapper stack");
            };
            plus_acc.push(b);
        }

        fn exit_np(&mut self) {
            let ctx = CtxNp::V1;
            let val = self.listener.exit_np(ctx);
            self.stack.push(EnumSynValue::Np(val));
        }

        fn exit_xp(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXp::V1 { a };
            let val = self.listener.exit_xp(ctx);
            self.stack.push(EnumSynValue::Xp(val));
        }

        fn exit_x(&mut self) {
            let ctx = CtxX::V1;
            self.listener.exit_x(ctx);
        }
    }

    // [wrapper source for rule 980 #2, start a]
    // ------------------------------------------------------------
}

// ================================================================================

pub(crate) mod rules_981_1 {
    /// User-defined type for `a`
    #[derive(Debug, PartialEq)] pub struct SynA();

    /// User-defined type for `s`
    #[derive(Debug, PartialEq)] pub struct SynS();

    /// User-defined type for `p`
    #[derive(Debug, PartialEq)] pub struct SynP();

    /// User-defined type for `vs`
    #[derive(Debug, PartialEq)] pub struct SynVs();

    /// User-defined type for `ns`
    #[derive(Debug, PartialEq)] pub struct SynNs();

    /// User-defined type for `xs`
    #[derive(Debug, PartialEq)] pub struct SynXs();

    /// User-defined type for `vp`
    #[derive(Debug, PartialEq)] pub struct SynVp();

    /// User-defined type for `np`
    #[derive(Debug, PartialEq)] pub struct SynNp();

    /// User-defined type for `xp`
    #[derive(Debug, PartialEq)] pub struct SynXp();
    // ------------------------------------------------------------
    // [wrapper source for rule 981 #1, start a]

    use lexigram_lib::{AltId, TokenId, VarId, lexer::PosSpan, log::{LogMsg, Logger}, parser::{Call, ListenerWrapper, Terminate}};

    #[derive(Debug)]
    pub enum CtxA {
        /// `a -> s p`
        V1 { s: SynS, p: SynP },
    }
    #[derive(Debug)]
    pub enum CtxS {
        /// `s -> vs ns xs`
        V1 { vs: SynVs, ns: SynNs, xs: SynXs },
    }
    #[derive(Debug)]
    pub enum CtxP {
        /// `p -> vp np xp`
        V1 { vp: SynVp, np: SynNp, xp: SynXp },
    }
    #[derive(Debug)]
    pub enum CtxVs {
        /// `vs -> A (B | C D)*`
        V1 { a: String, star: SynVs1 },
    }
    #[derive(Debug)]
    pub enum CtxNs {
        /// `ns -> "A" ("B" | "C" "D")*`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxXs {
        /// `xs -> A (x | y x)*`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxVp {
        /// `vp -> A (B | C D)+`
        V1 { a: String, plus: SynVp1 },
    }
    #[derive(Debug)]
    pub enum CtxNp {
        /// `np -> "A" ("B" | "C" "D")+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxXp {
        /// `xp -> A (x | y x)+`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxX {
        /// `x -> "X"`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxY {
        /// `y -> "Y"`
        V1,
    }

    /// Computed `(B | C D)*` array in `vs -> A  ►► (B | C D)* ◄◄ `
    #[derive(Debug, PartialEq)]
    pub struct SynVs1(pub Vec<SynVs1Item>);
    #[derive(Debug, PartialEq)]
    pub enum SynVs1Item {
        /// `B` item in `vs -> A ( ►► B ◄◄  | C D)*`
        V1 { b: String },
        /// `C D` item in `vs -> A (B |  ►► C D ◄◄ )*`
        V2 { c: String, d: String },
    }
    /// Computed `(B | C D)+` array in `vp -> A  ►► (B | C D)+ ◄◄ `
    #[derive(Debug, PartialEq)]
    pub struct SynVp1(pub Vec<SynVp1Item>);
    #[derive(Debug, PartialEq)]
    pub enum SynVp1Item {
        /// `B` item in `vp -> A ( ►► B ◄◄  | C D)+`
        V1 { b: String },
        /// `C D` item in `vp -> A (B |  ►► C D ◄◄ )+`
        V2 { c: String, d: String },
    }

    #[derive(Debug)]
    enum EnumSynValue { A(SynA), S(SynS), P(SynP), Vs(SynVs), Ns(SynNs), Xs(SynXs), Vp(SynVp), Np(SynNp), Xp(SynXp), Vs1(SynVs1), Vp1(SynVp1) }

    impl EnumSynValue {
        fn get_a(self) -> SynA {
            if let EnumSynValue::A(val) = self { val } else { panic!() }
        }
        fn get_s(self) -> SynS {
            if let EnumSynValue::S(val) = self { val } else { panic!() }
        }
        fn get_p(self) -> SynP {
            if let EnumSynValue::P(val) = self { val } else { panic!() }
        }
        fn get_vs(self) -> SynVs {
            if let EnumSynValue::Vs(val) = self { val } else { panic!() }
        }
        fn get_ns(self) -> SynNs {
            if let EnumSynValue::Ns(val) = self { val } else { panic!() }
        }
        fn get_xs(self) -> SynXs {
            if let EnumSynValue::Xs(val) = self { val } else { panic!() }
        }
        fn get_vp(self) -> SynVp {
            if let EnumSynValue::Vp(val) = self { val } else { panic!() }
        }
        fn get_np(self) -> SynNp {
            if let EnumSynValue::Np(val) = self { val } else { panic!() }
        }
        fn get_xp(self) -> SynXp {
            if let EnumSynValue::Xp(val) = self { val } else { panic!() }
        }
        fn get_vs1(self) -> SynVs1 {
            if let EnumSynValue::Vs1(val) = self { val } else { panic!() }
        }
        fn get_vp1(self) -> SynVp1 {
            if let EnumSynValue::Vp1(val) = self { val } else { panic!() }
        }
    }

    pub trait TestListener {
        /// Checks if the listener requests an abort. This happens if an error is too difficult to recover from
        /// and may corrupt the stack content. In that case, the parser immediately stops and returns `ParserError::AbortRequest`.
        fn check_abort_request(&self) -> Terminate { Terminate::None }
        fn get_log_mut(&mut self) -> &mut impl Logger;
        #[allow(unused_variables)]
        fn handle_msg(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.get_log_mut().add(msg);
        }
        #[allow(unused_variables)]
        fn intercept_token(&mut self, token: TokenId, text: &str, span: &PosSpan) -> TokenId { token }
        #[allow(unused_variables)]
        fn exit(&mut self, a: SynA, span: PosSpan) {}
        #[allow(unused_variables)]
        fn abort(&mut self, terminate: Terminate) {}
        fn init_a(&mut self) {}
        fn exit_a(&mut self, ctx: CtxA, spans: Vec<PosSpan>) -> SynA;
        fn init_s(&mut self) {}
        fn exit_s(&mut self, ctx: CtxS, spans: Vec<PosSpan>) -> SynS;
        fn init_p(&mut self) {}
        fn exit_p(&mut self, ctx: CtxP, spans: Vec<PosSpan>) -> SynP;
        fn init_vs(&mut self) {}
        fn exit_vs(&mut self, ctx: CtxVs, spans: Vec<PosSpan>) -> SynVs;
        fn init_ns(&mut self) {}
        fn exit_ns(&mut self, ctx: CtxNs, spans: Vec<PosSpan>) -> SynNs;
        fn init_xs(&mut self) {}
        fn exit_xs(&mut self, ctx: CtxXs, spans: Vec<PosSpan>) -> SynXs;
        fn init_vp(&mut self) {}
        fn exit_vp(&mut self, ctx: CtxVp, spans: Vec<PosSpan>) -> SynVp;
        fn init_np(&mut self) {}
        fn exit_np(&mut self, ctx: CtxNp, spans: Vec<PosSpan>) -> SynNp;
        fn init_xp(&mut self) {}
        fn exit_xp(&mut self, ctx: CtxXp, spans: Vec<PosSpan>) -> SynXp;
        fn init_x(&mut self) {}
        #[allow(unused_variables)]
        fn exit_x(&mut self, ctx: CtxX, spans: Vec<PosSpan>) {}
        fn init_y(&mut self) {}
        #[allow(unused_variables)]
        fn exit_y(&mut self, ctx: CtxY, spans: Vec<PosSpan>) {}
    }

    pub struct Wrapper<T> {
        verbose: bool,
        listener: T,
        stack: Vec<EnumSynValue>,
        max_stack: usize,
        stack_t: Vec<String>,
        stack_span: Vec<PosSpan>,
    }

    impl<T: TestListener> ListenerWrapper for Wrapper<T> {
        fn switch(&mut self, call: Call, nt: VarId, alt_id: AltId, t_data: Option<Vec<String>>) {
            if self.verbose {
                println!("switch: call={call:?}, nt={nt}, alt={alt_id}, t_data={t_data:?}");
            }
            if let Some(mut t_data) = t_data {
                self.stack_t.append(&mut t_data);
            }
            match call {
                Call::Enter => {
                    if matches!(nt, 11 ..= 16) {
                        self.stack_span.push(PosSpan::empty());
                    }
                    match nt {
                        0 => self.listener.init_a(),                // a
                        1 => self.listener.init_s(),                // s
                        2 => self.listener.init_p(),                // p
                        3 => self.listener.init_vs(),               // vs
                        11 => self.init_vs1(),                      // vs_1
                        4 => self.listener.init_ns(),               // ns
                        12 => {}                                    // ns_1
                        5 => self.listener.init_xs(),               // xs
                        13 => {}                                    // xs_1
                        6 => self.listener.init_vp(),               // vp
                        14 => self.init_vp1(),                      // vp_1
                        17 | 18 => {}                               // vp_2, vp_3
                        7 => self.listener.init_np(),               // np
                        15 => {}                                    // np_1
                        19 | 20 => {}                               // np_2, np_3
                        8 => self.listener.init_xp(),               // xp
                        16 => {}                                    // xp_1
                        21 | 22 => {}                               // xp_2, xp_3
                        9 => self.listener.init_x(),                // x
                        10 => self.listener.init_y(),               // y
                        _ => panic!("unexpected enter nonterminal id: {nt}")
                    }
                }
                Call::Loop => {}
                Call::Exit => {
                    match alt_id {
                        0 => self.exit_a(),                         // a -> s p
                        1 => self.exit_s(),                         // s -> vs ns xs
                        2 => self.exit_p(),                         // p -> vp np xp
                        3 => self.exit_vs(),                        // vs -> A vs_1
                        11 |                                        // vs_1 -> B vs_1
                        12 => self.exit_vs1(alt_id),                // vs_1 -> C D vs_1
                        13 => {}                                    // vs_1 -> ε
                        4 => self.exit_ns(),                        // ns -> "A" ns_1
                        14 |                                        // ns_1 -> "B" ns_1
                        15 => self.exit_ns1(alt_id),                // ns_1 -> "C" "D" ns_1
                        16 => {}                                    // ns_1 -> ε
                        5 => self.exit_xs(),                        // xs -> A xs_1
                        17 |                                        // xs_1 -> x xs_1
                        18 => self.exit_xs1(alt_id),                // xs_1 -> y x xs_1
                        19 => {}                                    // xs_1 -> ε
                        6 => self.exit_vp(),                        // vp -> A vp_1
                        26 |                                        // vp_2 -> vp_1
                        27 |                                        // vp_2 -> ε
                        28 |                                        // vp_3 -> vp_1
                        29 => self.exit_vp1(alt_id),                // vp_3 -> ε
                     /* 20 */                                       // vp_1 -> B vp_2 (never called)
                     /* 21 */                                       // vp_1 -> C D vp_3 (never called)
                        7 => self.exit_np(),                        // np -> "A" np_1
                        30 |                                        // np_2 -> np_1
                        31 |                                        // np_2 -> ε
                        32 |                                        // np_3 -> np_1
                        33 => self.exit_np1(alt_id),                // np_3 -> ε
                     /* 22 */                                       // np_1 -> "B" np_2 (never called)
                     /* 23 */                                       // np_1 -> "C" "D" np_3 (never called)
                        8 => self.exit_xp(),                        // xp -> A xp_1
                        34 |                                        // xp_2 -> xp_1
                        35 |                                        // xp_2 -> ε
                        36 |                                        // xp_3 -> xp_1
                        37 => self.exit_xp1(alt_id),                // xp_3 -> ε
                     /* 24 */                                       // xp_1 -> x xp_2 (never called)
                     /* 25 */                                       // xp_1 -> y x xp_3 (never called)
                        9 => self.exit_x(),                         // x -> "X"
                        10 => self.exit_y(),                        // y -> "Y"
                        _ => panic!("unexpected exit alternative id: {alt_id}")
                    }
                }
                Call::End(terminate) => {
                    match terminate {
                        Terminate::None => {
                            let val = self.stack.pop().unwrap().get_a();
                            let span = self.stack_span.pop().unwrap();
                            self.listener.exit(val, span);
                        }
                        Terminate::Abort | Terminate::Conclude => self.listener.abort(terminate),
                    }
                }
            }
            self.max_stack = std::cmp::max(self.max_stack, self.stack.len());
            if self.verbose {
                println!("> stack_t:   {}", self.stack_t.join(", "));
                println!("> stack:     {}", self.stack.iter().map(|it| format!("{it:?}")).collect::<Vec<_>>().join(", "));
            }
        }

        fn check_abort_request(&self) -> Terminate {
            self.listener.check_abort_request()
        }

        fn abort(&mut self) {
            self.stack.clear();
            self.stack_span.clear();
            self.stack_t.clear();
        }

        fn get_log_mut(&mut self) -> &mut impl Logger {
            self.listener.get_log_mut()
        }

        fn report(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.listener.handle_msg(span_opt, msg);
        }

        fn push_span(&mut self, span: PosSpan) {
            self.stack_span.push(span);
        }

        fn is_stack_empty(&self) -> bool {
            self.stack.is_empty()
        }

        fn is_stack_t_empty(&self) -> bool {
            self.stack_t.is_empty()
        }

        fn is_stack_span_empty(&self) -> bool {
            self.stack_span.is_empty()
        }

        fn intercept_token(&mut self, token: TokenId, text: &str, span: &PosSpan) -> TokenId {
            self.listener.intercept_token(token, text, span)
        }
    }

    impl<T: TestListener> Wrapper<T> {
        pub fn new(listener: T, verbose: bool) -> Self {
            Wrapper { verbose, listener, stack: Vec::new(), max_stack: 0, stack_t: Vec::new(), stack_span: Vec::new() }
        }

        pub fn get_listener(&self) -> &T {
            &self.listener
        }

        pub fn get_listener_mut(&mut self) -> &mut T {
            &mut self.listener
        }

        pub fn give_listener(self) -> T {
            self.listener
        }

        pub fn set_verbose(&mut self, verbose: bool) {
            self.verbose = verbose;
        }

        fn exit_a(&mut self) {
            let p = self.stack.pop().unwrap().get_p();
            let s = self.stack.pop().unwrap().get_s();
            let ctx = CtxA::V1 { s, p };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_a(ctx, spans);
            self.stack.push(EnumSynValue::A(val));
        }

        fn exit_s(&mut self) {
            let xs = self.stack.pop().unwrap().get_xs();
            let ns = self.stack.pop().unwrap().get_ns();
            let vs = self.stack.pop().unwrap().get_vs();
            let ctx = CtxS::V1 { vs, ns, xs };
            let spans = self.stack_span.drain(self.stack_span.len() - 3 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_s(ctx, spans);
            self.stack.push(EnumSynValue::S(val));
        }

        fn exit_p(&mut self) {
            let xp = self.stack.pop().unwrap().get_xp();
            let np = self.stack.pop().unwrap().get_np();
            let vp = self.stack.pop().unwrap().get_vp();
            let ctx = CtxP::V1 { vp, np, xp };
            let spans = self.stack_span.drain(self.stack_span.len() - 3 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_p(ctx, spans);
            self.stack.push(EnumSynValue::P(val));
        }

        fn exit_vs(&mut self) {
            let star = self.stack.pop().unwrap().get_vs1();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVs::V1 { a, star };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_vs(ctx, spans);
            self.stack.push(EnumSynValue::Vs(val));
        }

        fn init_vs1(&mut self) {
            let val = SynVs1(Vec::new());
            self.stack.push(EnumSynValue::Vs1(val));
        }

        fn exit_vs1(&mut self, alt_id: AltId) {
            let (n, val) = match alt_id {
                11 => {
                    let b = self.stack_t.pop().unwrap();
                    (2, SynVs1Item::V1 { b })
                }
                12 => {
                    let d = self.stack_t.pop().unwrap();
                    let c = self.stack_t.pop().unwrap();
                    (3, SynVs1Item::V2 { c, d })
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_vs1"),
            };
            let Some(EnumSynValue::Vs1(SynVs1(star_acc))) = self.stack.last_mut() else {
                panic!("expected SynVs1 item on wrapper stack");
            };
            star_acc.push(val);
            let spans = self.stack_span.drain(self.stack_span.len() - n ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_ns(&mut self) {
            let ctx = CtxNs::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_ns(ctx, spans);
            self.stack.push(EnumSynValue::Ns(val));
        }

        fn exit_ns1(&mut self, alt_id: AltId) {
            let n = match alt_id {
                14 => {
                    2
                }
                15 => {
                    3
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_ns1"),
            };
            let spans = self.stack_span.drain(self.stack_span.len() - n ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_xs(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXs::V1 { a };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_xs(ctx, spans);
            self.stack.push(EnumSynValue::Xs(val));
        }

        fn exit_xs1(&mut self, alt_id: AltId) {
            let n = match alt_id {
                17 => {
                    2
                }
                18 => {
                    3
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_xs1"),
            };
            let spans = self.stack_span.drain(self.stack_span.len() - n ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_vp(&mut self) {
            let plus = self.stack.pop().unwrap().get_vp1();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVp::V1 { a, plus };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_vp(ctx, spans);
            self.stack.push(EnumSynValue::Vp(val));
        }

        fn init_vp1(&mut self) {
            let val = SynVp1(Vec::new());
            self.stack.push(EnumSynValue::Vp1(val));
        }

        fn exit_vp1(&mut self, alt_id: AltId) {
            let (n, val) = match alt_id {
                26 | 27 => {
                    let b = self.stack_t.pop().unwrap();
                    (2, SynVp1Item::V1 { b })
                }
                28 | 29 => {
                    let d = self.stack_t.pop().unwrap();
                    let c = self.stack_t.pop().unwrap();
                    (3, SynVp1Item::V2 { c, d })
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_vp1"),
            };
            let Some(EnumSynValue::Vp1(SynVp1(plus_acc))) = self.stack.last_mut() else {
                panic!("expected SynVp1 item on wrapper stack");
            };
            plus_acc.push(val);
            let spans = self.stack_span.drain(self.stack_span.len() - n ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_np(&mut self) {
            let ctx = CtxNp::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_np(ctx, spans);
            self.stack.push(EnumSynValue::Np(val));
        }

        fn exit_np1(&mut self, alt_id: AltId) {
            let n = match alt_id {
                30 | 31 => {
                    2
                }
                32 | 33 => {
                    3
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_np1"),
            };
            let spans = self.stack_span.drain(self.stack_span.len() - n ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_xp(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXp::V1 { a };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_xp(ctx, spans);
            self.stack.push(EnumSynValue::Xp(val));
        }

        fn exit_xp1(&mut self, alt_id: AltId) {
            let n = match alt_id {
                34 | 35 => {
                    2
                }
                36 | 37 => {
                    3
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_xp1"),
            };
            let spans = self.stack_span.drain(self.stack_span.len() - n ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_x(&mut self) {
            let ctx = CtxX::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 1 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_x(ctx, spans);
        }

        fn exit_y(&mut self) {
            let ctx = CtxY::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 1 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_y(ctx, spans);
        }
    }

    // [wrapper source for rule 981 #1, start a]
    // ------------------------------------------------------------
}

// ================================================================================

pub(crate) mod rules_981_2 {
    /// User-defined type for `a`
    #[derive(Debug, PartialEq)] pub struct SynA();

    /// User-defined type for `s`
    #[derive(Debug, PartialEq)] pub struct SynS();

    /// User-defined type for `p`
    #[derive(Debug, PartialEq)] pub struct SynP();

    /// User-defined type for `vs`
    #[derive(Debug, PartialEq)] pub struct SynVs();

    /// User-defined type for `ns`
    #[derive(Debug, PartialEq)] pub struct SynNs();

    /// User-defined type for `xs`
    #[derive(Debug, PartialEq)] pub struct SynXs();

    /// User-defined type for `vp`
    #[derive(Debug, PartialEq)] pub struct SynVp();

    /// User-defined type for `np`
    #[derive(Debug, PartialEq)] pub struct SynNp();

    /// User-defined type for `xp`
    #[derive(Debug, PartialEq)] pub struct SynXp();
    // ------------------------------------------------------------
    // [wrapper source for rule 981 #2, start a]

    use lexigram_lib::{AltId, TokenId, VarId, lexer::PosSpan, log::{LogMsg, Logger}, parser::{Call, ListenerWrapper, Terminate}};

    #[derive(Debug)]
    pub enum CtxA {
        /// `a -> s p`
        V1 { s: SynS, p: SynP },
    }
    #[derive(Debug)]
    pub enum CtxS {
        /// `s -> vs ns xs`
        V1 { vs: SynVs, ns: SynNs, xs: SynXs },
    }
    #[derive(Debug)]
    pub enum CtxP {
        /// `p -> vp np xp`
        V1 { vp: SynVp, np: SynNp, xp: SynXp },
    }
    #[derive(Debug)]
    pub enum CtxVs {
        /// `vs -> A (B | C D)*`
        V1 { a: String, star: SynVs1 },
    }
    #[derive(Debug)]
    pub enum CtxNs {
        /// `ns -> "A" ("B" | "C" "D")*`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxXs {
        /// `xs -> A (x | y x)*`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxVp {
        /// `vp -> A (B | C D)+`
        V1 { a: String, plus: SynVp1 },
    }
    #[derive(Debug)]
    pub enum CtxNp {
        /// `np -> "A" ("B" | "C" "D")+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxXp {
        /// `xp -> A (x | y x)+`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxX {
        /// `x -> "X"`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxY {
        /// `y -> "Y"`
        V1,
    }

    /// Computed `(B | C D)*` array in `vs -> A  ►► (B | C D)* ◄◄ `
    #[derive(Debug, PartialEq)]
    pub struct SynVs1(pub Vec<SynVs1Item>);
    #[derive(Debug, PartialEq)]
    pub enum SynVs1Item {
        /// `B` item in `vs -> A ( ►► B ◄◄  | C D)*`
        V1 { b: String },
        /// `C D` item in `vs -> A (B |  ►► C D ◄◄ )*`
        V2 { c: String, d: String },
    }
    /// Computed `(B | C D)+` array in `vp -> A  ►► (B | C D)+ ◄◄ `
    #[derive(Debug, PartialEq)]
    pub struct SynVp1(pub Vec<SynVp1Item>);
    #[derive(Debug, PartialEq)]
    pub enum SynVp1Item {
        /// `B` item in `vp -> A ( ►► B ◄◄  | C D)+`
        V1 { b: String },
        /// `C D` item in `vp -> A (B |  ►► C D ◄◄ )+`
        V2 { c: String, d: String },
    }

    #[derive(Debug)]
    enum EnumSynValue { A(SynA), S(SynS), P(SynP), Vs(SynVs), Ns(SynNs), Xs(SynXs), Vp(SynVp), Np(SynNp), Xp(SynXp), Vs1(SynVs1), Vp1(SynVp1) }

    impl EnumSynValue {
        fn get_a(self) -> SynA {
            if let EnumSynValue::A(val) = self { val } else { panic!() }
        }
        fn get_s(self) -> SynS {
            if let EnumSynValue::S(val) = self { val } else { panic!() }
        }
        fn get_p(self) -> SynP {
            if let EnumSynValue::P(val) = self { val } else { panic!() }
        }
        fn get_vs(self) -> SynVs {
            if let EnumSynValue::Vs(val) = self { val } else { panic!() }
        }
        fn get_ns(self) -> SynNs {
            if let EnumSynValue::Ns(val) = self { val } else { panic!() }
        }
        fn get_xs(self) -> SynXs {
            if let EnumSynValue::Xs(val) = self { val } else { panic!() }
        }
        fn get_vp(self) -> SynVp {
            if let EnumSynValue::Vp(val) = self { val } else { panic!() }
        }
        fn get_np(self) -> SynNp {
            if let EnumSynValue::Np(val) = self { val } else { panic!() }
        }
        fn get_xp(self) -> SynXp {
            if let EnumSynValue::Xp(val) = self { val } else { panic!() }
        }
        fn get_vs1(self) -> SynVs1 {
            if let EnumSynValue::Vs1(val) = self { val } else { panic!() }
        }
        fn get_vp1(self) -> SynVp1 {
            if let EnumSynValue::Vp1(val) = self { val } else { panic!() }
        }
    }

    pub trait TestListener {
        /// Checks if the listener requests an abort. This happens if an error is too difficult to recover from
        /// and may corrupt the stack content. In that case, the parser immediately stops and returns `ParserError::AbortRequest`.
        fn check_abort_request(&self) -> Terminate { Terminate::None }
        fn get_log_mut(&mut self) -> &mut impl Logger;
        #[allow(unused_variables)]
        fn handle_msg(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.get_log_mut().add(msg);
        }
        #[allow(unused_variables)]
        fn intercept_token(&mut self, token: TokenId, text: &str) -> TokenId { token }
        #[allow(unused_variables)]
        fn exit(&mut self, a: SynA) {}
        #[allow(unused_variables)]
        fn abort(&mut self, terminate: Terminate) {}
        fn init_a(&mut self) {}
        fn exit_a(&mut self, ctx: CtxA) -> SynA;
        fn init_s(&mut self) {}
        fn exit_s(&mut self, ctx: CtxS) -> SynS;
        fn init_p(&mut self) {}
        fn exit_p(&mut self, ctx: CtxP) -> SynP;
        fn init_vs(&mut self) {}
        fn exit_vs(&mut self, ctx: CtxVs) -> SynVs;
        fn init_ns(&mut self) {}
        fn exit_ns(&mut self, ctx: CtxNs) -> SynNs;
        fn init_xs(&mut self) {}
        fn exit_xs(&mut self, ctx: CtxXs) -> SynXs;
        fn init_vp(&mut self) {}
        fn exit_vp(&mut self, ctx: CtxVp) -> SynVp;
        fn init_np(&mut self) {}
        fn exit_np(&mut self, ctx: CtxNp) -> SynNp;
        fn init_xp(&mut self) {}
        fn exit_xp(&mut self, ctx: CtxXp) -> SynXp;
        fn init_x(&mut self) {}
        #[allow(unused_variables)]
        fn exit_x(&mut self, ctx: CtxX) {}
        fn init_y(&mut self) {}
        #[allow(unused_variables)]
        fn exit_y(&mut self, ctx: CtxY) {}
    }

    pub struct Wrapper<T> {
        verbose: bool,
        listener: T,
        stack: Vec<EnumSynValue>,
        max_stack: usize,
        stack_t: Vec<String>,
    }

    impl<T: TestListener> ListenerWrapper for Wrapper<T> {
        fn switch(&mut self, call: Call, nt: VarId, alt_id: AltId, t_data: Option<Vec<String>>) {
            if self.verbose {
                println!("switch: call={call:?}, nt={nt}, alt={alt_id}, t_data={t_data:?}");
            }
            if let Some(mut t_data) = t_data {
                self.stack_t.append(&mut t_data);
            }
            match call {
                Call::Enter => {
                    match nt {
                        0 => self.listener.init_a(),                // a
                        1 => self.listener.init_s(),                // s
                        2 => self.listener.init_p(),                // p
                        3 => self.listener.init_vs(),               // vs
                        11 => self.init_vs1(),                      // vs_1
                        4 => self.listener.init_ns(),               // ns
                        12 => {}                                    // ns_1
                        5 => self.listener.init_xs(),               // xs
                        13 => {}                                    // xs_1
                        6 => self.listener.init_vp(),               // vp
                        14 => self.init_vp1(),                      // vp_1
                        17 | 18 => {}                               // vp_2, vp_3
                        7 => self.listener.init_np(),               // np
                        15 => {}                                    // np_1
                        19 | 20 => {}                               // np_2, np_3
                        8 => self.listener.init_xp(),               // xp
                        16 => {}                                    // xp_1
                        21 | 22 => {}                               // xp_2, xp_3
                        9 => self.listener.init_x(),                // x
                        10 => self.listener.init_y(),               // y
                        _ => panic!("unexpected enter nonterminal id: {nt}")
                    }
                }
                Call::Loop => {}
                Call::Exit => {
                    match alt_id {
                        0 => self.exit_a(),                         // a -> s p
                        1 => self.exit_s(),                         // s -> vs ns xs
                        2 => self.exit_p(),                         // p -> vp np xp
                        3 => self.exit_vs(),                        // vs -> A vs_1
                        11 |                                        // vs_1 -> B vs_1
                        12 => self.exit_vs1(alt_id),                // vs_1 -> C D vs_1
                        13 => {}                                    // vs_1 -> ε
                        4 => self.exit_ns(),                        // ns -> "A" ns_1
                        14 |                                        // ns_1 -> "B" ns_1
                        15 |                                        // ns_1 -> "C" "D" ns_1
                        16 => {}                                    // ns_1 -> ε
                        5 => self.exit_xs(),                        // xs -> A xs_1
                        17 |                                        // xs_1 -> x xs_1
                        18 |                                        // xs_1 -> y x xs_1
                        19 => {}                                    // xs_1 -> ε
                        6 => self.exit_vp(),                        // vp -> A vp_1
                        26 |                                        // vp_2 -> vp_1
                        27 |                                        // vp_2 -> ε
                        28 |                                        // vp_3 -> vp_1
                        29 => self.exit_vp1(alt_id),                // vp_3 -> ε
                     /* 20 */                                       // vp_1 -> B vp_2 (never called)
                     /* 21 */                                       // vp_1 -> C D vp_3 (never called)
                        7 => self.exit_np(),                        // np -> "A" np_1
                        30 |                                        // np_2 -> np_1
                        31 |                                        // np_2 -> ε
                        32 |                                        // np_3 -> np_1
                        33 => {}                                    // np_3 -> ε
                     /* 22 */                                       // np_1 -> "B" np_2 (never called)
                     /* 23 */                                       // np_1 -> "C" "D" np_3 (never called)
                        8 => self.exit_xp(),                        // xp -> A xp_1
                        34 |                                        // xp_2 -> xp_1
                        35 |                                        // xp_2 -> ε
                        36 |                                        // xp_3 -> xp_1
                        37 => {}                                    // xp_3 -> ε
                     /* 24 */                                       // xp_1 -> x xp_2 (never called)
                     /* 25 */                                       // xp_1 -> y x xp_3 (never called)
                        9 => self.exit_x(),                         // x -> "X"
                        10 => self.exit_y(),                        // y -> "Y"
                        _ => panic!("unexpected exit alternative id: {alt_id}")
                    }
                }
                Call::End(terminate) => {
                    match terminate {
                        Terminate::None => {
                            let val = self.stack.pop().unwrap().get_a();
                            self.listener.exit(val);
                        }
                        Terminate::Abort | Terminate::Conclude => self.listener.abort(terminate),
                    }
                }
            }
            self.max_stack = std::cmp::max(self.max_stack, self.stack.len());
            if self.verbose {
                println!("> stack_t:   {}", self.stack_t.join(", "));
                println!("> stack:     {}", self.stack.iter().map(|it| format!("{it:?}")).collect::<Vec<_>>().join(", "));
            }
        }

        fn check_abort_request(&self) -> Terminate {
            self.listener.check_abort_request()
        }

        fn abort(&mut self) {
            self.stack.clear();
            self.stack_t.clear();
        }

        fn get_log_mut(&mut self) -> &mut impl Logger {
            self.listener.get_log_mut()
        }

        fn report(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.listener.handle_msg(span_opt, msg);
        }

        fn is_stack_empty(&self) -> bool {
            self.stack.is_empty()
        }

        fn is_stack_t_empty(&self) -> bool {
            self.stack_t.is_empty()
        }

        fn intercept_token(&mut self, token: TokenId, text: &str, _span: &PosSpan) -> TokenId {
            self.listener.intercept_token(token, text)
        }
    }

    impl<T: TestListener> Wrapper<T> {
        pub fn new(listener: T, verbose: bool) -> Self {
            Wrapper { verbose, listener, stack: Vec::new(), max_stack: 0, stack_t: Vec::new() }
        }

        pub fn get_listener(&self) -> &T {
            &self.listener
        }

        pub fn get_listener_mut(&mut self) -> &mut T {
            &mut self.listener
        }

        pub fn give_listener(self) -> T {
            self.listener
        }

        pub fn set_verbose(&mut self, verbose: bool) {
            self.verbose = verbose;
        }

        fn exit_a(&mut self) {
            let p = self.stack.pop().unwrap().get_p();
            let s = self.stack.pop().unwrap().get_s();
            let ctx = CtxA::V1 { s, p };
            let val = self.listener.exit_a(ctx);
            self.stack.push(EnumSynValue::A(val));
        }

        fn exit_s(&mut self) {
            let xs = self.stack.pop().unwrap().get_xs();
            let ns = self.stack.pop().unwrap().get_ns();
            let vs = self.stack.pop().unwrap().get_vs();
            let ctx = CtxS::V1 { vs, ns, xs };
            let val = self.listener.exit_s(ctx);
            self.stack.push(EnumSynValue::S(val));
        }

        fn exit_p(&mut self) {
            let xp = self.stack.pop().unwrap().get_xp();
            let np = self.stack.pop().unwrap().get_np();
            let vp = self.stack.pop().unwrap().get_vp();
            let ctx = CtxP::V1 { vp, np, xp };
            let val = self.listener.exit_p(ctx);
            self.stack.push(EnumSynValue::P(val));
        }

        fn exit_vs(&mut self) {
            let star = self.stack.pop().unwrap().get_vs1();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVs::V1 { a, star };
            let val = self.listener.exit_vs(ctx);
            self.stack.push(EnumSynValue::Vs(val));
        }

        fn init_vs1(&mut self) {
            let val = SynVs1(Vec::new());
            self.stack.push(EnumSynValue::Vs1(val));
        }

        fn exit_vs1(&mut self, alt_id: AltId) {
            let val = match alt_id {
                11 => {
                    let b = self.stack_t.pop().unwrap();
                    SynVs1Item::V1 { b }
                }
                12 => {
                    let d = self.stack_t.pop().unwrap();
                    let c = self.stack_t.pop().unwrap();
                    SynVs1Item::V2 { c, d }
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_vs1"),
            };
            let Some(EnumSynValue::Vs1(SynVs1(star_acc))) = self.stack.last_mut() else {
                panic!("expected SynVs1 item on wrapper stack");
            };
            star_acc.push(val);
        }

        fn exit_ns(&mut self) {
            let ctx = CtxNs::V1;
            let val = self.listener.exit_ns(ctx);
            self.stack.push(EnumSynValue::Ns(val));
        }

        fn exit_xs(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXs::V1 { a };
            let val = self.listener.exit_xs(ctx);
            self.stack.push(EnumSynValue::Xs(val));
        }

        fn exit_vp(&mut self) {
            let plus = self.stack.pop().unwrap().get_vp1();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVp::V1 { a, plus };
            let val = self.listener.exit_vp(ctx);
            self.stack.push(EnumSynValue::Vp(val));
        }

        fn init_vp1(&mut self) {
            let val = SynVp1(Vec::new());
            self.stack.push(EnumSynValue::Vp1(val));
        }

        fn exit_vp1(&mut self, alt_id: AltId) {
            let val = match alt_id {
                26 | 27 => {
                    let b = self.stack_t.pop().unwrap();
                    SynVp1Item::V1 { b }
                }
                28 | 29 => {
                    let d = self.stack_t.pop().unwrap();
                    let c = self.stack_t.pop().unwrap();
                    SynVp1Item::V2 { c, d }
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_vp1"),
            };
            let Some(EnumSynValue::Vp1(SynVp1(plus_acc))) = self.stack.last_mut() else {
                panic!("expected SynVp1 item on wrapper stack");
            };
            plus_acc.push(val);
        }

        fn exit_np(&mut self) {
            let ctx = CtxNp::V1;
            let val = self.listener.exit_np(ctx);
            self.stack.push(EnumSynValue::Np(val));
        }

        fn exit_xp(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXp::V1 { a };
            let val = self.listener.exit_xp(ctx);
            self.stack.push(EnumSynValue::Xp(val));
        }

        fn exit_x(&mut self) {
            let ctx = CtxX::V1;
            self.listener.exit_x(ctx);
        }

        fn exit_y(&mut self) {
            let ctx = CtxY::V1;
            self.listener.exit_y(ctx);
        }
    }

    // [wrapper source for rule 981 #2, start a]
    // ------------------------------------------------------------
}

// ================================================================================

pub(crate) mod rules_982_1 {
    /// User-defined type for `a`
    #[derive(Debug, PartialEq)] pub struct SynA();

    /// User-defined type for `s`
    #[derive(Debug, PartialEq)] pub struct SynS();

    /// User-defined type for `p`
    #[derive(Debug, PartialEq)] pub struct SynP();

    /// User-defined type for `vs`
    #[derive(Debug, PartialEq)] pub struct SynVs();

    /// User-defined type for `<L> B` iteration in `vs -> A ( ►► <L> B ◄◄ )*`
    #[derive(Debug, PartialEq)] pub struct SynIvs();

    /// User-defined type for `ns`
    #[derive(Debug, PartialEq)] pub struct SynNs();

    /// User-defined type for `xs`
    #[derive(Debug, PartialEq)] pub struct SynXs();

    /// User-defined type for `vp`
    #[derive(Debug, PartialEq)] pub struct SynVp();

    /// User-defined type for `<L> B` iteration in `vp -> A ( ►► <L> B ◄◄ )+`
    #[derive(Debug, PartialEq)] pub struct SynIvp();

    /// User-defined type for `np`
    #[derive(Debug, PartialEq)] pub struct SynNp();

    /// User-defined type for `xp`
    #[derive(Debug, PartialEq)] pub struct SynXp();
    // ------------------------------------------------------------
    // [wrapper source for rule 982 #1, start a]

    use lexigram_lib::{AltId, TokenId, VarId, lexer::PosSpan, log::{LogMsg, Logger}, parser::{Call, ListenerWrapper, Terminate}};

    #[derive(Debug)]
    pub enum CtxA {
        /// `a -> s p`
        V1 { s: SynS, p: SynP },
    }
    #[derive(Debug)]
    pub enum CtxS {
        /// `s -> vs ns xs`
        V1 { vs: SynVs, ns: SynNs, xs: SynXs },
    }
    #[derive(Debug)]
    pub enum CtxP {
        /// `p -> vp np xp`
        V1 { vp: SynVp, np: SynNp, xp: SynXp },
    }
    #[derive(Debug)]
    pub enum CtxVs {
        /// `vs -> A (<L> B)*`
        V1 { a: String, star: SynIvs },
    }
    #[derive(Debug)]
    pub enum CtxIvs {
        /// `<L> B` iteration in `vs -> A ( ►► <L> B ◄◄ )*`
        V1 { b: String },
    }
    #[derive(Debug)]
    pub enum CtxNs {
        /// `ns -> "A" (<L> "B")*`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxIns {
        /// `<L> "B"` iteration in `ns -> "A" ( ►► <L> "B" ◄◄ )*`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxXs {
        /// `xs -> A (<L> x)*`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxIxs {
        /// `<L> x` iteration in `xs -> A ( ►► <L> x ◄◄ )*`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxVp {
        /// `vp -> A (<L> B)+`
        V1 { a: String, plus: SynIvp },
    }
    #[derive(Debug)]
    pub enum CtxIvp {
        /// `<L> B` iteration in `vp -> A ( ►► <L> B ◄◄ )+`
        V1 { b: String, last_iteration: bool },
    }
    #[derive(Debug)]
    pub enum CtxNp {
        /// `np -> "A" (<L> "B")+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxInp {
        /// `<L> "B"` iteration in `np -> "A" ( ►► <L> "B" ◄◄ )+`
        V1 { last_iteration: bool },
    }
    #[derive(Debug)]
    pub enum CtxXp {
        /// `xp -> A (<L> x)+`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxIxp {
        /// `<L> x` iteration in `xp -> A ( ►► <L> x ◄◄ )+`
        V1 { last_iteration: bool },
    }
    #[derive(Debug)]
    pub enum CtxX {
        /// `x -> "X"`
        V1,
    }

    #[derive(Debug)]
    enum EnumSynValue { A(SynA), S(SynS), P(SynP), Vs(SynVs), Ivs(SynIvs), Ns(SynNs), Xs(SynXs), Vp(SynVp), Ivp(SynIvp), Np(SynNp), Xp(SynXp) }

    impl EnumSynValue {
        fn get_a(self) -> SynA {
            if let EnumSynValue::A(val) = self { val } else { panic!() }
        }
        fn get_s(self) -> SynS {
            if let EnumSynValue::S(val) = self { val } else { panic!() }
        }
        fn get_p(self) -> SynP {
            if let EnumSynValue::P(val) = self { val } else { panic!() }
        }
        fn get_vs(self) -> SynVs {
            if let EnumSynValue::Vs(val) = self { val } else { panic!() }
        }
        fn get_ivs(self) -> SynIvs {
            if let EnumSynValue::Ivs(val) = self { val } else { panic!() }
        }
        fn get_ns(self) -> SynNs {
            if let EnumSynValue::Ns(val) = self { val } else { panic!() }
        }
        fn get_xs(self) -> SynXs {
            if let EnumSynValue::Xs(val) = self { val } else { panic!() }
        }
        fn get_vp(self) -> SynVp {
            if let EnumSynValue::Vp(val) = self { val } else { panic!() }
        }
        fn get_ivp(self) -> SynIvp {
            if let EnumSynValue::Ivp(val) = self { val } else { panic!() }
        }
        fn get_np(self) -> SynNp {
            if let EnumSynValue::Np(val) = self { val } else { panic!() }
        }
        fn get_xp(self) -> SynXp {
            if let EnumSynValue::Xp(val) = self { val } else { panic!() }
        }
    }

    pub trait TestListener {
        /// Checks if the listener requests an abort. This happens if an error is too difficult to recover from
        /// and may corrupt the stack content. In that case, the parser immediately stops and returns `ParserError::AbortRequest`.
        fn check_abort_request(&self) -> Terminate { Terminate::None }
        fn get_log_mut(&mut self) -> &mut impl Logger;
        #[allow(unused_variables)]
        fn handle_msg(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.get_log_mut().add(msg);
        }
        #[allow(unused_variables)]
        fn intercept_token(&mut self, token: TokenId, text: &str, span: &PosSpan) -> TokenId { token }
        #[allow(unused_variables)]
        fn exit(&mut self, a: SynA, span: PosSpan) {}
        #[allow(unused_variables)]
        fn abort(&mut self, terminate: Terminate) {}
        fn init_a(&mut self) {}
        fn exit_a(&mut self, ctx: CtxA, spans: Vec<PosSpan>) -> SynA;
        fn init_s(&mut self) {}
        fn exit_s(&mut self, ctx: CtxS, spans: Vec<PosSpan>) -> SynS;
        fn init_p(&mut self) {}
        fn exit_p(&mut self, ctx: CtxP, spans: Vec<PosSpan>) -> SynP;
        fn init_vs(&mut self) {}
        fn exit_vs(&mut self, ctx: CtxVs, spans: Vec<PosSpan>) -> SynVs;
        fn init_ivs(&mut self) -> SynIvs;
        fn exit_ivs(&mut self, acc: &mut SynIvs, ctx: CtxIvs, spans: Vec<PosSpan>);
        #[allow(unused_variables)]
        fn exitloop_ivs(&mut self, acc: &mut SynIvs) {}
        fn init_ns(&mut self) {}
        fn exit_ns(&mut self, ctx: CtxNs, spans: Vec<PosSpan>) -> SynNs;
        fn init_ins(&mut self) {}
        #[allow(unused_variables)]
        fn exit_ins(&mut self, ctx: CtxIns, spans: Vec<PosSpan>) {}
        #[allow(unused_variables)]
        fn exitloop_ins(&mut self) {}
        fn init_xs(&mut self) {}
        fn exit_xs(&mut self, ctx: CtxXs, spans: Vec<PosSpan>) -> SynXs;
        fn init_ixs(&mut self) {}
        #[allow(unused_variables)]
        fn exit_ixs(&mut self, ctx: CtxIxs, spans: Vec<PosSpan>) {}
        #[allow(unused_variables)]
        fn exitloop_ixs(&mut self) {}
        fn init_vp(&mut self) {}
        fn exit_vp(&mut self, ctx: CtxVp, spans: Vec<PosSpan>) -> SynVp;
        fn init_ivp(&mut self) -> SynIvp;
        fn exit_ivp(&mut self, acc: &mut SynIvp, ctx: CtxIvp, spans: Vec<PosSpan>);
        fn init_np(&mut self) {}
        fn exit_np(&mut self, ctx: CtxNp, spans: Vec<PosSpan>) -> SynNp;
        fn init_inp(&mut self) {}
        #[allow(unused_variables)]
        fn exit_inp(&mut self, ctx: CtxInp, spans: Vec<PosSpan>) {}
        fn init_xp(&mut self) {}
        fn exit_xp(&mut self, ctx: CtxXp, spans: Vec<PosSpan>) -> SynXp;
        fn init_ixp(&mut self) {}
        #[allow(unused_variables)]
        fn exit_ixp(&mut self, ctx: CtxIxp, spans: Vec<PosSpan>) {}
        fn init_x(&mut self) {}
        #[allow(unused_variables)]
        fn exit_x(&mut self, ctx: CtxX, spans: Vec<PosSpan>) {}
    }

    pub struct Wrapper<T> {
        verbose: bool,
        listener: T,
        stack: Vec<EnumSynValue>,
        max_stack: usize,
        stack_t: Vec<String>,
        stack_span: Vec<PosSpan>,
    }

    impl<T: TestListener> ListenerWrapper for Wrapper<T> {
        fn switch(&mut self, call: Call, nt: VarId, alt_id: AltId, t_data: Option<Vec<String>>) {
            if self.verbose {
                println!("switch: call={call:?}, nt={nt}, alt={alt_id}, t_data={t_data:?}");
            }
            if let Some(mut t_data) = t_data {
                self.stack_t.append(&mut t_data);
            }
            match call {
                Call::Enter => {
                    if matches!(nt, 4 | 6 | 8 | 10 | 12 | 14) {
                        self.stack_span.push(PosSpan::empty());
                    }
                    match nt {
                        0 => self.listener.init_a(),                // a
                        1 => self.listener.init_s(),                // s
                        2 => self.listener.init_p(),                // p
                        3 => self.listener.init_vs(),               // vs
                        4 => self.init_ivs(),                       // ivs
                        5 => self.listener.init_ns(),               // ns
                        6 => self.listener.init_ins(),              // ins
                        7 => self.listener.init_xs(),               // xs
                        8 => self.listener.init_ixs(),              // ixs
                        9 => self.listener.init_vp(),               // vp
                        10 => self.init_ivp(),                      // ivp
                        16 => {}                                    // ivp_1
                        11 => self.listener.init_np(),              // np
                        12 => self.listener.init_inp(),             // inp
                        17 => {}                                    // inp_1
                        13 => self.listener.init_xp(),              // xp
                        14 => self.listener.init_ixp(),             // ixp
                        18 => {}                                    // ixp_1
                        15 => self.listener.init_x(),               // x
                        _ => panic!("unexpected enter nonterminal id: {nt}")
                    }
                }
                Call::Loop => {}
                Call::Exit => {
                    match alt_id {
                        0 => self.exit_a(),                         // a -> s p
                        1 => self.exit_s(),                         // s -> vs ns xs
                        2 => self.exit_p(),                         // p -> vp np xp
                        3 => self.exit_vs(),                        // vs -> A ivs
                        4 => self.exit_ivs(),                       // ivs -> <L> B ivs
                        5 => self.exitloop_ivs(),                   // ivs -> <L> ε
                        6 => self.exit_ns(),                        // ns -> "A" ins
                        7 => self.exit_ins(),                       // ins -> <L> "B" ins
                        8 => self.listener.exitloop_ins(),          // ins -> <L> ε
                        9 => self.exit_xs(),                        // xs -> A ixs
                        10 => self.exit_ixs(),                      // ixs -> <L> x ixs
                        11 => self.listener.exitloop_ixs(),         // ixs -> <L> ε
                        12 => self.exit_vp(),                       // vp -> A ivp
                        19 |                                        // ivp_1 -> ivp
                        20 => self.exit_ivp(alt_id),                // ivp_1 -> ε
                     /* 13 */                                       // ivp -> <L> B ivp_1 (never called)
                        14 => self.exit_np(),                       // np -> "A" inp
                        21 |                                        // inp_1 -> inp
                        22 => self.exit_inp(alt_id),                // inp_1 -> ε
                     /* 15 */                                       // inp -> <L> "B" inp_1 (never called)
                        16 => self.exit_xp(),                       // xp -> A ixp
                        23 |                                        // ixp_1 -> ixp
                        24 => self.exit_ixp(alt_id),                // ixp_1 -> ε
                     /* 17 */                                       // ixp -> <L> x ixp_1 (never called)
                        18 => self.exit_x(),                        // x -> "X"
                        _ => panic!("unexpected exit alternative id: {alt_id}")
                    }
                }
                Call::End(terminate) => {
                    match terminate {
                        Terminate::None => {
                            let val = self.stack.pop().unwrap().get_a();
                            let span = self.stack_span.pop().unwrap();
                            self.listener.exit(val, span);
                        }
                        Terminate::Abort | Terminate::Conclude => self.listener.abort(terminate),
                    }
                }
            }
            self.max_stack = std::cmp::max(self.max_stack, self.stack.len());
            if self.verbose {
                println!("> stack_t:   {}", self.stack_t.join(", "));
                println!("> stack:     {}", self.stack.iter().map(|it| format!("{it:?}")).collect::<Vec<_>>().join(", "));
            }
        }

        fn check_abort_request(&self) -> Terminate {
            self.listener.check_abort_request()
        }

        fn abort(&mut self) {
            self.stack.clear();
            self.stack_span.clear();
            self.stack_t.clear();
        }

        fn get_log_mut(&mut self) -> &mut impl Logger {
            self.listener.get_log_mut()
        }

        fn report(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.listener.handle_msg(span_opt, msg);
        }

        fn push_span(&mut self, span: PosSpan) {
            self.stack_span.push(span);
        }

        fn is_stack_empty(&self) -> bool {
            self.stack.is_empty()
        }

        fn is_stack_t_empty(&self) -> bool {
            self.stack_t.is_empty()
        }

        fn is_stack_span_empty(&self) -> bool {
            self.stack_span.is_empty()
        }

        fn intercept_token(&mut self, token: TokenId, text: &str, span: &PosSpan) -> TokenId {
            self.listener.intercept_token(token, text, span)
        }
    }

    impl<T: TestListener> Wrapper<T> {
        pub fn new(listener: T, verbose: bool) -> Self {
            Wrapper { verbose, listener, stack: Vec::new(), max_stack: 0, stack_t: Vec::new(), stack_span: Vec::new() }
        }

        pub fn get_listener(&self) -> &T {
            &self.listener
        }

        pub fn get_listener_mut(&mut self) -> &mut T {
            &mut self.listener
        }

        pub fn give_listener(self) -> T {
            self.listener
        }

        pub fn set_verbose(&mut self, verbose: bool) {
            self.verbose = verbose;
        }

        fn exit_a(&mut self) {
            let p = self.stack.pop().unwrap().get_p();
            let s = self.stack.pop().unwrap().get_s();
            let ctx = CtxA::V1 { s, p };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_a(ctx, spans);
            self.stack.push(EnumSynValue::A(val));
        }

        fn exit_s(&mut self) {
            let xs = self.stack.pop().unwrap().get_xs();
            let ns = self.stack.pop().unwrap().get_ns();
            let vs = self.stack.pop().unwrap().get_vs();
            let ctx = CtxS::V1 { vs, ns, xs };
            let spans = self.stack_span.drain(self.stack_span.len() - 3 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_s(ctx, spans);
            self.stack.push(EnumSynValue::S(val));
        }

        fn exit_p(&mut self) {
            let xp = self.stack.pop().unwrap().get_xp();
            let np = self.stack.pop().unwrap().get_np();
            let vp = self.stack.pop().unwrap().get_vp();
            let ctx = CtxP::V1 { vp, np, xp };
            let spans = self.stack_span.drain(self.stack_span.len() - 3 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_p(ctx, spans);
            self.stack.push(EnumSynValue::P(val));
        }

        fn exit_vs(&mut self) {
            let star = self.stack.pop().unwrap().get_ivs();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVs::V1 { a, star };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_vs(ctx, spans);
            self.stack.push(EnumSynValue::Vs(val));
        }

        fn init_ivs(&mut self) {
            let val = self.listener.init_ivs();
            self.stack.push(EnumSynValue::Ivs(val));
        }

        fn exit_ivs(&mut self) {
            let b = self.stack_t.pop().unwrap();
            let ctx = CtxIvs::V1 { b };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let Some(EnumSynValue::Ivs(acc)) = self.stack.last_mut() else { panic!() };
            self.listener.exit_ivs(acc, ctx, spans);
        }

        fn exitloop_ivs(&mut self) {
            let EnumSynValue::Ivs(acc) = self.stack.last_mut().unwrap() else { panic!() };
            self.listener.exitloop_ivs(acc);
        }

        fn exit_ns(&mut self) {
            let ctx = CtxNs::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_ns(ctx, spans);
            self.stack.push(EnumSynValue::Ns(val));
        }

        fn exit_ins(&mut self) {
            let ctx = CtxIns::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_ins(ctx, spans);
        }

        fn exit_xs(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXs::V1 { a };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_xs(ctx, spans);
            self.stack.push(EnumSynValue::Xs(val));
        }

        fn exit_ixs(&mut self) {
            let ctx = CtxIxs::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_ixs(ctx, spans);
        }

        fn exit_vp(&mut self) {
            let plus = self.stack.pop().unwrap().get_ivp();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVp::V1 { a, plus };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_vp(ctx, spans);
            self.stack.push(EnumSynValue::Vp(val));
        }

        fn init_ivp(&mut self) {
            let val = self.listener.init_ivp();
            self.stack.push(EnumSynValue::Ivp(val));
        }

        fn exit_ivp(&mut self, alt_id: AltId) {
            let last_iteration = alt_id == 20;
            let b = self.stack_t.pop().unwrap();
            let ctx = CtxIvp::V1 { b, last_iteration };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let Some(EnumSynValue::Ivp(acc)) = self.stack.last_mut() else { panic!() };
            self.listener.exit_ivp(acc, ctx, spans);
        }

        fn exit_np(&mut self) {
            let ctx = CtxNp::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_np(ctx, spans);
            self.stack.push(EnumSynValue::Np(val));
        }

        fn exit_inp(&mut self, alt_id: AltId) {
            let last_iteration = alt_id == 22;
            let ctx = CtxInp::V1 { last_iteration };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_inp(ctx, spans);
        }

        fn exit_xp(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXp::V1 { a };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_xp(ctx, spans);
            self.stack.push(EnumSynValue::Xp(val));
        }

        fn exit_ixp(&mut self, alt_id: AltId) {
            let last_iteration = alt_id == 24;
            let ctx = CtxIxp::V1 { last_iteration };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_ixp(ctx, spans);
        }

        fn exit_x(&mut self) {
            let ctx = CtxX::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 1 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_x(ctx, spans);
        }
    }

    // [wrapper source for rule 982 #1, start a]
    // ------------------------------------------------------------
}

// ================================================================================

pub(crate) mod rules_982_2 {
    /// User-defined type for `a`
    #[derive(Debug, PartialEq)] pub struct SynA();

    /// User-defined type for `s`
    #[derive(Debug, PartialEq)] pub struct SynS();

    /// User-defined type for `p`
    #[derive(Debug, PartialEq)] pub struct SynP();

    /// User-defined type for `vs`
    #[derive(Debug, PartialEq)] pub struct SynVs();

    /// User-defined type for `<L> B` iteration in `vs -> A ( ►► <L> B ◄◄ )*`
    #[derive(Debug, PartialEq)] pub struct SynIvs();

    /// User-defined type for `ns`
    #[derive(Debug, PartialEq)] pub struct SynNs();

    /// User-defined type for `xs`
    #[derive(Debug, PartialEq)] pub struct SynXs();

    /// User-defined type for `vp`
    #[derive(Debug, PartialEq)] pub struct SynVp();

    /// User-defined type for `<L> B` iteration in `vp -> A ( ►► <L> B ◄◄ )+`
    #[derive(Debug, PartialEq)] pub struct SynIvp();

    /// User-defined type for `np`
    #[derive(Debug, PartialEq)] pub struct SynNp();

    /// User-defined type for `xp`
    #[derive(Debug, PartialEq)] pub struct SynXp();
    // ------------------------------------------------------------
    // [wrapper source for rule 982 #2, start a]

    use lexigram_lib::{AltId, TokenId, VarId, lexer::PosSpan, log::{LogMsg, Logger}, parser::{Call, ListenerWrapper, Terminate}};

    #[derive(Debug)]
    pub enum CtxA {
        /// `a -> s p`
        V1 { s: SynS, p: SynP },
    }
    #[derive(Debug)]
    pub enum CtxS {
        /// `s -> vs ns xs`
        V1 { vs: SynVs, ns: SynNs, xs: SynXs },
    }
    #[derive(Debug)]
    pub enum CtxP {
        /// `p -> vp np xp`
        V1 { vp: SynVp, np: SynNp, xp: SynXp },
    }
    #[derive(Debug)]
    pub enum CtxVs {
        /// `vs -> A (<L> B)*`
        V1 { a: String, star: SynIvs },
    }
    #[derive(Debug)]
    pub enum CtxIvs {
        /// `<L> B` iteration in `vs -> A ( ►► <L> B ◄◄ )*`
        V1 { b: String },
    }
    #[derive(Debug)]
    pub enum CtxNs {
        /// `ns -> "A" (<L> "B")*`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxIns {
        /// `<L> "B"` iteration in `ns -> "A" ( ►► <L> "B" ◄◄ )*`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxXs {
        /// `xs -> A (<L> x)*`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxIxs {
        /// `<L> x` iteration in `xs -> A ( ►► <L> x ◄◄ )*`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxVp {
        /// `vp -> A (<L> B)+`
        V1 { a: String, plus: SynIvp },
    }
    #[derive(Debug)]
    pub enum CtxIvp {
        /// `<L> B` iteration in `vp -> A ( ►► <L> B ◄◄ )+`
        V1 { b: String, last_iteration: bool },
    }
    #[derive(Debug)]
    pub enum CtxNp {
        /// `np -> "A" (<L> "B")+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxInp {
        /// `<L> "B"` iteration in `np -> "A" ( ►► <L> "B" ◄◄ )+`
        V1 { last_iteration: bool },
    }
    #[derive(Debug)]
    pub enum CtxXp {
        /// `xp -> A (<L> x)+`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxIxp {
        /// `<L> x` iteration in `xp -> A ( ►► <L> x ◄◄ )+`
        V1 { last_iteration: bool },
    }
    #[derive(Debug)]
    pub enum CtxX {
        /// `x -> "X"`
        V1,
    }

    #[derive(Debug)]
    enum EnumSynValue { A(SynA), S(SynS), P(SynP), Vs(SynVs), Ivs(SynIvs), Ns(SynNs), Xs(SynXs), Vp(SynVp), Ivp(SynIvp), Np(SynNp), Xp(SynXp) }

    impl EnumSynValue {
        fn get_a(self) -> SynA {
            if let EnumSynValue::A(val) = self { val } else { panic!() }
        }
        fn get_s(self) -> SynS {
            if let EnumSynValue::S(val) = self { val } else { panic!() }
        }
        fn get_p(self) -> SynP {
            if let EnumSynValue::P(val) = self { val } else { panic!() }
        }
        fn get_vs(self) -> SynVs {
            if let EnumSynValue::Vs(val) = self { val } else { panic!() }
        }
        fn get_ivs(self) -> SynIvs {
            if let EnumSynValue::Ivs(val) = self { val } else { panic!() }
        }
        fn get_ns(self) -> SynNs {
            if let EnumSynValue::Ns(val) = self { val } else { panic!() }
        }
        fn get_xs(self) -> SynXs {
            if let EnumSynValue::Xs(val) = self { val } else { panic!() }
        }
        fn get_vp(self) -> SynVp {
            if let EnumSynValue::Vp(val) = self { val } else { panic!() }
        }
        fn get_ivp(self) -> SynIvp {
            if let EnumSynValue::Ivp(val) = self { val } else { panic!() }
        }
        fn get_np(self) -> SynNp {
            if let EnumSynValue::Np(val) = self { val } else { panic!() }
        }
        fn get_xp(self) -> SynXp {
            if let EnumSynValue::Xp(val) = self { val } else { panic!() }
        }
    }

    pub trait TestListener {
        /// Checks if the listener requests an abort. This happens if an error is too difficult to recover from
        /// and may corrupt the stack content. In that case, the parser immediately stops and returns `ParserError::AbortRequest`.
        fn check_abort_request(&self) -> Terminate { Terminate::None }
        fn get_log_mut(&mut self) -> &mut impl Logger;
        #[allow(unused_variables)]
        fn handle_msg(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.get_log_mut().add(msg);
        }
        #[allow(unused_variables)]
        fn intercept_token(&mut self, token: TokenId, text: &str) -> TokenId { token }
        #[allow(unused_variables)]
        fn exit(&mut self, a: SynA) {}
        #[allow(unused_variables)]
        fn abort(&mut self, terminate: Terminate) {}
        fn init_a(&mut self) {}
        fn exit_a(&mut self, ctx: CtxA) -> SynA;
        fn init_s(&mut self) {}
        fn exit_s(&mut self, ctx: CtxS) -> SynS;
        fn init_p(&mut self) {}
        fn exit_p(&mut self, ctx: CtxP) -> SynP;
        fn init_vs(&mut self) {}
        fn exit_vs(&mut self, ctx: CtxVs) -> SynVs;
        fn init_ivs(&mut self) -> SynIvs;
        fn exit_ivs(&mut self, acc: &mut SynIvs, ctx: CtxIvs);
        #[allow(unused_variables)]
        fn exitloop_ivs(&mut self, acc: &mut SynIvs) {}
        fn init_ns(&mut self) {}
        fn exit_ns(&mut self, ctx: CtxNs) -> SynNs;
        fn init_ins(&mut self) {}
        #[allow(unused_variables)]
        fn exit_ins(&mut self, ctx: CtxIns) {}
        #[allow(unused_variables)]
        fn exitloop_ins(&mut self) {}
        fn init_xs(&mut self) {}
        fn exit_xs(&mut self, ctx: CtxXs) -> SynXs;
        fn init_ixs(&mut self) {}
        #[allow(unused_variables)]
        fn exit_ixs(&mut self, ctx: CtxIxs) {}
        #[allow(unused_variables)]
        fn exitloop_ixs(&mut self) {}
        fn init_vp(&mut self) {}
        fn exit_vp(&mut self, ctx: CtxVp) -> SynVp;
        fn init_ivp(&mut self) -> SynIvp;
        fn exit_ivp(&mut self, acc: &mut SynIvp, ctx: CtxIvp);
        fn init_np(&mut self) {}
        fn exit_np(&mut self, ctx: CtxNp) -> SynNp;
        fn init_inp(&mut self) {}
        #[allow(unused_variables)]
        fn exit_inp(&mut self, ctx: CtxInp) {}
        fn init_xp(&mut self) {}
        fn exit_xp(&mut self, ctx: CtxXp) -> SynXp;
        fn init_ixp(&mut self) {}
        #[allow(unused_variables)]
        fn exit_ixp(&mut self, ctx: CtxIxp) {}
        fn init_x(&mut self) {}
        #[allow(unused_variables)]
        fn exit_x(&mut self, ctx: CtxX) {}
    }

    pub struct Wrapper<T> {
        verbose: bool,
        listener: T,
        stack: Vec<EnumSynValue>,
        max_stack: usize,
        stack_t: Vec<String>,
    }

    impl<T: TestListener> ListenerWrapper for Wrapper<T> {
        fn switch(&mut self, call: Call, nt: VarId, alt_id: AltId, t_data: Option<Vec<String>>) {
            if self.verbose {
                println!("switch: call={call:?}, nt={nt}, alt={alt_id}, t_data={t_data:?}");
            }
            if let Some(mut t_data) = t_data {
                self.stack_t.append(&mut t_data);
            }
            match call {
                Call::Enter => {
                    match nt {
                        0 => self.listener.init_a(),                // a
                        1 => self.listener.init_s(),                // s
                        2 => self.listener.init_p(),                // p
                        3 => self.listener.init_vs(),               // vs
                        4 => self.init_ivs(),                       // ivs
                        5 => self.listener.init_ns(),               // ns
                        6 => self.listener.init_ins(),              // ins
                        7 => self.listener.init_xs(),               // xs
                        8 => self.listener.init_ixs(),              // ixs
                        9 => self.listener.init_vp(),               // vp
                        10 => self.init_ivp(),                      // ivp
                        16 => {}                                    // ivp_1
                        11 => self.listener.init_np(),              // np
                        12 => self.listener.init_inp(),             // inp
                        17 => {}                                    // inp_1
                        13 => self.listener.init_xp(),              // xp
                        14 => self.listener.init_ixp(),             // ixp
                        18 => {}                                    // ixp_1
                        15 => self.listener.init_x(),               // x
                        _ => panic!("unexpected enter nonterminal id: {nt}")
                    }
                }
                Call::Loop => {}
                Call::Exit => {
                    match alt_id {
                        0 => self.exit_a(),                         // a -> s p
                        1 => self.exit_s(),                         // s -> vs ns xs
                        2 => self.exit_p(),                         // p -> vp np xp
                        3 => self.exit_vs(),                        // vs -> A ivs
                        4 => self.exit_ivs(),                       // ivs -> <L> B ivs
                        5 => self.exitloop_ivs(),                   // ivs -> <L> ε
                        6 => self.exit_ns(),                        // ns -> "A" ins
                        7 => self.exit_ins(),                       // ins -> <L> "B" ins
                        8 => self.listener.exitloop_ins(),          // ins -> <L> ε
                        9 => self.exit_xs(),                        // xs -> A ixs
                        10 => self.exit_ixs(),                      // ixs -> <L> x ixs
                        11 => self.listener.exitloop_ixs(),         // ixs -> <L> ε
                        12 => self.exit_vp(),                       // vp -> A ivp
                        19 |                                        // ivp_1 -> ivp
                        20 => self.exit_ivp(alt_id),                // ivp_1 -> ε
                     /* 13 */                                       // ivp -> <L> B ivp_1 (never called)
                        14 => self.exit_np(),                       // np -> "A" inp
                        21 |                                        // inp_1 -> inp
                        22 => self.exit_inp(alt_id),                // inp_1 -> ε
                     /* 15 */                                       // inp -> <L> "B" inp_1 (never called)
                        16 => self.exit_xp(),                       // xp -> A ixp
                        23 |                                        // ixp_1 -> ixp
                        24 => self.exit_ixp(alt_id),                // ixp_1 -> ε
                     /* 17 */                                       // ixp -> <L> x ixp_1 (never called)
                        18 => self.exit_x(),                        // x -> "X"
                        _ => panic!("unexpected exit alternative id: {alt_id}")
                    }
                }
                Call::End(terminate) => {
                    match terminate {
                        Terminate::None => {
                            let val = self.stack.pop().unwrap().get_a();
                            self.listener.exit(val);
                        }
                        Terminate::Abort | Terminate::Conclude => self.listener.abort(terminate),
                    }
                }
            }
            self.max_stack = std::cmp::max(self.max_stack, self.stack.len());
            if self.verbose {
                println!("> stack_t:   {}", self.stack_t.join(", "));
                println!("> stack:     {}", self.stack.iter().map(|it| format!("{it:?}")).collect::<Vec<_>>().join(", "));
            }
        }

        fn check_abort_request(&self) -> Terminate {
            self.listener.check_abort_request()
        }

        fn abort(&mut self) {
            self.stack.clear();
            self.stack_t.clear();
        }

        fn get_log_mut(&mut self) -> &mut impl Logger {
            self.listener.get_log_mut()
        }

        fn report(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.listener.handle_msg(span_opt, msg);
        }

        fn is_stack_empty(&self) -> bool {
            self.stack.is_empty()
        }

        fn is_stack_t_empty(&self) -> bool {
            self.stack_t.is_empty()
        }

        fn intercept_token(&mut self, token: TokenId, text: &str, _span: &PosSpan) -> TokenId {
            self.listener.intercept_token(token, text)
        }
    }

    impl<T: TestListener> Wrapper<T> {
        pub fn new(listener: T, verbose: bool) -> Self {
            Wrapper { verbose, listener, stack: Vec::new(), max_stack: 0, stack_t: Vec::new() }
        }

        pub fn get_listener(&self) -> &T {
            &self.listener
        }

        pub fn get_listener_mut(&mut self) -> &mut T {
            &mut self.listener
        }

        pub fn give_listener(self) -> T {
            self.listener
        }

        pub fn set_verbose(&mut self, verbose: bool) {
            self.verbose = verbose;
        }

        fn exit_a(&mut self) {
            let p = self.stack.pop().unwrap().get_p();
            let s = self.stack.pop().unwrap().get_s();
            let ctx = CtxA::V1 { s, p };
            let val = self.listener.exit_a(ctx);
            self.stack.push(EnumSynValue::A(val));
        }

        fn exit_s(&mut self) {
            let xs = self.stack.pop().unwrap().get_xs();
            let ns = self.stack.pop().unwrap().get_ns();
            let vs = self.stack.pop().unwrap().get_vs();
            let ctx = CtxS::V1 { vs, ns, xs };
            let val = self.listener.exit_s(ctx);
            self.stack.push(EnumSynValue::S(val));
        }

        fn exit_p(&mut self) {
            let xp = self.stack.pop().unwrap().get_xp();
            let np = self.stack.pop().unwrap().get_np();
            let vp = self.stack.pop().unwrap().get_vp();
            let ctx = CtxP::V1 { vp, np, xp };
            let val = self.listener.exit_p(ctx);
            self.stack.push(EnumSynValue::P(val));
        }

        fn exit_vs(&mut self) {
            let star = self.stack.pop().unwrap().get_ivs();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVs::V1 { a, star };
            let val = self.listener.exit_vs(ctx);
            self.stack.push(EnumSynValue::Vs(val));
        }

        fn init_ivs(&mut self) {
            let val = self.listener.init_ivs();
            self.stack.push(EnumSynValue::Ivs(val));
        }

        fn exit_ivs(&mut self) {
            let b = self.stack_t.pop().unwrap();
            let ctx = CtxIvs::V1 { b };
            let Some(EnumSynValue::Ivs(acc)) = self.stack.last_mut() else { panic!() };
            self.listener.exit_ivs(acc, ctx);
        }

        fn exitloop_ivs(&mut self) {
            let EnumSynValue::Ivs(acc) = self.stack.last_mut().unwrap() else { panic!() };
            self.listener.exitloop_ivs(acc);
        }

        fn exit_ns(&mut self) {
            let ctx = CtxNs::V1;
            let val = self.listener.exit_ns(ctx);
            self.stack.push(EnumSynValue::Ns(val));
        }

        fn exit_ins(&mut self) {
            let ctx = CtxIns::V1;
            self.listener.exit_ins(ctx);
        }

        fn exit_xs(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXs::V1 { a };
            let val = self.listener.exit_xs(ctx);
            self.stack.push(EnumSynValue::Xs(val));
        }

        fn exit_ixs(&mut self) {
            let ctx = CtxIxs::V1;
            self.listener.exit_ixs(ctx);
        }

        fn exit_vp(&mut self) {
            let plus = self.stack.pop().unwrap().get_ivp();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVp::V1 { a, plus };
            let val = self.listener.exit_vp(ctx);
            self.stack.push(EnumSynValue::Vp(val));
        }

        fn init_ivp(&mut self) {
            let val = self.listener.init_ivp();
            self.stack.push(EnumSynValue::Ivp(val));
        }

        fn exit_ivp(&mut self, alt_id: AltId) {
            let last_iteration = alt_id == 20;
            let b = self.stack_t.pop().unwrap();
            let ctx = CtxIvp::V1 { b, last_iteration };
            let Some(EnumSynValue::Ivp(acc)) = self.stack.last_mut() else { panic!() };
            self.listener.exit_ivp(acc, ctx);
        }

        fn exit_np(&mut self) {
            let ctx = CtxNp::V1;
            let val = self.listener.exit_np(ctx);
            self.stack.push(EnumSynValue::Np(val));
        }

        fn exit_inp(&mut self, alt_id: AltId) {
            let last_iteration = alt_id == 22;
            let ctx = CtxInp::V1 { last_iteration };
            self.listener.exit_inp(ctx);
        }

        fn exit_xp(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXp::V1 { a };
            let val = self.listener.exit_xp(ctx);
            self.stack.push(EnumSynValue::Xp(val));
        }

        fn exit_ixp(&mut self, alt_id: AltId) {
            let last_iteration = alt_id == 24;
            let ctx = CtxIxp::V1 { last_iteration };
            self.listener.exit_ixp(ctx);
        }

        fn exit_x(&mut self) {
            let ctx = CtxX::V1;
            self.listener.exit_x(ctx);
        }
    }

    // [wrapper source for rule 982 #2, start a]
    // ------------------------------------------------------------
}

// ================================================================================

pub(crate) mod rules_983_1 {
    /// User-defined type for `a`
    #[derive(Debug, PartialEq)] pub struct SynA();

    /// User-defined type for `s`
    #[derive(Debug, PartialEq)] pub struct SynS();

    /// User-defined type for `p`
    #[derive(Debug, PartialEq)] pub struct SynP();

    /// User-defined type for `vs`
    #[derive(Debug, PartialEq)] pub struct SynVs();

    /// User-defined type for `<L> B` iteration in `vs -> A ( ►► <L> B ◄◄  | C D)*`
    #[derive(Debug, PartialEq)] pub struct SynIvs();

    /// User-defined type for `ns`
    #[derive(Debug, PartialEq)] pub struct SynNs();

    /// User-defined type for `xs`
    #[derive(Debug, PartialEq)] pub struct SynXs();

    /// User-defined type for `vp`
    #[derive(Debug, PartialEq)] pub struct SynVp();

    /// User-defined type for `<L> B` iteration in `vp -> A ( ►► <L> B ◄◄  | C D)+`
    #[derive(Debug, PartialEq)] pub struct SynIvp();

    /// User-defined type for `np`
    #[derive(Debug, PartialEq)] pub struct SynNp();

    /// User-defined type for `xp`
    #[derive(Debug, PartialEq)] pub struct SynXp();
    // ------------------------------------------------------------
    // [wrapper source for rule 983 #1, start a]

    use lexigram_lib::{AltId, TokenId, VarId, lexer::PosSpan, log::{LogMsg, Logger}, parser::{Call, ListenerWrapper, Terminate}};

    #[derive(Debug)]
    pub enum CtxA {
        /// `a -> s p`
        V1 { s: SynS, p: SynP },
    }
    #[derive(Debug)]
    pub enum CtxS {
        /// `s -> vs ns xs`
        V1 { vs: SynVs, ns: SynNs, xs: SynXs },
    }
    #[derive(Debug)]
    pub enum CtxP {
        /// `p -> vp np xp`
        V1 { vp: SynVp, np: SynNp, xp: SynXp },
    }
    #[derive(Debug)]
    pub enum CtxVs {
        /// `vs -> A (<L> B | C D)*`
        V1 { a: String, star: SynIvs },
    }
    #[derive(Debug)]
    pub enum CtxIvs {
        /// `<L> B` iteration in `vs -> A ( ►► <L> B ◄◄  | C D)*`
        V1 { b: String },
        /// `C D` iteration in `vs -> A (<L> B |  ►► C D ◄◄ )*`
        V2 { c: String, d: String },
    }
    #[derive(Debug)]
    pub enum CtxNs {
        /// `ns -> "A" (<L> "B" | "C" "D")*`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxIns {
        /// `<L> "B"` iteration in `ns -> "A" ( ►► <L> "B" ◄◄  | "C" "D")*`
        V1,
        /// `"C" "D"` iteration in `ns -> "A" (<L> "B" |  ►► "C" "D" ◄◄ )*`
        V2,
    }
    #[derive(Debug)]
    pub enum CtxXs {
        /// `xs -> A (<L> x | y x)*`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxIxs {
        /// `<L> x` iteration in `xs -> A ( ►► <L> x ◄◄  | y x)*`
        V1,
        /// `y x` iteration in `xs -> A (<L> x |  ►► y x ◄◄ )*`
        V2,
    }
    #[derive(Debug)]
    pub enum CtxVp {
        /// `vp -> A (<L> B | C D)+`
        V1 { a: String, plus: SynIvp },
    }
    #[derive(Debug)]
    pub enum CtxIvp {
        /// `<L> B` iteration in `vp -> A ( ►► <L> B ◄◄  | C D)+`
        V1 { b: String, last_iteration: bool },
        /// `C D` iteration in `vp -> A (<L> B |  ►► C D ◄◄ )+`
        V2 { c: String, d: String, last_iteration: bool },
    }
    #[derive(Debug)]
    pub enum CtxNp {
        /// `np -> "A" (<L> "B" | "C" "D")+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxInp {
        /// `<L> "B"` iteration in `np -> "A" ( ►► <L> "B" ◄◄  | "C" "D")+`
        V1 { last_iteration: bool },
        /// `"C" "D"` iteration in `np -> "A" (<L> "B" |  ►► "C" "D" ◄◄ )+`
        V2 { last_iteration: bool },
    }
    #[derive(Debug)]
    pub enum CtxXp {
        /// `xp -> A (<L> x | y x)+`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxIxp {
        /// `<L> x` iteration in `xp -> A ( ►► <L> x ◄◄  | y x)+`
        V1 { last_iteration: bool },
        /// `y x` iteration in `xp -> A (<L> x |  ►► y x ◄◄ )+`
        V2 { last_iteration: bool },
    }
    #[derive(Debug)]
    pub enum CtxX {
        /// `x -> "X"`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxY {
        /// `y -> "Y"`
        V1,
    }

    #[derive(Debug)]
    enum EnumSynValue { A(SynA), S(SynS), P(SynP), Vs(SynVs), Ivs(SynIvs), Ns(SynNs), Xs(SynXs), Vp(SynVp), Ivp(SynIvp), Np(SynNp), Xp(SynXp) }

    impl EnumSynValue {
        fn get_a(self) -> SynA {
            if let EnumSynValue::A(val) = self { val } else { panic!() }
        }
        fn get_s(self) -> SynS {
            if let EnumSynValue::S(val) = self { val } else { panic!() }
        }
        fn get_p(self) -> SynP {
            if let EnumSynValue::P(val) = self { val } else { panic!() }
        }
        fn get_vs(self) -> SynVs {
            if let EnumSynValue::Vs(val) = self { val } else { panic!() }
        }
        fn get_ivs(self) -> SynIvs {
            if let EnumSynValue::Ivs(val) = self { val } else { panic!() }
        }
        fn get_ns(self) -> SynNs {
            if let EnumSynValue::Ns(val) = self { val } else { panic!() }
        }
        fn get_xs(self) -> SynXs {
            if let EnumSynValue::Xs(val) = self { val } else { panic!() }
        }
        fn get_vp(self) -> SynVp {
            if let EnumSynValue::Vp(val) = self { val } else { panic!() }
        }
        fn get_ivp(self) -> SynIvp {
            if let EnumSynValue::Ivp(val) = self { val } else { panic!() }
        }
        fn get_np(self) -> SynNp {
            if let EnumSynValue::Np(val) = self { val } else { panic!() }
        }
        fn get_xp(self) -> SynXp {
            if let EnumSynValue::Xp(val) = self { val } else { panic!() }
        }
    }

    pub trait TestListener {
        /// Checks if the listener requests an abort. This happens if an error is too difficult to recover from
        /// and may corrupt the stack content. In that case, the parser immediately stops and returns `ParserError::AbortRequest`.
        fn check_abort_request(&self) -> Terminate { Terminate::None }
        fn get_log_mut(&mut self) -> &mut impl Logger;
        #[allow(unused_variables)]
        fn handle_msg(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.get_log_mut().add(msg);
        }
        #[allow(unused_variables)]
        fn intercept_token(&mut self, token: TokenId, text: &str, span: &PosSpan) -> TokenId { token }
        #[allow(unused_variables)]
        fn exit(&mut self, a: SynA, span: PosSpan) {}
        #[allow(unused_variables)]
        fn abort(&mut self, terminate: Terminate) {}
        fn init_a(&mut self) {}
        fn exit_a(&mut self, ctx: CtxA, spans: Vec<PosSpan>) -> SynA;
        fn init_s(&mut self) {}
        fn exit_s(&mut self, ctx: CtxS, spans: Vec<PosSpan>) -> SynS;
        fn init_p(&mut self) {}
        fn exit_p(&mut self, ctx: CtxP, spans: Vec<PosSpan>) -> SynP;
        fn init_vs(&mut self) {}
        fn exit_vs(&mut self, ctx: CtxVs, spans: Vec<PosSpan>) -> SynVs;
        fn init_ivs(&mut self) -> SynIvs;
        fn exit_ivs(&mut self, acc: &mut SynIvs, ctx: CtxIvs, spans: Vec<PosSpan>);
        #[allow(unused_variables)]
        fn exitloop_ivs(&mut self, acc: &mut SynIvs) {}
        fn init_ns(&mut self) {}
        fn exit_ns(&mut self, ctx: CtxNs, spans: Vec<PosSpan>) -> SynNs;
        fn init_ins(&mut self) {}
        #[allow(unused_variables)]
        fn exit_ins(&mut self, ctx: CtxIns, spans: Vec<PosSpan>) {}
        #[allow(unused_variables)]
        fn exitloop_ins(&mut self) {}
        fn init_xs(&mut self) {}
        fn exit_xs(&mut self, ctx: CtxXs, spans: Vec<PosSpan>) -> SynXs;
        fn init_ixs(&mut self) {}
        #[allow(unused_variables)]
        fn exit_ixs(&mut self, ctx: CtxIxs, spans: Vec<PosSpan>) {}
        #[allow(unused_variables)]
        fn exitloop_ixs(&mut self) {}
        fn init_vp(&mut self) {}
        fn exit_vp(&mut self, ctx: CtxVp, spans: Vec<PosSpan>) -> SynVp;
        fn init_ivp(&mut self) -> SynIvp;
        fn exit_ivp(&mut self, acc: &mut SynIvp, ctx: CtxIvp, spans: Vec<PosSpan>);
        fn init_np(&mut self) {}
        fn exit_np(&mut self, ctx: CtxNp, spans: Vec<PosSpan>) -> SynNp;
        fn init_inp(&mut self) {}
        #[allow(unused_variables)]
        fn exit_inp(&mut self, ctx: CtxInp, spans: Vec<PosSpan>) {}
        fn init_xp(&mut self) {}
        fn exit_xp(&mut self, ctx: CtxXp, spans: Vec<PosSpan>) -> SynXp;
        fn init_ixp(&mut self) {}
        #[allow(unused_variables)]
        fn exit_ixp(&mut self, ctx: CtxIxp, spans: Vec<PosSpan>) {}
        fn init_x(&mut self) {}
        #[allow(unused_variables)]
        fn exit_x(&mut self, ctx: CtxX, spans: Vec<PosSpan>) {}
        fn init_y(&mut self) {}
        #[allow(unused_variables)]
        fn exit_y(&mut self, ctx: CtxY, spans: Vec<PosSpan>) {}
    }

    pub struct Wrapper<T> {
        verbose: bool,
        listener: T,
        stack: Vec<EnumSynValue>,
        max_stack: usize,
        stack_t: Vec<String>,
        stack_span: Vec<PosSpan>,
    }

    impl<T: TestListener> ListenerWrapper for Wrapper<T> {
        fn switch(&mut self, call: Call, nt: VarId, alt_id: AltId, t_data: Option<Vec<String>>) {
            if self.verbose {
                println!("switch: call={call:?}, nt={nt}, alt={alt_id}, t_data={t_data:?}");
            }
            if let Some(mut t_data) = t_data {
                self.stack_t.append(&mut t_data);
            }
            match call {
                Call::Enter => {
                    if matches!(nt, 4 | 6 | 8 | 10 | 12 | 14) {
                        self.stack_span.push(PosSpan::empty());
                    }
                    match nt {
                        0 => self.listener.init_a(),                // a
                        1 => self.listener.init_s(),                // s
                        2 => self.listener.init_p(),                // p
                        3 => self.listener.init_vs(),               // vs
                        4 => self.init_ivs(),                       // ivs
                        5 => self.listener.init_ns(),               // ns
                        6 => self.listener.init_ins(),              // ins
                        7 => self.listener.init_xs(),               // xs
                        8 => self.listener.init_ixs(),              // ixs
                        9 => self.listener.init_vp(),               // vp
                        10 => self.init_ivp(),                      // ivp
                        17 | 18 => {}                               // ivp_1, ivp_2
                        11 => self.listener.init_np(),              // np
                        12 => self.listener.init_inp(),             // inp
                        19 | 20 => {}                               // inp_1, inp_2
                        13 => self.listener.init_xp(),              // xp
                        14 => self.listener.init_ixp(),             // ixp
                        21 | 22 => {}                               // ixp_1, ixp_2
                        15 => self.listener.init_x(),               // x
                        16 => self.listener.init_y(),               // y
                        _ => panic!("unexpected enter nonterminal id: {nt}")
                    }
                }
                Call::Loop => {}
                Call::Exit => {
                    match alt_id {
                        0 => self.exit_a(),                         // a -> s p
                        1 => self.exit_s(),                         // s -> vs ns xs
                        2 => self.exit_p(),                         // p -> vp np xp
                        3 => self.exit_vs(),                        // vs -> A ivs
                        4 |                                         // ivs -> <L> B ivs
                        5 => self.exit_ivs(alt_id),                 // ivs -> <L> C D ivs
                        6 => self.exitloop_ivs(),                   // ivs -> <L> ε
                        7 => self.exit_ns(),                        // ns -> "A" ins
                        8 |                                         // ins -> <L> "B" ins
                        9 => self.exit_ins(alt_id),                 // ins -> <L> "C" "D" ins
                        10 => self.listener.exitloop_ins(),         // ins -> <L> ε
                        11 => self.exit_xs(),                       // xs -> A ixs
                        12 |                                        // ixs -> <L> x ixs
                        13 => self.exit_ixs(alt_id),                // ixs -> <L> y x ixs
                        14 => self.listener.exitloop_ixs(),         // ixs -> <L> ε
                        15 => self.exit_vp(),                       // vp -> A ivp
                        26 |                                        // ivp_1 -> ivp
                        27 |                                        // ivp_1 -> ε
                        28 |                                        // ivp_2 -> ivp
                        29 => self.exit_ivp(alt_id),                // ivp_2 -> ε
                     /* 16 */                                       // ivp -> <L> B ivp_1 (never called)
                     /* 17 */                                       // ivp -> <L> C D ivp_2 (never called)
                        18 => self.exit_np(),                       // np -> "A" inp
                        30 |                                        // inp_1 -> inp
                        31 |                                        // inp_1 -> ε
                        32 |                                        // inp_2 -> inp
                        33 => self.exit_inp(alt_id),                // inp_2 -> ε
                     /* 19 */                                       // inp -> <L> "B" inp_1 (never called)
                     /* 20 */                                       // inp -> <L> "C" "D" inp_2 (never called)
                        21 => self.exit_xp(),                       // xp -> A ixp
                        34 |                                        // ixp_1 -> ixp
                        35 |                                        // ixp_1 -> ε
                        36 |                                        // ixp_2 -> ixp
                        37 => self.exit_ixp(alt_id),                // ixp_2 -> ε
                     /* 22 */                                       // ixp -> <L> x ixp_1 (never called)
                     /* 23 */                                       // ixp -> <L> y x ixp_2 (never called)
                        24 => self.exit_x(),                        // x -> "X"
                        25 => self.exit_y(),                        // y -> "Y"
                        _ => panic!("unexpected exit alternative id: {alt_id}")
                    }
                }
                Call::End(terminate) => {
                    match terminate {
                        Terminate::None => {
                            let val = self.stack.pop().unwrap().get_a();
                            let span = self.stack_span.pop().unwrap();
                            self.listener.exit(val, span);
                        }
                        Terminate::Abort | Terminate::Conclude => self.listener.abort(terminate),
                    }
                }
            }
            self.max_stack = std::cmp::max(self.max_stack, self.stack.len());
            if self.verbose {
                println!("> stack_t:   {}", self.stack_t.join(", "));
                println!("> stack:     {}", self.stack.iter().map(|it| format!("{it:?}")).collect::<Vec<_>>().join(", "));
            }
        }

        fn check_abort_request(&self) -> Terminate {
            self.listener.check_abort_request()
        }

        fn abort(&mut self) {
            self.stack.clear();
            self.stack_span.clear();
            self.stack_t.clear();
        }

        fn get_log_mut(&mut self) -> &mut impl Logger {
            self.listener.get_log_mut()
        }

        fn report(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.listener.handle_msg(span_opt, msg);
        }

        fn push_span(&mut self, span: PosSpan) {
            self.stack_span.push(span);
        }

        fn is_stack_empty(&self) -> bool {
            self.stack.is_empty()
        }

        fn is_stack_t_empty(&self) -> bool {
            self.stack_t.is_empty()
        }

        fn is_stack_span_empty(&self) -> bool {
            self.stack_span.is_empty()
        }

        fn intercept_token(&mut self, token: TokenId, text: &str, span: &PosSpan) -> TokenId {
            self.listener.intercept_token(token, text, span)
        }
    }

    impl<T: TestListener> Wrapper<T> {
        pub fn new(listener: T, verbose: bool) -> Self {
            Wrapper { verbose, listener, stack: Vec::new(), max_stack: 0, stack_t: Vec::new(), stack_span: Vec::new() }
        }

        pub fn get_listener(&self) -> &T {
            &self.listener
        }

        pub fn get_listener_mut(&mut self) -> &mut T {
            &mut self.listener
        }

        pub fn give_listener(self) -> T {
            self.listener
        }

        pub fn set_verbose(&mut self, verbose: bool) {
            self.verbose = verbose;
        }

        fn exit_a(&mut self) {
            let p = self.stack.pop().unwrap().get_p();
            let s = self.stack.pop().unwrap().get_s();
            let ctx = CtxA::V1 { s, p };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_a(ctx, spans);
            self.stack.push(EnumSynValue::A(val));
        }

        fn exit_s(&mut self) {
            let xs = self.stack.pop().unwrap().get_xs();
            let ns = self.stack.pop().unwrap().get_ns();
            let vs = self.stack.pop().unwrap().get_vs();
            let ctx = CtxS::V1 { vs, ns, xs };
            let spans = self.stack_span.drain(self.stack_span.len() - 3 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_s(ctx, spans);
            self.stack.push(EnumSynValue::S(val));
        }

        fn exit_p(&mut self) {
            let xp = self.stack.pop().unwrap().get_xp();
            let np = self.stack.pop().unwrap().get_np();
            let vp = self.stack.pop().unwrap().get_vp();
            let ctx = CtxP::V1 { vp, np, xp };
            let spans = self.stack_span.drain(self.stack_span.len() - 3 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_p(ctx, spans);
            self.stack.push(EnumSynValue::P(val));
        }

        fn exit_vs(&mut self) {
            let star = self.stack.pop().unwrap().get_ivs();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVs::V1 { a, star };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_vs(ctx, spans);
            self.stack.push(EnumSynValue::Vs(val));
        }

        fn init_ivs(&mut self) {
            let val = self.listener.init_ivs();
            self.stack.push(EnumSynValue::Ivs(val));
        }

        fn exit_ivs(&mut self, alt_id: AltId) {
            let (n, ctx) = match alt_id {
                4 => {
                    let b = self.stack_t.pop().unwrap();
                    (2, CtxIvs::V1 { b })
                }
                5 => {
                    let d = self.stack_t.pop().unwrap();
                    let c = self.stack_t.pop().unwrap();
                    (3, CtxIvs::V2 { c, d })
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_ivs")
            };
            let spans = self.stack_span.drain(self.stack_span.len() - n ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let Some(EnumSynValue::Ivs(acc)) = self.stack.last_mut() else { panic!() };
            self.listener.exit_ivs(acc, ctx, spans);
        }

        fn exitloop_ivs(&mut self) {
            let EnumSynValue::Ivs(acc) = self.stack.last_mut().unwrap() else { panic!() };
            self.listener.exitloop_ivs(acc);
        }

        fn exit_ns(&mut self) {
            let ctx = CtxNs::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_ns(ctx, spans);
            self.stack.push(EnumSynValue::Ns(val));
        }

        fn exit_ins(&mut self, alt_id: AltId) {
            let (n, ctx) = match alt_id {
                8 => {
                    (2, CtxIns::V1)
                }
                9 => {
                    (3, CtxIns::V2)
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_ins")
            };
            let spans = self.stack_span.drain(self.stack_span.len() - n ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_ins(ctx, spans);
        }

        fn exit_xs(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXs::V1 { a };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_xs(ctx, spans);
            self.stack.push(EnumSynValue::Xs(val));
        }

        fn exit_ixs(&mut self, alt_id: AltId) {
            let (n, ctx) = match alt_id {
                12 => {
                    (2, CtxIxs::V1)
                }
                13 => {
                    (3, CtxIxs::V2)
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_ixs")
            };
            let spans = self.stack_span.drain(self.stack_span.len() - n ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_ixs(ctx, spans);
        }

        fn exit_vp(&mut self) {
            let plus = self.stack.pop().unwrap().get_ivp();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVp::V1 { a, plus };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_vp(ctx, spans);
            self.stack.push(EnumSynValue::Vp(val));
        }

        fn init_ivp(&mut self) {
            let val = self.listener.init_ivp();
            self.stack.push(EnumSynValue::Ivp(val));
        }

        fn exit_ivp(&mut self, alt_id: AltId) {
            let (n, ctx) = match alt_id {
                26 | 27 => {
                    let last_iteration = alt_id == 27;
                    let b = self.stack_t.pop().unwrap();
                    (2, CtxIvp::V1 { b, last_iteration })
                }
                28 | 29 => {
                    let last_iteration = alt_id == 29;
                    let d = self.stack_t.pop().unwrap();
                    let c = self.stack_t.pop().unwrap();
                    (3, CtxIvp::V2 { c, d, last_iteration })
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_ivp")
            };
            let spans = self.stack_span.drain(self.stack_span.len() - n ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let Some(EnumSynValue::Ivp(acc)) = self.stack.last_mut() else { panic!() };
            self.listener.exit_ivp(acc, ctx, spans);
        }

        fn exit_np(&mut self) {
            let ctx = CtxNp::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_np(ctx, spans);
            self.stack.push(EnumSynValue::Np(val));
        }

        fn exit_inp(&mut self, alt_id: AltId) {
            let (n, ctx) = match alt_id {
                30 | 31 => {
                    let last_iteration = alt_id == 31;
                    (2, CtxInp::V1 { last_iteration })
                }
                32 | 33 => {
                    let last_iteration = alt_id == 33;
                    (3, CtxInp::V2 { last_iteration })
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_inp")
            };
            let spans = self.stack_span.drain(self.stack_span.len() - n ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_inp(ctx, spans);
        }

        fn exit_xp(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXp::V1 { a };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_xp(ctx, spans);
            self.stack.push(EnumSynValue::Xp(val));
        }

        fn exit_ixp(&mut self, alt_id: AltId) {
            let (n, ctx) = match alt_id {
                34 | 35 => {
                    let last_iteration = alt_id == 35;
                    (2, CtxIxp::V1 { last_iteration })
                }
                36 | 37 => {
                    let last_iteration = alt_id == 37;
                    (3, CtxIxp::V2 { last_iteration })
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_ixp")
            };
            let spans = self.stack_span.drain(self.stack_span.len() - n ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_ixp(ctx, spans);
        }

        fn exit_x(&mut self) {
            let ctx = CtxX::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 1 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_x(ctx, spans);
        }

        fn exit_y(&mut self) {
            let ctx = CtxY::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 1 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_y(ctx, spans);
        }
    }

    // [wrapper source for rule 983 #1, start a]
    // ------------------------------------------------------------
}

// ================================================================================

pub(crate) mod rules_983_2 {
    /// User-defined type for `a`
    #[derive(Debug, PartialEq)] pub struct SynA();

    /// User-defined type for `s`
    #[derive(Debug, PartialEq)] pub struct SynS();

    /// User-defined type for `p`
    #[derive(Debug, PartialEq)] pub struct SynP();

    /// User-defined type for `vs`
    #[derive(Debug, PartialEq)] pub struct SynVs();

    /// User-defined type for `<L> B` iteration in `vs -> A ( ►► <L> B ◄◄  | C D)*`
    #[derive(Debug, PartialEq)] pub struct SynIvs();

    /// User-defined type for `ns`
    #[derive(Debug, PartialEq)] pub struct SynNs();

    /// User-defined type for `xs`
    #[derive(Debug, PartialEq)] pub struct SynXs();

    /// User-defined type for `vp`
    #[derive(Debug, PartialEq)] pub struct SynVp();

    /// User-defined type for `<L> B` iteration in `vp -> A ( ►► <L> B ◄◄  | C D)+`
    #[derive(Debug, PartialEq)] pub struct SynIvp();

    /// User-defined type for `np`
    #[derive(Debug, PartialEq)] pub struct SynNp();

    /// User-defined type for `xp`
    #[derive(Debug, PartialEq)] pub struct SynXp();
    // ------------------------------------------------------------
    // [wrapper source for rule 983 #2, start a]

    use lexigram_lib::{AltId, TokenId, VarId, lexer::PosSpan, log::{LogMsg, Logger}, parser::{Call, ListenerWrapper, Terminate}};

    #[derive(Debug)]
    pub enum CtxA {
        /// `a -> s p`
        V1 { s: SynS, p: SynP },
    }
    #[derive(Debug)]
    pub enum CtxS {
        /// `s -> vs ns xs`
        V1 { vs: SynVs, ns: SynNs, xs: SynXs },
    }
    #[derive(Debug)]
    pub enum CtxP {
        /// `p -> vp np xp`
        V1 { vp: SynVp, np: SynNp, xp: SynXp },
    }
    #[derive(Debug)]
    pub enum CtxVs {
        /// `vs -> A (<L> B | C D)*`
        V1 { a: String, star: SynIvs },
    }
    #[derive(Debug)]
    pub enum CtxIvs {
        /// `<L> B` iteration in `vs -> A ( ►► <L> B ◄◄  | C D)*`
        V1 { b: String },
        /// `C D` iteration in `vs -> A (<L> B |  ►► C D ◄◄ )*`
        V2 { c: String, d: String },
    }
    #[derive(Debug)]
    pub enum CtxNs {
        /// `ns -> "A" (<L> "B" | "C" "D")*`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxIns {
        /// `<L> "B"` iteration in `ns -> "A" ( ►► <L> "B" ◄◄  | "C" "D")*`
        V1,
        /// `"C" "D"` iteration in `ns -> "A" (<L> "B" |  ►► "C" "D" ◄◄ )*`
        V2,
    }
    #[derive(Debug)]
    pub enum CtxXs {
        /// `xs -> A (<L> x | y x)*`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxIxs {
        /// `<L> x` iteration in `xs -> A ( ►► <L> x ◄◄  | y x)*`
        V1,
        /// `y x` iteration in `xs -> A (<L> x |  ►► y x ◄◄ )*`
        V2,
    }
    #[derive(Debug)]
    pub enum CtxVp {
        /// `vp -> A (<L> B | C D)+`
        V1 { a: String, plus: SynIvp },
    }
    #[derive(Debug)]
    pub enum CtxIvp {
        /// `<L> B` iteration in `vp -> A ( ►► <L> B ◄◄  | C D)+`
        V1 { b: String, last_iteration: bool },
        /// `C D` iteration in `vp -> A (<L> B |  ►► C D ◄◄ )+`
        V2 { c: String, d: String, last_iteration: bool },
    }
    #[derive(Debug)]
    pub enum CtxNp {
        /// `np -> "A" (<L> "B" | "C" "D")+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxInp {
        /// `<L> "B"` iteration in `np -> "A" ( ►► <L> "B" ◄◄  | "C" "D")+`
        V1 { last_iteration: bool },
        /// `"C" "D"` iteration in `np -> "A" (<L> "B" |  ►► "C" "D" ◄◄ )+`
        V2 { last_iteration: bool },
    }
    #[derive(Debug)]
    pub enum CtxXp {
        /// `xp -> A (<L> x | y x)+`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxIxp {
        /// `<L> x` iteration in `xp -> A ( ►► <L> x ◄◄  | y x)+`
        V1 { last_iteration: bool },
        /// `y x` iteration in `xp -> A (<L> x |  ►► y x ◄◄ )+`
        V2 { last_iteration: bool },
    }
    #[derive(Debug)]
    pub enum CtxX {
        /// `x -> "X"`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxY {
        /// `y -> "Y"`
        V1,
    }

    #[derive(Debug)]
    enum EnumSynValue { A(SynA), S(SynS), P(SynP), Vs(SynVs), Ivs(SynIvs), Ns(SynNs), Xs(SynXs), Vp(SynVp), Ivp(SynIvp), Np(SynNp), Xp(SynXp) }

    impl EnumSynValue {
        fn get_a(self) -> SynA {
            if let EnumSynValue::A(val) = self { val } else { panic!() }
        }
        fn get_s(self) -> SynS {
            if let EnumSynValue::S(val) = self { val } else { panic!() }
        }
        fn get_p(self) -> SynP {
            if let EnumSynValue::P(val) = self { val } else { panic!() }
        }
        fn get_vs(self) -> SynVs {
            if let EnumSynValue::Vs(val) = self { val } else { panic!() }
        }
        fn get_ivs(self) -> SynIvs {
            if let EnumSynValue::Ivs(val) = self { val } else { panic!() }
        }
        fn get_ns(self) -> SynNs {
            if let EnumSynValue::Ns(val) = self { val } else { panic!() }
        }
        fn get_xs(self) -> SynXs {
            if let EnumSynValue::Xs(val) = self { val } else { panic!() }
        }
        fn get_vp(self) -> SynVp {
            if let EnumSynValue::Vp(val) = self { val } else { panic!() }
        }
        fn get_ivp(self) -> SynIvp {
            if let EnumSynValue::Ivp(val) = self { val } else { panic!() }
        }
        fn get_np(self) -> SynNp {
            if let EnumSynValue::Np(val) = self { val } else { panic!() }
        }
        fn get_xp(self) -> SynXp {
            if let EnumSynValue::Xp(val) = self { val } else { panic!() }
        }
    }

    pub trait TestListener {
        /// Checks if the listener requests an abort. This happens if an error is too difficult to recover from
        /// and may corrupt the stack content. In that case, the parser immediately stops and returns `ParserError::AbortRequest`.
        fn check_abort_request(&self) -> Terminate { Terminate::None }
        fn get_log_mut(&mut self) -> &mut impl Logger;
        #[allow(unused_variables)]
        fn handle_msg(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.get_log_mut().add(msg);
        }
        #[allow(unused_variables)]
        fn intercept_token(&mut self, token: TokenId, text: &str) -> TokenId { token }
        #[allow(unused_variables)]
        fn exit(&mut self, a: SynA) {}
        #[allow(unused_variables)]
        fn abort(&mut self, terminate: Terminate) {}
        fn init_a(&mut self) {}
        fn exit_a(&mut self, ctx: CtxA) -> SynA;
        fn init_s(&mut self) {}
        fn exit_s(&mut self, ctx: CtxS) -> SynS;
        fn init_p(&mut self) {}
        fn exit_p(&mut self, ctx: CtxP) -> SynP;
        fn init_vs(&mut self) {}
        fn exit_vs(&mut self, ctx: CtxVs) -> SynVs;
        fn init_ivs(&mut self) -> SynIvs;
        fn exit_ivs(&mut self, acc: &mut SynIvs, ctx: CtxIvs);
        #[allow(unused_variables)]
        fn exitloop_ivs(&mut self, acc: &mut SynIvs) {}
        fn init_ns(&mut self) {}
        fn exit_ns(&mut self, ctx: CtxNs) -> SynNs;
        fn init_ins(&mut self) {}
        #[allow(unused_variables)]
        fn exit_ins(&mut self, ctx: CtxIns) {}
        #[allow(unused_variables)]
        fn exitloop_ins(&mut self) {}
        fn init_xs(&mut self) {}
        fn exit_xs(&mut self, ctx: CtxXs) -> SynXs;
        fn init_ixs(&mut self) {}
        #[allow(unused_variables)]
        fn exit_ixs(&mut self, ctx: CtxIxs) {}
        #[allow(unused_variables)]
        fn exitloop_ixs(&mut self) {}
        fn init_vp(&mut self) {}
        fn exit_vp(&mut self, ctx: CtxVp) -> SynVp;
        fn init_ivp(&mut self) -> SynIvp;
        fn exit_ivp(&mut self, acc: &mut SynIvp, ctx: CtxIvp);
        fn init_np(&mut self) {}
        fn exit_np(&mut self, ctx: CtxNp) -> SynNp;
        fn init_inp(&mut self) {}
        #[allow(unused_variables)]
        fn exit_inp(&mut self, ctx: CtxInp) {}
        fn init_xp(&mut self) {}
        fn exit_xp(&mut self, ctx: CtxXp) -> SynXp;
        fn init_ixp(&mut self) {}
        #[allow(unused_variables)]
        fn exit_ixp(&mut self, ctx: CtxIxp) {}
        fn init_x(&mut self) {}
        #[allow(unused_variables)]
        fn exit_x(&mut self, ctx: CtxX) {}
        fn init_y(&mut self) {}
        #[allow(unused_variables)]
        fn exit_y(&mut self, ctx: CtxY) {}
    }

    pub struct Wrapper<T> {
        verbose: bool,
        listener: T,
        stack: Vec<EnumSynValue>,
        max_stack: usize,
        stack_t: Vec<String>,
    }

    impl<T: TestListener> ListenerWrapper for Wrapper<T> {
        fn switch(&mut self, call: Call, nt: VarId, alt_id: AltId, t_data: Option<Vec<String>>) {
            if self.verbose {
                println!("switch: call={call:?}, nt={nt}, alt={alt_id}, t_data={t_data:?}");
            }
            if let Some(mut t_data) = t_data {
                self.stack_t.append(&mut t_data);
            }
            match call {
                Call::Enter => {
                    match nt {
                        0 => self.listener.init_a(),                // a
                        1 => self.listener.init_s(),                // s
                        2 => self.listener.init_p(),                // p
                        3 => self.listener.init_vs(),               // vs
                        4 => self.init_ivs(),                       // ivs
                        5 => self.listener.init_ns(),               // ns
                        6 => self.listener.init_ins(),              // ins
                        7 => self.listener.init_xs(),               // xs
                        8 => self.listener.init_ixs(),              // ixs
                        9 => self.listener.init_vp(),               // vp
                        10 => self.init_ivp(),                      // ivp
                        17 | 18 => {}                               // ivp_1, ivp_2
                        11 => self.listener.init_np(),              // np
                        12 => self.listener.init_inp(),             // inp
                        19 | 20 => {}                               // inp_1, inp_2
                        13 => self.listener.init_xp(),              // xp
                        14 => self.listener.init_ixp(),             // ixp
                        21 | 22 => {}                               // ixp_1, ixp_2
                        15 => self.listener.init_x(),               // x
                        16 => self.listener.init_y(),               // y
                        _ => panic!("unexpected enter nonterminal id: {nt}")
                    }
                }
                Call::Loop => {}
                Call::Exit => {
                    match alt_id {
                        0 => self.exit_a(),                         // a -> s p
                        1 => self.exit_s(),                         // s -> vs ns xs
                        2 => self.exit_p(),                         // p -> vp np xp
                        3 => self.exit_vs(),                        // vs -> A ivs
                        4 |                                         // ivs -> <L> B ivs
                        5 => self.exit_ivs(alt_id),                 // ivs -> <L> C D ivs
                        6 => self.exitloop_ivs(),                   // ivs -> <L> ε
                        7 => self.exit_ns(),                        // ns -> "A" ins
                        8 |                                         // ins -> <L> "B" ins
                        9 => self.exit_ins(alt_id),                 // ins -> <L> "C" "D" ins
                        10 => self.listener.exitloop_ins(),         // ins -> <L> ε
                        11 => self.exit_xs(),                       // xs -> A ixs
                        12 |                                        // ixs -> <L> x ixs
                        13 => self.exit_ixs(alt_id),                // ixs -> <L> y x ixs
                        14 => self.listener.exitloop_ixs(),         // ixs -> <L> ε
                        15 => self.exit_vp(),                       // vp -> A ivp
                        26 |                                        // ivp_1 -> ivp
                        27 |                                        // ivp_1 -> ε
                        28 |                                        // ivp_2 -> ivp
                        29 => self.exit_ivp(alt_id),                // ivp_2 -> ε
                     /* 16 */                                       // ivp -> <L> B ivp_1 (never called)
                     /* 17 */                                       // ivp -> <L> C D ivp_2 (never called)
                        18 => self.exit_np(),                       // np -> "A" inp
                        30 |                                        // inp_1 -> inp
                        31 |                                        // inp_1 -> ε
                        32 |                                        // inp_2 -> inp
                        33 => self.exit_inp(alt_id),                // inp_2 -> ε
                     /* 19 */                                       // inp -> <L> "B" inp_1 (never called)
                     /* 20 */                                       // inp -> <L> "C" "D" inp_2 (never called)
                        21 => self.exit_xp(),                       // xp -> A ixp
                        34 |                                        // ixp_1 -> ixp
                        35 |                                        // ixp_1 -> ε
                        36 |                                        // ixp_2 -> ixp
                        37 => self.exit_ixp(alt_id),                // ixp_2 -> ε
                     /* 22 */                                       // ixp -> <L> x ixp_1 (never called)
                     /* 23 */                                       // ixp -> <L> y x ixp_2 (never called)
                        24 => self.exit_x(),                        // x -> "X"
                        25 => self.exit_y(),                        // y -> "Y"
                        _ => panic!("unexpected exit alternative id: {alt_id}")
                    }
                }
                Call::End(terminate) => {
                    match terminate {
                        Terminate::None => {
                            let val = self.stack.pop().unwrap().get_a();
                            self.listener.exit(val);
                        }
                        Terminate::Abort | Terminate::Conclude => self.listener.abort(terminate),
                    }
                }
            }
            self.max_stack = std::cmp::max(self.max_stack, self.stack.len());
            if self.verbose {
                println!("> stack_t:   {}", self.stack_t.join(", "));
                println!("> stack:     {}", self.stack.iter().map(|it| format!("{it:?}")).collect::<Vec<_>>().join(", "));
            }
        }

        fn check_abort_request(&self) -> Terminate {
            self.listener.check_abort_request()
        }

        fn abort(&mut self) {
            self.stack.clear();
            self.stack_t.clear();
        }

        fn get_log_mut(&mut self) -> &mut impl Logger {
            self.listener.get_log_mut()
        }

        fn report(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.listener.handle_msg(span_opt, msg);
        }

        fn is_stack_empty(&self) -> bool {
            self.stack.is_empty()
        }

        fn is_stack_t_empty(&self) -> bool {
            self.stack_t.is_empty()
        }

        fn intercept_token(&mut self, token: TokenId, text: &str, _span: &PosSpan) -> TokenId {
            self.listener.intercept_token(token, text)
        }
    }

    impl<T: TestListener> Wrapper<T> {
        pub fn new(listener: T, verbose: bool) -> Self {
            Wrapper { verbose, listener, stack: Vec::new(), max_stack: 0, stack_t: Vec::new() }
        }

        pub fn get_listener(&self) -> &T {
            &self.listener
        }

        pub fn get_listener_mut(&mut self) -> &mut T {
            &mut self.listener
        }

        pub fn give_listener(self) -> T {
            self.listener
        }

        pub fn set_verbose(&mut self, verbose: bool) {
            self.verbose = verbose;
        }

        fn exit_a(&mut self) {
            let p = self.stack.pop().unwrap().get_p();
            let s = self.stack.pop().unwrap().get_s();
            let ctx = CtxA::V1 { s, p };
            let val = self.listener.exit_a(ctx);
            self.stack.push(EnumSynValue::A(val));
        }

        fn exit_s(&mut self) {
            let xs = self.stack.pop().unwrap().get_xs();
            let ns = self.stack.pop().unwrap().get_ns();
            let vs = self.stack.pop().unwrap().get_vs();
            let ctx = CtxS::V1 { vs, ns, xs };
            let val = self.listener.exit_s(ctx);
            self.stack.push(EnumSynValue::S(val));
        }

        fn exit_p(&mut self) {
            let xp = self.stack.pop().unwrap().get_xp();
            let np = self.stack.pop().unwrap().get_np();
            let vp = self.stack.pop().unwrap().get_vp();
            let ctx = CtxP::V1 { vp, np, xp };
            let val = self.listener.exit_p(ctx);
            self.stack.push(EnumSynValue::P(val));
        }

        fn exit_vs(&mut self) {
            let star = self.stack.pop().unwrap().get_ivs();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVs::V1 { a, star };
            let val = self.listener.exit_vs(ctx);
            self.stack.push(EnumSynValue::Vs(val));
        }

        fn init_ivs(&mut self) {
            let val = self.listener.init_ivs();
            self.stack.push(EnumSynValue::Ivs(val));
        }

        fn exit_ivs(&mut self, alt_id: AltId) {
            let ctx = match alt_id {
                4 => {
                    let b = self.stack_t.pop().unwrap();
                    CtxIvs::V1 { b }
                }
                5 => {
                    let d = self.stack_t.pop().unwrap();
                    let c = self.stack_t.pop().unwrap();
                    CtxIvs::V2 { c, d }
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_ivs")
            };
            let Some(EnumSynValue::Ivs(acc)) = self.stack.last_mut() else { panic!() };
            self.listener.exit_ivs(acc, ctx);
        }

        fn exitloop_ivs(&mut self) {
            let EnumSynValue::Ivs(acc) = self.stack.last_mut().unwrap() else { panic!() };
            self.listener.exitloop_ivs(acc);
        }

        fn exit_ns(&mut self) {
            let ctx = CtxNs::V1;
            let val = self.listener.exit_ns(ctx);
            self.stack.push(EnumSynValue::Ns(val));
        }

        fn exit_ins(&mut self, alt_id: AltId) {
            let ctx = match alt_id {
                8 => {
                    CtxIns::V1
                }
                9 => {
                    CtxIns::V2
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_ins")
            };
            self.listener.exit_ins(ctx);
        }

        fn exit_xs(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXs::V1 { a };
            let val = self.listener.exit_xs(ctx);
            self.stack.push(EnumSynValue::Xs(val));
        }

        fn exit_ixs(&mut self, alt_id: AltId) {
            let ctx = match alt_id {
                12 => {
                    CtxIxs::V1
                }
                13 => {
                    CtxIxs::V2
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_ixs")
            };
            self.listener.exit_ixs(ctx);
        }

        fn exit_vp(&mut self) {
            let plus = self.stack.pop().unwrap().get_ivp();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVp::V1 { a, plus };
            let val = self.listener.exit_vp(ctx);
            self.stack.push(EnumSynValue::Vp(val));
        }

        fn init_ivp(&mut self) {
            let val = self.listener.init_ivp();
            self.stack.push(EnumSynValue::Ivp(val));
        }

        fn exit_ivp(&mut self, alt_id: AltId) {
            let ctx = match alt_id {
                26 | 27 => {
                    let last_iteration = alt_id == 27;
                    let b = self.stack_t.pop().unwrap();
                    CtxIvp::V1 { b, last_iteration }
                }
                28 | 29 => {
                    let last_iteration = alt_id == 29;
                    let d = self.stack_t.pop().unwrap();
                    let c = self.stack_t.pop().unwrap();
                    CtxIvp::V2 { c, d, last_iteration }
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_ivp")
            };
            let Some(EnumSynValue::Ivp(acc)) = self.stack.last_mut() else { panic!() };
            self.listener.exit_ivp(acc, ctx);
        }

        fn exit_np(&mut self) {
            let ctx = CtxNp::V1;
            let val = self.listener.exit_np(ctx);
            self.stack.push(EnumSynValue::Np(val));
        }

        fn exit_inp(&mut self, alt_id: AltId) {
            let ctx = match alt_id {
                30 | 31 => {
                    let last_iteration = alt_id == 31;
                    CtxInp::V1 { last_iteration }
                }
                32 | 33 => {
                    let last_iteration = alt_id == 33;
                    CtxInp::V2 { last_iteration }
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_inp")
            };
            self.listener.exit_inp(ctx);
        }

        fn exit_xp(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXp::V1 { a };
            let val = self.listener.exit_xp(ctx);
            self.stack.push(EnumSynValue::Xp(val));
        }

        fn exit_ixp(&mut self, alt_id: AltId) {
            let ctx = match alt_id {
                34 | 35 => {
                    let last_iteration = alt_id == 35;
                    CtxIxp::V1 { last_iteration }
                }
                36 | 37 => {
                    let last_iteration = alt_id == 37;
                    CtxIxp::V2 { last_iteration }
                }
                _ => panic!("unexpected alt id {alt_id} in method exit_ixp")
            };
            self.listener.exit_ixp(ctx);
        }

        fn exit_x(&mut self) {
            let ctx = CtxX::V1;
            self.listener.exit_x(ctx);
        }

        fn exit_y(&mut self) {
            let ctx = CtxY::V1;
            self.listener.exit_y(ctx);
        }
    }

    // [wrapper source for rule 983 #2, start a]
    // ------------------------------------------------------------
}

// ================================================================================

pub(crate) mod rules_984_1 {
    /// User-defined type for `a`
    #[derive(Debug, PartialEq)] pub struct SynA();

    /// User-defined type for `vp`
    #[derive(Debug, PartialEq)] pub struct SynVp();

    /// User-defined type for `np`
    #[derive(Debug, PartialEq)] pub struct SynNp();

    /// User-defined type for `xp`
    #[derive(Debug, PartialEq)] pub struct SynXp();
    // ------------------------------------------------------------
    // [wrapper source for rule 984 #1, start a]

    use lexigram_lib::{AltId, TokenId, VarId, lexer::PosSpan, log::{LogMsg, Logger}, parser::{Call, ListenerWrapper, Terminate}};

    #[derive(Debug)]
    pub enum CtxA {
        /// `a -> vp np xp`
        V1 { vp: SynVp, np: SynNp, xp: SynXp },
    }
    #[derive(Debug)]
    pub enum CtxVp {
        /// `vp -> A (B C / ",")+`
        V1 { a: String, plus: SynVp1 },
    }
    #[derive(Debug)]
    pub enum CtxNp {
        /// `np -> "A" ("B" "C" / ",")+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxXp {
        /// `xp -> A (x y / ",")+`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxX {
        /// `x -> "X"`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxY {
        /// `y -> "Y"`
        V1,
    }

    /// Computed `(B C / ",")+` array in `vp -> A  ►► (B C / ",")+ ◄◄ `
    #[derive(Debug, PartialEq)]
    pub struct SynVp1(pub Vec<SynVp1Item>);
    /// `B C / ","` item in `vp -> A ( ►► B C / "," ◄◄ )+`
    #[derive(Debug, PartialEq)]
    pub struct SynVp1Item { pub b: String, pub c: String }

    #[derive(Debug)]
    enum EnumSynValue { A(SynA), Vp(SynVp), Np(SynNp), Xp(SynXp), Vp1(SynVp1) }

    impl EnumSynValue {
        fn get_a(self) -> SynA {
            if let EnumSynValue::A(val) = self { val } else { panic!() }
        }
        fn get_vp(self) -> SynVp {
            if let EnumSynValue::Vp(val) = self { val } else { panic!() }
        }
        fn get_np(self) -> SynNp {
            if let EnumSynValue::Np(val) = self { val } else { panic!() }
        }
        fn get_xp(self) -> SynXp {
            if let EnumSynValue::Xp(val) = self { val } else { panic!() }
        }
        fn get_vp1(self) -> SynVp1 {
            if let EnumSynValue::Vp1(val) = self { val } else { panic!() }
        }
    }

    pub trait TestListener {
        /// Checks if the listener requests an abort. This happens if an error is too difficult to recover from
        /// and may corrupt the stack content. In that case, the parser immediately stops and returns `ParserError::AbortRequest`.
        fn check_abort_request(&self) -> Terminate { Terminate::None }
        fn get_log_mut(&mut self) -> &mut impl Logger;
        #[allow(unused_variables)]
        fn handle_msg(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.get_log_mut().add(msg);
        }
        #[allow(unused_variables)]
        fn intercept_token(&mut self, token: TokenId, text: &str, span: &PosSpan) -> TokenId { token }
        #[allow(unused_variables)]
        fn exit(&mut self, a: SynA, span: PosSpan) {}
        #[allow(unused_variables)]
        fn abort(&mut self, terminate: Terminate) {}
        fn init_a(&mut self) {}
        fn exit_a(&mut self, ctx: CtxA, spans: Vec<PosSpan>) -> SynA;
        fn init_vp(&mut self) {}
        fn exit_vp(&mut self, ctx: CtxVp, spans: Vec<PosSpan>) -> SynVp;
        fn init_np(&mut self) {}
        fn exit_np(&mut self, ctx: CtxNp, spans: Vec<PosSpan>) -> SynNp;
        fn init_xp(&mut self) {}
        fn exit_xp(&mut self, ctx: CtxXp, spans: Vec<PosSpan>) -> SynXp;
        fn init_x(&mut self) {}
        #[allow(unused_variables)]
        fn exit_x(&mut self, ctx: CtxX, spans: Vec<PosSpan>) {}
        fn init_y(&mut self) {}
        #[allow(unused_variables)]
        fn exit_y(&mut self, ctx: CtxY, spans: Vec<PosSpan>) {}
    }

    pub struct Wrapper<T> {
        verbose: bool,
        listener: T,
        stack: Vec<EnumSynValue>,
        max_stack: usize,
        stack_t: Vec<String>,
        stack_span: Vec<PosSpan>,
    }

    impl<T: TestListener> ListenerWrapper for Wrapper<T> {
        fn switch(&mut self, call: Call, nt: VarId, alt_id: AltId, t_data: Option<Vec<String>>) {
            if self.verbose {
                println!("switch: call={call:?}, nt={nt}, alt={alt_id}, t_data={t_data:?}");
            }
            if let Some(mut t_data) = t_data {
                self.stack_t.append(&mut t_data);
            }
            match call {
                Call::Enter => {
                    match nt {
                        0 => self.listener.init_a(),                // a
                        1 => self.listener.init_vp(),               // vp
                        6 => self.init_vp1(),                       // vp_1
                        2 => self.listener.init_np(),               // np
                        7 => self.init_np1(),                       // np_1
                        3 => self.listener.init_xp(),               // xp
                        8 => self.init_xp1(),                       // xp_1
                        4 => self.listener.init_x(),                // x
                        5 => self.listener.init_y(),                // y
                        _ => panic!("unexpected enter nonterminal id: {nt}")
                    }
                }
                Call::Loop => {}
                Call::Exit => {
                    match alt_id {
                        0 => self.exit_a(),                         // a -> vp np xp
                        1 => self.exit_vp(),                        // vp -> A B C vp_1
                        6 => self.exit_vp1(),                       // vp_1 -> "," B C vp_1
                        7 => {}                                     // vp_1 -> ε
                        2 => self.exit_np(),                        // np -> "A" "B" "C" np_1
                        8 => self.exit_np1(),                       // np_1 -> "," "B" "C" np_1
                        9 => {}                                     // np_1 -> ε
                        3 => self.exit_xp(),                        // xp -> A x y xp_1
                        10 => self.exit_xp1(),                      // xp_1 -> "," x y xp_1
                        11 => {}                                    // xp_1 -> ε
                        4 => self.exit_x(),                         // x -> "X"
                        5 => self.exit_y(),                         // y -> "Y"
                        _ => panic!("unexpected exit alternative id: {alt_id}")
                    }
                }
                Call::End(terminate) => {
                    match terminate {
                        Terminate::None => {
                            let val = self.stack.pop().unwrap().get_a();
                            let span = self.stack_span.pop().unwrap();
                            self.listener.exit(val, span);
                        }
                        Terminate::Abort | Terminate::Conclude => self.listener.abort(terminate),
                    }
                }
            }
            self.max_stack = std::cmp::max(self.max_stack, self.stack.len());
            if self.verbose {
                println!("> stack_t:   {}", self.stack_t.join(", "));
                println!("> stack:     {}", self.stack.iter().map(|it| format!("{it:?}")).collect::<Vec<_>>().join(", "));
            }
        }

        fn check_abort_request(&self) -> Terminate {
            self.listener.check_abort_request()
        }

        fn abort(&mut self) {
            self.stack.clear();
            self.stack_span.clear();
            self.stack_t.clear();
        }

        fn get_log_mut(&mut self) -> &mut impl Logger {
            self.listener.get_log_mut()
        }

        fn report(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.listener.handle_msg(span_opt, msg);
        }

        fn push_span(&mut self, span: PosSpan) {
            self.stack_span.push(span);
        }

        fn is_stack_empty(&self) -> bool {
            self.stack.is_empty()
        }

        fn is_stack_t_empty(&self) -> bool {
            self.stack_t.is_empty()
        }

        fn is_stack_span_empty(&self) -> bool {
            self.stack_span.is_empty()
        }

        fn intercept_token(&mut self, token: TokenId, text: &str, span: &PosSpan) -> TokenId {
            self.listener.intercept_token(token, text, span)
        }
    }

    impl<T: TestListener> Wrapper<T> {
        pub fn new(listener: T, verbose: bool) -> Self {
            Wrapper { verbose, listener, stack: Vec::new(), max_stack: 0, stack_t: Vec::new(), stack_span: Vec::new() }
        }

        pub fn get_listener(&self) -> &T {
            &self.listener
        }

        pub fn get_listener_mut(&mut self) -> &mut T {
            &mut self.listener
        }

        pub fn give_listener(self) -> T {
            self.listener
        }

        pub fn set_verbose(&mut self, verbose: bool) {
            self.verbose = verbose;
        }

        fn exit_a(&mut self) {
            let xp = self.stack.pop().unwrap().get_xp();
            let np = self.stack.pop().unwrap().get_np();
            let vp = self.stack.pop().unwrap().get_vp();
            let ctx = CtxA::V1 { vp, np, xp };
            let spans = self.stack_span.drain(self.stack_span.len() - 3 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_a(ctx, spans);
            self.stack.push(EnumSynValue::A(val));
        }

        fn exit_vp(&mut self) {
            let plus = self.stack.pop().unwrap().get_vp1();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVp::V1 { a, plus };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_vp(ctx, spans);
            self.stack.push(EnumSynValue::Vp(val));
        }

        fn init_vp1(&mut self) {
            let c = self.stack_t.pop().unwrap();
            let b = self.stack_t.pop().unwrap();
            let val = SynVp1Item { b, c };
            self.stack.push(EnumSynValue::Vp1(SynVp1(vec![val])));
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_vp1(&mut self) {
            let c = self.stack_t.pop().unwrap();
            let b = self.stack_t.pop().unwrap();
            let val = SynVp1Item { b, c };
            let Some(EnumSynValue::Vp1(SynVp1(star_acc))) = self.stack.last_mut() else {
                panic!("expected SynVp1 item on wrapper stack");
            };
            star_acc.push(val);
            let spans = self.stack_span.drain(self.stack_span.len() - 4 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_np(&mut self) {
            let ctx = CtxNp::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_np(ctx, spans);
            self.stack.push(EnumSynValue::Np(val));
        }

        fn init_np1(&mut self) {
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_np1(&mut self) {
            let spans = self.stack_span.drain(self.stack_span.len() - 4 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_xp(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXp::V1 { a };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_xp(ctx, spans);
            self.stack.push(EnumSynValue::Xp(val));
        }

        fn init_xp1(&mut self) {
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_xp1(&mut self) {
            let spans = self.stack_span.drain(self.stack_span.len() - 4 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
        }

        fn exit_x(&mut self) {
            let ctx = CtxX::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 1 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_x(ctx, spans);
        }

        fn exit_y(&mut self) {
            let ctx = CtxY::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 1 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_y(ctx, spans);
        }
    }

    // [wrapper source for rule 984 #1, start a]
    // ------------------------------------------------------------
}

// ================================================================================

pub(crate) mod rules_984_2 {
    /// User-defined type for `a`
    #[derive(Debug, PartialEq)] pub struct SynA();

    /// User-defined type for `vp`
    #[derive(Debug, PartialEq)] pub struct SynVp();

    /// User-defined type for `np`
    #[derive(Debug, PartialEq)] pub struct SynNp();

    /// User-defined type for `xp`
    #[derive(Debug, PartialEq)] pub struct SynXp();
    // ------------------------------------------------------------
    // [wrapper source for rule 984 #2, start a]

    use lexigram_lib::{AltId, TokenId, VarId, lexer::PosSpan, log::{LogMsg, Logger}, parser::{Call, ListenerWrapper, Terminate}};

    #[derive(Debug)]
    pub enum CtxA {
        /// `a -> vp np xp`
        V1 { vp: SynVp, np: SynNp, xp: SynXp },
    }
    #[derive(Debug)]
    pub enum CtxVp {
        /// `vp -> A (B C / ",")+`
        V1 { a: String, plus: SynVp1 },
    }
    #[derive(Debug)]
    pub enum CtxNp {
        /// `np -> "A" ("B" "C" / ",")+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxXp {
        /// `xp -> A (x y / ",")+`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum CtxX {
        /// `x -> "X"`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxY {
        /// `y -> "Y"`
        V1,
    }

    /// Computed `(B C / ",")+` array in `vp -> A  ►► (B C / ",")+ ◄◄ `
    #[derive(Debug, PartialEq)]
    pub struct SynVp1(pub Vec<SynVp1Item>);
    /// `B C / ","` item in `vp -> A ( ►► B C / "," ◄◄ )+`
    #[derive(Debug, PartialEq)]
    pub struct SynVp1Item { pub b: String, pub c: String }

    #[derive(Debug)]
    enum EnumSynValue { A(SynA), Vp(SynVp), Np(SynNp), Xp(SynXp), Vp1(SynVp1) }

    impl EnumSynValue {
        fn get_a(self) -> SynA {
            if let EnumSynValue::A(val) = self { val } else { panic!() }
        }
        fn get_vp(self) -> SynVp {
            if let EnumSynValue::Vp(val) = self { val } else { panic!() }
        }
        fn get_np(self) -> SynNp {
            if let EnumSynValue::Np(val) = self { val } else { panic!() }
        }
        fn get_xp(self) -> SynXp {
            if let EnumSynValue::Xp(val) = self { val } else { panic!() }
        }
        fn get_vp1(self) -> SynVp1 {
            if let EnumSynValue::Vp1(val) = self { val } else { panic!() }
        }
    }

    pub trait TestListener {
        /// Checks if the listener requests an abort. This happens if an error is too difficult to recover from
        /// and may corrupt the stack content. In that case, the parser immediately stops and returns `ParserError::AbortRequest`.
        fn check_abort_request(&self) -> Terminate { Terminate::None }
        fn get_log_mut(&mut self) -> &mut impl Logger;
        #[allow(unused_variables)]
        fn handle_msg(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.get_log_mut().add(msg);
        }
        #[allow(unused_variables)]
        fn intercept_token(&mut self, token: TokenId, text: &str) -> TokenId { token }
        #[allow(unused_variables)]
        fn exit(&mut self, a: SynA) {}
        #[allow(unused_variables)]
        fn abort(&mut self, terminate: Terminate) {}
        fn init_a(&mut self) {}
        fn exit_a(&mut self, ctx: CtxA) -> SynA;
        fn init_vp(&mut self) {}
        fn exit_vp(&mut self, ctx: CtxVp) -> SynVp;
        fn init_np(&mut self) {}
        fn exit_np(&mut self, ctx: CtxNp) -> SynNp;
        fn init_xp(&mut self) {}
        fn exit_xp(&mut self, ctx: CtxXp) -> SynXp;
        fn init_x(&mut self) {}
        #[allow(unused_variables)]
        fn exit_x(&mut self, ctx: CtxX) {}
        fn init_y(&mut self) {}
        #[allow(unused_variables)]
        fn exit_y(&mut self, ctx: CtxY) {}
    }

    pub struct Wrapper<T> {
        verbose: bool,
        listener: T,
        stack: Vec<EnumSynValue>,
        max_stack: usize,
        stack_t: Vec<String>,
    }

    impl<T: TestListener> ListenerWrapper for Wrapper<T> {
        fn switch(&mut self, call: Call, nt: VarId, alt_id: AltId, t_data: Option<Vec<String>>) {
            if self.verbose {
                println!("switch: call={call:?}, nt={nt}, alt={alt_id}, t_data={t_data:?}");
            }
            if let Some(mut t_data) = t_data {
                self.stack_t.append(&mut t_data);
            }
            match call {
                Call::Enter => {
                    match nt {
                        0 => self.listener.init_a(),                // a
                        1 => self.listener.init_vp(),               // vp
                        6 => self.init_vp1(),                       // vp_1
                        2 => self.listener.init_np(),               // np
                        7 => {}                                     // np_1
                        3 => self.listener.init_xp(),               // xp
                        8 => {}                                     // xp_1
                        4 => self.listener.init_x(),                // x
                        5 => self.listener.init_y(),                // y
                        _ => panic!("unexpected enter nonterminal id: {nt}")
                    }
                }
                Call::Loop => {}
                Call::Exit => {
                    match alt_id {
                        0 => self.exit_a(),                         // a -> vp np xp
                        1 => self.exit_vp(),                        // vp -> A B C vp_1
                        6 => self.exit_vp1(),                       // vp_1 -> "," B C vp_1
                        7 => {}                                     // vp_1 -> ε
                        2 => self.exit_np(),                        // np -> "A" "B" "C" np_1
                        8 |                                         // np_1 -> "," "B" "C" np_1
                        9 => {}                                     // np_1 -> ε
                        3 => self.exit_xp(),                        // xp -> A x y xp_1
                        10 |                                        // xp_1 -> "," x y xp_1
                        11 => {}                                    // xp_1 -> ε
                        4 => self.exit_x(),                         // x -> "X"
                        5 => self.exit_y(),                         // y -> "Y"
                        _ => panic!("unexpected exit alternative id: {alt_id}")
                    }
                }
                Call::End(terminate) => {
                    match terminate {
                        Terminate::None => {
                            let val = self.stack.pop().unwrap().get_a();
                            self.listener.exit(val);
                        }
                        Terminate::Abort | Terminate::Conclude => self.listener.abort(terminate),
                    }
                }
            }
            self.max_stack = std::cmp::max(self.max_stack, self.stack.len());
            if self.verbose {
                println!("> stack_t:   {}", self.stack_t.join(", "));
                println!("> stack:     {}", self.stack.iter().map(|it| format!("{it:?}")).collect::<Vec<_>>().join(", "));
            }
        }

        fn check_abort_request(&self) -> Terminate {
            self.listener.check_abort_request()
        }

        fn abort(&mut self) {
            self.stack.clear();
            self.stack_t.clear();
        }

        fn get_log_mut(&mut self) -> &mut impl Logger {
            self.listener.get_log_mut()
        }

        fn report(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.listener.handle_msg(span_opt, msg);
        }

        fn is_stack_empty(&self) -> bool {
            self.stack.is_empty()
        }

        fn is_stack_t_empty(&self) -> bool {
            self.stack_t.is_empty()
        }

        fn intercept_token(&mut self, token: TokenId, text: &str, _span: &PosSpan) -> TokenId {
            self.listener.intercept_token(token, text)
        }
    }

    impl<T: TestListener> Wrapper<T> {
        pub fn new(listener: T, verbose: bool) -> Self {
            Wrapper { verbose, listener, stack: Vec::new(), max_stack: 0, stack_t: Vec::new() }
        }

        pub fn get_listener(&self) -> &T {
            &self.listener
        }

        pub fn get_listener_mut(&mut self) -> &mut T {
            &mut self.listener
        }

        pub fn give_listener(self) -> T {
            self.listener
        }

        pub fn set_verbose(&mut self, verbose: bool) {
            self.verbose = verbose;
        }

        fn exit_a(&mut self) {
            let xp = self.stack.pop().unwrap().get_xp();
            let np = self.stack.pop().unwrap().get_np();
            let vp = self.stack.pop().unwrap().get_vp();
            let ctx = CtxA::V1 { vp, np, xp };
            let val = self.listener.exit_a(ctx);
            self.stack.push(EnumSynValue::A(val));
        }

        fn exit_vp(&mut self) {
            let plus = self.stack.pop().unwrap().get_vp1();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVp::V1 { a, plus };
            let val = self.listener.exit_vp(ctx);
            self.stack.push(EnumSynValue::Vp(val));
        }

        fn init_vp1(&mut self) {
            let c = self.stack_t.pop().unwrap();
            let b = self.stack_t.pop().unwrap();
            let val = SynVp1Item { b, c };
            self.stack.push(EnumSynValue::Vp1(SynVp1(vec![val])));
        }

        fn exit_vp1(&mut self) {
            let c = self.stack_t.pop().unwrap();
            let b = self.stack_t.pop().unwrap();
            let val = SynVp1Item { b, c };
            let Some(EnumSynValue::Vp1(SynVp1(star_acc))) = self.stack.last_mut() else {
                panic!("expected SynVp1 item on wrapper stack");
            };
            star_acc.push(val);
        }

        fn exit_np(&mut self) {
            let ctx = CtxNp::V1;
            let val = self.listener.exit_np(ctx);
            self.stack.push(EnumSynValue::Np(val));
        }

        fn exit_xp(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXp::V1 { a };
            let val = self.listener.exit_xp(ctx);
            self.stack.push(EnumSynValue::Xp(val));
        }

        fn exit_x(&mut self) {
            let ctx = CtxX::V1;
            self.listener.exit_x(ctx);
        }

        fn exit_y(&mut self) {
            let ctx = CtxY::V1;
            self.listener.exit_y(ctx);
        }
    }

    // [wrapper source for rule 984 #2, start a]
    // ------------------------------------------------------------
}

// ================================================================================

pub(crate) mod rules_985_1 {
    /// User-defined type for `a`
    #[derive(Debug, PartialEq)] pub struct SynA();

    /// User-defined type for `vp`
    #[derive(Debug, PartialEq)] pub struct SynVp();

    /// User-defined type for `<L> B / ","` iteration in `vp -> A ( ►► <L> B / "," ◄◄ )+`
    #[derive(Debug, PartialEq)] pub struct SynIvp();

    /// User-defined type for `np`
    #[derive(Debug, PartialEq)] pub struct SynNp();

    /// User-defined type for `xp`
    #[derive(Debug, PartialEq)] pub struct SynXp();
    // ------------------------------------------------------------
    // [wrapper source for rule 985 #1, start a]

    use lexigram_lib::{AltId, TokenId, VarId, lexer::PosSpan, log::{LogMsg, Logger}, parser::{Call, ListenerWrapper, Terminate}};

    #[derive(Debug)]
    pub enum CtxA {
        /// `a -> vp np xp`
        V1 { vp: SynVp, np: SynNp, xp: SynXp },
    }
    #[derive(Debug)]
    pub enum CtxVp {
        /// `vp -> A (<L> B C / ",")+`
        V1 { a: String, plus: SynIvp },
    }
    #[derive(Debug)]
    pub enum InitCtxIvp {
        /// first `<L> B C / ","` iteration in `vp -> A ( ►► <L> B C / "," ◄◄ )+`
        V1 { b: String, c: String },
    }
    #[derive(Debug)]
    pub enum CtxIvp {
        /// `<L> B C / ","` iteration in `vp -> A ( ►► <L> B C / "," ◄◄ )+`
        V1 { b: String, c: String },
    }
    #[derive(Debug)]
    pub enum CtxNp {
        /// `np -> "A" (<L> "B" "C" / ",")+`
        V1,
    }
    #[derive(Debug)]
    pub enum InitCtxInp {
        /// first `<L> "B" "C" / ","` iteration in `np -> "A" ( ►► <L> "B" "C" / "," ◄◄ )+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxInp {
        /// `<L> "B" "C" / ","` iteration in `np -> "A" ( ►► <L> "B" "C" / "," ◄◄ )+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxXp {
        /// `xp -> A (<L> x y / ",")+`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum InitCtxIxp {
        /// first `<L> x y / ","` iteration in `xp -> A ( ►► <L> x y / "," ◄◄ )+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxIxp {
        /// `<L> x y / ","` iteration in `xp -> A ( ►► <L> x y / "," ◄◄ )+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxX {
        /// `x -> "X"`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxY {
        /// `y -> "Y"`
        V1,
    }

    #[derive(Debug)]
    enum EnumSynValue { A(SynA), Vp(SynVp), Ivp(SynIvp), Np(SynNp), Xp(SynXp) }

    impl EnumSynValue {
        fn get_a(self) -> SynA {
            if let EnumSynValue::A(val) = self { val } else { panic!() }
        }
        fn get_vp(self) -> SynVp {
            if let EnumSynValue::Vp(val) = self { val } else { panic!() }
        }
        fn get_ivp(self) -> SynIvp {
            if let EnumSynValue::Ivp(val) = self { val } else { panic!() }
        }
        fn get_np(self) -> SynNp {
            if let EnumSynValue::Np(val) = self { val } else { panic!() }
        }
        fn get_xp(self) -> SynXp {
            if let EnumSynValue::Xp(val) = self { val } else { panic!() }
        }
    }

    pub trait TestListener {
        /// Checks if the listener requests an abort. This happens if an error is too difficult to recover from
        /// and may corrupt the stack content. In that case, the parser immediately stops and returns `ParserError::AbortRequest`.
        fn check_abort_request(&self) -> Terminate { Terminate::None }
        fn get_log_mut(&mut self) -> &mut impl Logger;
        #[allow(unused_variables)]
        fn handle_msg(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.get_log_mut().add(msg);
        }
        #[allow(unused_variables)]
        fn intercept_token(&mut self, token: TokenId, text: &str, span: &PosSpan) -> TokenId { token }
        #[allow(unused_variables)]
        fn exit(&mut self, a: SynA, span: PosSpan) {}
        #[allow(unused_variables)]
        fn abort(&mut self, terminate: Terminate) {}
        fn init_a(&mut self) {}
        fn exit_a(&mut self, ctx: CtxA, spans: Vec<PosSpan>) -> SynA;
        fn init_vp(&mut self) {}
        fn exit_vp(&mut self, ctx: CtxVp, spans: Vec<PosSpan>) -> SynVp;
        fn init_ivp(&mut self, ctx: InitCtxIvp, spans: Vec<PosSpan>) -> SynIvp;
        fn exit_ivp(&mut self, acc: &mut SynIvp, ctx: CtxIvp, spans: Vec<PosSpan>);
        #[allow(unused_variables)]
        fn exitloop_ivp(&mut self, acc: &mut SynIvp) {}
        fn init_np(&mut self) {}
        fn exit_np(&mut self, ctx: CtxNp, spans: Vec<PosSpan>) -> SynNp;
        #[allow(unused_variables)]
        fn init_inp(&mut self, ctx: InitCtxInp, spans: Vec<PosSpan>) {}
        #[allow(unused_variables)]
        fn exit_inp(&mut self, ctx: CtxInp, spans: Vec<PosSpan>) {}
        #[allow(unused_variables)]
        fn exitloop_inp(&mut self) {}
        fn init_xp(&mut self) {}
        fn exit_xp(&mut self, ctx: CtxXp, spans: Vec<PosSpan>) -> SynXp;
        #[allow(unused_variables)]
        fn init_ixp(&mut self, ctx: InitCtxIxp, spans: Vec<PosSpan>) {}
        #[allow(unused_variables)]
        fn exit_ixp(&mut self, ctx: CtxIxp, spans: Vec<PosSpan>) {}
        #[allow(unused_variables)]
        fn exitloop_ixp(&mut self) {}
        fn init_x(&mut self) {}
        #[allow(unused_variables)]
        fn exit_x(&mut self, ctx: CtxX, spans: Vec<PosSpan>) {}
        fn init_y(&mut self) {}
        #[allow(unused_variables)]
        fn exit_y(&mut self, ctx: CtxY, spans: Vec<PosSpan>) {}
    }

    pub struct Wrapper<T> {
        verbose: bool,
        listener: T,
        stack: Vec<EnumSynValue>,
        max_stack: usize,
        stack_t: Vec<String>,
        stack_span: Vec<PosSpan>,
    }

    impl<T: TestListener> ListenerWrapper for Wrapper<T> {
        fn switch(&mut self, call: Call, nt: VarId, alt_id: AltId, t_data: Option<Vec<String>>) {
            if self.verbose {
                println!("switch: call={call:?}, nt={nt}, alt={alt_id}, t_data={t_data:?}");
            }
            if let Some(mut t_data) = t_data {
                self.stack_t.append(&mut t_data);
            }
            match call {
                Call::Enter => {
                    match nt {
                        0 => self.listener.init_a(),                // a
                        1 => self.listener.init_vp(),               // vp
                        2 => self.init_ivp(),                       // ivp
                        3 => self.listener.init_np(),               // np
                        4 => self.init_inp(),                       // inp
                        5 => self.listener.init_xp(),               // xp
                        6 => self.init_ixp(),                       // ixp
                        7 => self.listener.init_x(),                // x
                        8 => self.listener.init_y(),                // y
                        _ => panic!("unexpected enter nonterminal id: {nt}")
                    }
                }
                Call::Loop => {}
                Call::Exit => {
                    match alt_id {
                        0 => self.exit_a(),                         // a -> vp np xp
                        1 => self.exit_vp(),                        // vp -> A B C ivp
                        2 => self.exit_ivp(),                       // ivp -> <L> "," B C ivp
                        3 => self.exitloop_ivp(),                   // ivp -> <L> ε
                        4 => self.exit_np(),                        // np -> "A" "B" "C" inp
                        5 => self.exit_inp(),                       // inp -> <L> "," "B" "C" inp
                        6 => self.listener.exitloop_inp(),          // inp -> <L> ε
                        7 => self.exit_xp(),                        // xp -> A x y ixp
                        8 => self.exit_ixp(),                       // ixp -> <L> "," x y ixp
                        9 => self.listener.exitloop_ixp(),          // ixp -> <L> ε
                        10 => self.exit_x(),                        // x -> "X"
                        11 => self.exit_y(),                        // y -> "Y"
                        _ => panic!("unexpected exit alternative id: {alt_id}")
                    }
                }
                Call::End(terminate) => {
                    match terminate {
                        Terminate::None => {
                            let val = self.stack.pop().unwrap().get_a();
                            let span = self.stack_span.pop().unwrap();
                            self.listener.exit(val, span);
                        }
                        Terminate::Abort | Terminate::Conclude => self.listener.abort(terminate),
                    }
                }
            }
            self.max_stack = std::cmp::max(self.max_stack, self.stack.len());
            if self.verbose {
                println!("> stack_t:   {}", self.stack_t.join(", "));
                println!("> stack:     {}", self.stack.iter().map(|it| format!("{it:?}")).collect::<Vec<_>>().join(", "));
            }
        }

        fn check_abort_request(&self) -> Terminate {
            self.listener.check_abort_request()
        }

        fn abort(&mut self) {
            self.stack.clear();
            self.stack_span.clear();
            self.stack_t.clear();
        }

        fn get_log_mut(&mut self) -> &mut impl Logger {
            self.listener.get_log_mut()
        }

        fn report(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.listener.handle_msg(span_opt, msg);
        }

        fn push_span(&mut self, span: PosSpan) {
            self.stack_span.push(span);
        }

        fn is_stack_empty(&self) -> bool {
            self.stack.is_empty()
        }

        fn is_stack_t_empty(&self) -> bool {
            self.stack_t.is_empty()
        }

        fn is_stack_span_empty(&self) -> bool {
            self.stack_span.is_empty()
        }

        fn intercept_token(&mut self, token: TokenId, text: &str, span: &PosSpan) -> TokenId {
            self.listener.intercept_token(token, text, span)
        }
    }

    impl<T: TestListener> Wrapper<T> {
        pub fn new(listener: T, verbose: bool) -> Self {
            Wrapper { verbose, listener, stack: Vec::new(), max_stack: 0, stack_t: Vec::new(), stack_span: Vec::new() }
        }

        pub fn get_listener(&self) -> &T {
            &self.listener
        }

        pub fn get_listener_mut(&mut self) -> &mut T {
            &mut self.listener
        }

        pub fn give_listener(self) -> T {
            self.listener
        }

        pub fn set_verbose(&mut self, verbose: bool) {
            self.verbose = verbose;
        }

        fn exit_a(&mut self) {
            let xp = self.stack.pop().unwrap().get_xp();
            let np = self.stack.pop().unwrap().get_np();
            let vp = self.stack.pop().unwrap().get_vp();
            let ctx = CtxA::V1 { vp, np, xp };
            let spans = self.stack_span.drain(self.stack_span.len() - 3 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_a(ctx, spans);
            self.stack.push(EnumSynValue::A(val));
        }

        fn exit_vp(&mut self) {
            let plus = self.stack.pop().unwrap().get_ivp();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVp::V1 { a, plus };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_vp(ctx, spans);
            self.stack.push(EnumSynValue::Vp(val));
        }

        fn init_ivp(&mut self) {
            let c = self.stack_t.pop().unwrap();
            let b = self.stack_t.pop().unwrap();
            let ctx = InitCtxIvp::V1 { b, c };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.init_ivp(ctx, spans);
            self.stack.push(EnumSynValue::Ivp(val));
        }

        fn exit_ivp(&mut self) {
            let c = self.stack_t.pop().unwrap();
            let b = self.stack_t.pop().unwrap();
            let ctx = CtxIvp::V1 { b, c };
            let spans = self.stack_span.drain(self.stack_span.len() - 4 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let Some(EnumSynValue::Ivp(acc)) = self.stack.last_mut() else { panic!() };
            self.listener.exit_ivp(acc, ctx, spans);
        }

        fn exitloop_ivp(&mut self) {
            let EnumSynValue::Ivp(acc) = self.stack.last_mut().unwrap() else { panic!() };
            self.listener.exitloop_ivp(acc);
        }

        fn exit_np(&mut self) {
            let ctx = CtxNp::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_np(ctx, spans);
            self.stack.push(EnumSynValue::Np(val));
        }

        fn init_inp(&mut self) {
            let ctx = InitCtxInp::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.init_inp(ctx, spans);
        }

        fn exit_inp(&mut self) {
            let ctx = CtxInp::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 4 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_inp(ctx, spans);
        }

        fn exit_xp(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXp::V1 { a };
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            let val = self.listener.exit_xp(ctx, spans);
            self.stack.push(EnumSynValue::Xp(val));
        }

        fn init_ixp(&mut self) {
            let ctx = InitCtxIxp::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 2 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.init_ixp(ctx, spans);
        }

        fn exit_ixp(&mut self) {
            let ctx = CtxIxp::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 4 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_ixp(ctx, spans);
        }

        fn exit_x(&mut self) {
            let ctx = CtxX::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 1 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_x(ctx, spans);
        }

        fn exit_y(&mut self) {
            let ctx = CtxY::V1;
            let spans = self.stack_span.drain(self.stack_span.len() - 1 ..).collect::<Vec<_>>();
            self.stack_span.push(spans.iter().fold(PosSpan::empty(), |acc, sp| acc + sp));
            self.listener.exit_y(ctx, spans);
        }
    }

    // [wrapper source for rule 985 #1, start a]
    // ------------------------------------------------------------
}

// ================================================================================

pub(crate) mod rules_985_2 {
    /// User-defined type for `a`
    #[derive(Debug, PartialEq)] pub struct SynA();

    /// User-defined type for `vp`
    #[derive(Debug, PartialEq)] pub struct SynVp();

    /// User-defined type for `<L> B / ","` iteration in `vp -> A ( ►► <L> B / "," ◄◄ )+`
    #[derive(Debug, PartialEq)] pub struct SynIvp();

    /// User-defined type for `np`
    #[derive(Debug, PartialEq)] pub struct SynNp();

    /// User-defined type for `xp`
    #[derive(Debug, PartialEq)] pub struct SynXp();
    // ------------------------------------------------------------
    // [wrapper source for rule 985 #2, start a]

    use lexigram_lib::{AltId, TokenId, VarId, lexer::PosSpan, log::{LogMsg, Logger}, parser::{Call, ListenerWrapper, Terminate}};

    #[derive(Debug)]
    pub enum CtxA {
        /// `a -> vp np xp`
        V1 { vp: SynVp, np: SynNp, xp: SynXp },
    }
    #[derive(Debug)]
    pub enum CtxVp {
        /// `vp -> A (<L> B C / ",")+`
        V1 { a: String, plus: SynIvp },
    }
    #[derive(Debug)]
    pub enum InitCtxIvp {
        /// first `<L> B C / ","` iteration in `vp -> A ( ►► <L> B C / "," ◄◄ )+`
        V1 { b: String, c: String },
    }
    #[derive(Debug)]
    pub enum CtxIvp {
        /// `<L> B C / ","` iteration in `vp -> A ( ►► <L> B C / "," ◄◄ )+`
        V1 { b: String, c: String },
    }
    #[derive(Debug)]
    pub enum CtxNp {
        /// `np -> "A" (<L> "B" "C" / ",")+`
        V1,
    }
    #[derive(Debug)]
    pub enum InitCtxInp {
        /// first `<L> "B" "C" / ","` iteration in `np -> "A" ( ►► <L> "B" "C" / "," ◄◄ )+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxInp {
        /// `<L> "B" "C" / ","` iteration in `np -> "A" ( ►► <L> "B" "C" / "," ◄◄ )+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxXp {
        /// `xp -> A (<L> x y / ",")+`
        V1 { a: String },
    }
    #[derive(Debug)]
    pub enum InitCtxIxp {
        /// first `<L> x y / ","` iteration in `xp -> A ( ►► <L> x y / "," ◄◄ )+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxIxp {
        /// `<L> x y / ","` iteration in `xp -> A ( ►► <L> x y / "," ◄◄ )+`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxX {
        /// `x -> "X"`
        V1,
    }
    #[derive(Debug)]
    pub enum CtxY {
        /// `y -> "Y"`
        V1,
    }

    #[derive(Debug)]
    enum EnumSynValue { A(SynA), Vp(SynVp), Ivp(SynIvp), Np(SynNp), Xp(SynXp) }

    impl EnumSynValue {
        fn get_a(self) -> SynA {
            if let EnumSynValue::A(val) = self { val } else { panic!() }
        }
        fn get_vp(self) -> SynVp {
            if let EnumSynValue::Vp(val) = self { val } else { panic!() }
        }
        fn get_ivp(self) -> SynIvp {
            if let EnumSynValue::Ivp(val) = self { val } else { panic!() }
        }
        fn get_np(self) -> SynNp {
            if let EnumSynValue::Np(val) = self { val } else { panic!() }
        }
        fn get_xp(self) -> SynXp {
            if let EnumSynValue::Xp(val) = self { val } else { panic!() }
        }
    }

    pub trait TestListener {
        /// Checks if the listener requests an abort. This happens if an error is too difficult to recover from
        /// and may corrupt the stack content. In that case, the parser immediately stops and returns `ParserError::AbortRequest`.
        fn check_abort_request(&self) -> Terminate { Terminate::None }
        fn get_log_mut(&mut self) -> &mut impl Logger;
        #[allow(unused_variables)]
        fn handle_msg(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.get_log_mut().add(msg);
        }
        #[allow(unused_variables)]
        fn intercept_token(&mut self, token: TokenId, text: &str) -> TokenId { token }
        #[allow(unused_variables)]
        fn exit(&mut self, a: SynA) {}
        #[allow(unused_variables)]
        fn abort(&mut self, terminate: Terminate) {}
        fn init_a(&mut self) {}
        fn exit_a(&mut self, ctx: CtxA) -> SynA;
        fn init_vp(&mut self) {}
        fn exit_vp(&mut self, ctx: CtxVp) -> SynVp;
        fn init_ivp(&mut self, ctx: InitCtxIvp) -> SynIvp;
        fn exit_ivp(&mut self, acc: &mut SynIvp, ctx: CtxIvp);
        #[allow(unused_variables)]
        fn exitloop_ivp(&mut self, acc: &mut SynIvp) {}
        fn init_np(&mut self) {}
        fn exit_np(&mut self, ctx: CtxNp) -> SynNp;
        #[allow(unused_variables)]
        fn init_inp(&mut self, ctx: InitCtxInp) {}
        #[allow(unused_variables)]
        fn exit_inp(&mut self, ctx: CtxInp) {}
        #[allow(unused_variables)]
        fn exitloop_inp(&mut self) {}
        fn init_xp(&mut self) {}
        fn exit_xp(&mut self, ctx: CtxXp) -> SynXp;
        #[allow(unused_variables)]
        fn init_ixp(&mut self, ctx: InitCtxIxp) {}
        #[allow(unused_variables)]
        fn exit_ixp(&mut self, ctx: CtxIxp) {}
        #[allow(unused_variables)]
        fn exitloop_ixp(&mut self) {}
        fn init_x(&mut self) {}
        #[allow(unused_variables)]
        fn exit_x(&mut self, ctx: CtxX) {}
        fn init_y(&mut self) {}
        #[allow(unused_variables)]
        fn exit_y(&mut self, ctx: CtxY) {}
    }

    pub struct Wrapper<T> {
        verbose: bool,
        listener: T,
        stack: Vec<EnumSynValue>,
        max_stack: usize,
        stack_t: Vec<String>,
    }

    impl<T: TestListener> ListenerWrapper for Wrapper<T> {
        fn switch(&mut self, call: Call, nt: VarId, alt_id: AltId, t_data: Option<Vec<String>>) {
            if self.verbose {
                println!("switch: call={call:?}, nt={nt}, alt={alt_id}, t_data={t_data:?}");
            }
            if let Some(mut t_data) = t_data {
                self.stack_t.append(&mut t_data);
            }
            match call {
                Call::Enter => {
                    match nt {
                        0 => self.listener.init_a(),                // a
                        1 => self.listener.init_vp(),               // vp
                        2 => self.init_ivp(),                       // ivp
                        3 => self.listener.init_np(),               // np
                        4 => self.init_inp(),                       // inp
                        5 => self.listener.init_xp(),               // xp
                        6 => self.init_ixp(),                       // ixp
                        7 => self.listener.init_x(),                // x
                        8 => self.listener.init_y(),                // y
                        _ => panic!("unexpected enter nonterminal id: {nt}")
                    }
                }
                Call::Loop => {}
                Call::Exit => {
                    match alt_id {
                        0 => self.exit_a(),                         // a -> vp np xp
                        1 => self.exit_vp(),                        // vp -> A B C ivp
                        2 => self.exit_ivp(),                       // ivp -> <L> "," B C ivp
                        3 => self.exitloop_ivp(),                   // ivp -> <L> ε
                        4 => self.exit_np(),                        // np -> "A" "B" "C" inp
                        5 => self.exit_inp(),                       // inp -> <L> "," "B" "C" inp
                        6 => self.listener.exitloop_inp(),          // inp -> <L> ε
                        7 => self.exit_xp(),                        // xp -> A x y ixp
                        8 => self.exit_ixp(),                       // ixp -> <L> "," x y ixp
                        9 => self.listener.exitloop_ixp(),          // ixp -> <L> ε
                        10 => self.exit_x(),                        // x -> "X"
                        11 => self.exit_y(),                        // y -> "Y"
                        _ => panic!("unexpected exit alternative id: {alt_id}")
                    }
                }
                Call::End(terminate) => {
                    match terminate {
                        Terminate::None => {
                            let val = self.stack.pop().unwrap().get_a();
                            self.listener.exit(val);
                        }
                        Terminate::Abort | Terminate::Conclude => self.listener.abort(terminate),
                    }
                }
            }
            self.max_stack = std::cmp::max(self.max_stack, self.stack.len());
            if self.verbose {
                println!("> stack_t:   {}", self.stack_t.join(", "));
                println!("> stack:     {}", self.stack.iter().map(|it| format!("{it:?}")).collect::<Vec<_>>().join(", "));
            }
        }

        fn check_abort_request(&self) -> Terminate {
            self.listener.check_abort_request()
        }

        fn abort(&mut self) {
            self.stack.clear();
            self.stack_t.clear();
        }

        fn get_log_mut(&mut self) -> &mut impl Logger {
            self.listener.get_log_mut()
        }

        fn report(&mut self, span_opt: Option<&PosSpan>, msg: LogMsg) {
            self.listener.handle_msg(span_opt, msg);
        }

        fn is_stack_empty(&self) -> bool {
            self.stack.is_empty()
        }

        fn is_stack_t_empty(&self) -> bool {
            self.stack_t.is_empty()
        }

        fn intercept_token(&mut self, token: TokenId, text: &str, _span: &PosSpan) -> TokenId {
            self.listener.intercept_token(token, text)
        }
    }

    impl<T: TestListener> Wrapper<T> {
        pub fn new(listener: T, verbose: bool) -> Self {
            Wrapper { verbose, listener, stack: Vec::new(), max_stack: 0, stack_t: Vec::new() }
        }

        pub fn get_listener(&self) -> &T {
            &self.listener
        }

        pub fn get_listener_mut(&mut self) -> &mut T {
            &mut self.listener
        }

        pub fn give_listener(self) -> T {
            self.listener
        }

        pub fn set_verbose(&mut self, verbose: bool) {
            self.verbose = verbose;
        }

        fn exit_a(&mut self) {
            let xp = self.stack.pop().unwrap().get_xp();
            let np = self.stack.pop().unwrap().get_np();
            let vp = self.stack.pop().unwrap().get_vp();
            let ctx = CtxA::V1 { vp, np, xp };
            let val = self.listener.exit_a(ctx);
            self.stack.push(EnumSynValue::A(val));
        }

        fn exit_vp(&mut self) {
            let plus = self.stack.pop().unwrap().get_ivp();
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxVp::V1 { a, plus };
            let val = self.listener.exit_vp(ctx);
            self.stack.push(EnumSynValue::Vp(val));
        }

        fn init_ivp(&mut self) {
            let c = self.stack_t.pop().unwrap();
            let b = self.stack_t.pop().unwrap();
            let ctx = InitCtxIvp::V1 { b, c };
            let val = self.listener.init_ivp(ctx);
            self.stack.push(EnumSynValue::Ivp(val));
        }

        fn exit_ivp(&mut self) {
            let c = self.stack_t.pop().unwrap();
            let b = self.stack_t.pop().unwrap();
            let ctx = CtxIvp::V1 { b, c };
            let Some(EnumSynValue::Ivp(acc)) = self.stack.last_mut() else { panic!() };
            self.listener.exit_ivp(acc, ctx);
        }

        fn exitloop_ivp(&mut self) {
            let EnumSynValue::Ivp(acc) = self.stack.last_mut().unwrap() else { panic!() };
            self.listener.exitloop_ivp(acc);
        }

        fn exit_np(&mut self) {
            let ctx = CtxNp::V1;
            let val = self.listener.exit_np(ctx);
            self.stack.push(EnumSynValue::Np(val));
        }

        fn init_inp(&mut self) {
            let ctx = InitCtxInp::V1;
            self.listener.init_inp(ctx);
        }

        fn exit_inp(&mut self) {
            let ctx = CtxInp::V1;
            self.listener.exit_inp(ctx);
        }

        fn exit_xp(&mut self) {
            let a = self.stack_t.pop().unwrap();
            let ctx = CtxXp::V1 { a };
            let val = self.listener.exit_xp(ctx);
            self.stack.push(EnumSynValue::Xp(val));
        }

        fn init_ixp(&mut self) {
            let ctx = InitCtxIxp::V1;
            self.listener.init_ixp(ctx);
        }

        fn exit_ixp(&mut self) {
            let ctx = CtxIxp::V1;
            self.listener.exit_ixp(ctx);
        }

        fn exit_x(&mut self) {
            let ctx = CtxX::V1;
            self.listener.exit_x(ctx);
        }

        fn exit_y(&mut self) {
            let ctx = CtxY::V1;
            self.listener.exit_y(ctx);
        }
    }

    // [wrapper source for rule 985 #2, start a]
    // ------------------------------------------------------------
}

