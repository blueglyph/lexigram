...
fn gramparser_source(indent: usize, verbose: bool) -> Result<(BufLog, String, String, String), BufLog> {
    ...
    static ORIGIN: [(Option<usize>, &[(GrNode, &[usize])]); 9] = [
        ...
        (Some(3), &[(gnode!(t 9), &[]), (gnode!(t 16), &[]), (gnode!(t 6), &[]), (gnode!(&), &[0,1,2])]),
        (Some(4), &[(gnode!(nt 3), &[]), (gnode!(nt 2), &[]), (gnode!(nt 3), &[]), (gnode!(&), &[1,2]), (gnode!(|), &[0,3])]),
        (Some(11), &[(gnode!(nt 4), &[]), (gnode!(t 0), &[]), (gnode!(nt 5), &[]), (gnode!(t 10), &[]), (gnode!(t 6), &[]), (gnode!(&), &[0,1,2,3,4]), (gnode!(nt 4), &[]), (gnode!(t 0), &[]), (gnode!(nt 5), &[]), (gnode!(t 6), &[]), (gnode!(&), &[6,7,8,9]), (gnode!(|), &[5,10])]),
        (Some(5), &[(gnode!(t 15), &[]), (gnode!(t 16), &[]), (gnode!(&), &[0,1]), (gnode!(t 16), &[]), (gnode!(&), &[3]), (gnode!(|), &[2,4])]),
        ...
        (Some(10), &[(gnode!(t 16), &[]), (gnode!(t 11), &[]), (gnode!(t 12), &[]), (gnode!(t 13), &[]), (gnode!(t 14), &[]), (gnode!(t 1), &[]), (gnode!(nt 5), &[]), (gnode!(t 5), &[]), (gnode!(&), &[5,6,7]), (gnode!(t 7), &[]), (gnode!(|), &[0,1,2,3,4,8,9])]),
    ];
    ...
    let ll1_tables = ProdRuleSetTables::new(
        ...
        vec![
            prule!(%(0, 2), nt 1, nt 2),
            prule!(%(1, 3), t 9, t 16, t 6),
            prule!(%(2, 0), nt 3, nt 10),
            prule!(nt 4, t 0, nt 5, nt 12),
            prule!(%(4, 2), t 15, t 16; %(4, 4), t 16),
            prule!(%(5, 0), nt 6, nt 11),
            prule!(%(6, 2), nt 9),
            prule!(nt 8, nt 13),
            prule!(%(8, 0), t 16; %(8, 1), t 11; %(8, 2), t 12; %(8, 3), t 13; %(8, 4), t 14; %(8, 8), t 1, nt 5, t 5; %(8, 9), t 7),
            prule!(%(6, 1), nt 7, nt 9; e),
            prule!(#(0, 0), %(2, 3), nt 3, nt 10; e),
            prule!(#(0, 1), %(5, 4), t 2, nt 6, nt 11; e),
            prule!(%(3, 10), t 6; %(3, 5), t 10, t 6),
            prule!(%(7, 2), t 3; %(7, 8), t 4; %(7, 5), t 8; %(7, 10), e),
        ],
        origin,
        vec![("Colon", Some(":")), ("Lparen", Some("(")), ("Or", Some("|")), ("Plus", Some("+")), ("Question", Some("?")), ("Rparen", Some(")")), ("Semicolon", Some(";")), ("Sep", Some("/")), ("Star", Some("*")), ("Grammar", Some("grammar")), ("SymEof", Some("EOF")), ("Lform", None), ("Rform", Some("<R>")), ("Pform", Some("<P>")), ("Greedy", Some("<G>")), ("ResolveTag", Some("<resolve>")), ("Id", None)],
