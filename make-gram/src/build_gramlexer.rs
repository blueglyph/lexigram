...
pub fn write_gramlexer() {
    let (result_sym, result_src) = gramlexer_source(GRAMLEXER_LEXICON, 4, true)
        ...
        .unwrap();
    replace_tagged_source(BUILD_GRAMPARSER_FILENAME, GRAM_SYM_TAG, &result_sym)
...
mod tests {
    ...
    fn test_source() {
        ...
        let (_result_sym, _result_src) = gramlexer_source(GRAMLEXER_LEXICON, 4, VERBOSE)
            ...
            .unwrap();
        if !cfg!(miri) {
            let expected_sym = get_tagged_source(BUILD_GRAMPARSER_FILENAME, GRAM_SYM_TAG).unwrap_or(String::new());
