...
/// Makes a stream for the parser from a text input and basic token information. The caret information is very crude:
/// the column increments on each token, and the line remains at 1.
///
/// For tests purposes only.
///
/// * [tokens]: Iterable of `(&str, Option<&str>)`, or something that can be converted to it; e.g. the `SYMBOL_TABLE_T` generated by
///             Lexigram is fine. The first `&str` is the name of the token, the second is the fixed text it scans or `None` if it's variable.
...
/// // ...
///
/// let stream = make_stream(input, SYMBOL_TABLE_T, 2, 9999, false);
