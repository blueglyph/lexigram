...
// #[derive(Clone, Debug)]
// pub struct LALR;
pub use lexigram_core::LALR;
use lexigram_core::lexer::{CaretCol, Pos, PosSpan};
use lexigram_core::parser::ParserToken;

pub use make_stream::{make_stream, MakeStreamIterator};
...
mod make_stream {
    ...
    use iter_index::Indexer;
    use super::*;

    /// Makes a stream for the parser from an input string and basic token information.
    ///
    /// For tests purposes only.
    ///
    /// The caret information is very crude:
    /// the column increments on each token, and the line remains at 1. All space characters are discarded and used to separate tokens,
    /// so there must be at least one space between each token.
    ///
    /// Arguments:
    /// * [tokens]: Iterable of `(&str, Option<&str>)`, or something that can be converted to it; e.g. the `SYMBOLS_T` generated by
    ///   Lexigram is fine.
    ///   * The first `&str` is the **name** of the token used in the grammar,
    ///   * the `Option<&str>` is the **fixed value** it scans (like a keyword) or `None` if it's variable (like an identifier or a number).
    /// * [include_tnames]: When true, the token names are used to scan the input when the fixed value doesn't exist. When false,
    ///   only the fixed values are used to scan the input.
    /// * [id]: The [TokenId] of an identifier: anything that isn't in the `Option<&str>` items and which starts with a non-digit will
    ///   be converted into that token. Put a very large number if not used.
    /// * [num]: The [TokenId] of a number: anything starting with a digit which isn't in the `Option<&str>` items will be converted into
    ///   that token. Put a very large number if not used.
    /// * [verbose]: When true, prints out the tokens as they're issued.
    ///
    /// ## Example
    ///
    /// This example will stream tokens `A`, `Id(a)`, `Id(b)`, `Id(c)`, `C`:
    /// ```ignore
    /// use lexigram_lib::make_stream;
    ///
    /// static SYMBOLS_T: [(&str, Option<&str>); 3] = [("A", None), ("C", None), ("Id", None)];
    /// // ...
    ///
    /// let stream = make_stream("A a b c C", SYMBOLS_T, true, 2, 9999, false);
    /// let mut listener = Listener::new();
    /// let mut wrapper = Wrapper::new(listener, false);
    /// let result = parser.parse_stream(&mut wrapper, stream);
    /// ```
    pub fn make_stream<S, T>(
        input: &str,
        tokens: T,
        include_tnames: bool,
        id: Option<TokenId>,
        num: Option<TokenId>,
        verbose: bool
    ) -> impl Iterator<Item=ParserToken>
    where
        S: std::hash::Hash + Eq + std::borrow::Borrow<str>,
        T: IntoIterator<Item=(S, Option<S>)>,
    {
        MakeStreamIterator::new(input, tokens, include_tnames, id, num, verbose)
    }

    enum MakeStreamMode { Token, Error(String, PosSpan), Eof }

    pub struct MakeStreamIterator<'s, S> {
        items: Indexer<SplitAsciiWhitespace<'s>, CaretCol>,
        mode: MakeStreamMode,
        symbols: HashMap<S, TokenId>,
        id: Option<TokenId>,
        num: Option<TokenId>,
        verbose: bool
   }

    impl<'s, S> MakeStreamIterator<'s, S>
    where
        S: std::hash::Hash + Eq + std::borrow::Borrow<str>,
    {
        pub fn new<T: IntoIterator<Item=(S, Option<S>)>> (
            input: &'s str,
            tokens: T,
            include_tnames: bool,
            id: Option<TokenId>,
            ...
            let symbols: HashMap<S, TokenId> = tokens.into_iter().index::<TokenId>()
                ...
                .collect::<HashMap<_, _>>();
            MakeStreamIterator {
                items: input.split_ascii_whitespace().index_start::<CaretCol>(1),
                mode: MakeStreamMode::Token,
                symbols,
                id,
                num,
                verbose
    ...
    impl<S> Iterator for MakeStreamIterator<'_, S>
    where
        S: std::hash::Hash + Eq + std::borrow::Borrow<str>,
    {
        ...
        fn next(&mut self) -> Option<Self::Item> {
            let next = match &mut self.mode {
                MakeStreamMode::Token => {
                    if let Some((pos, w)) = self.items.next() {
                        let pos_span = PosSpan::new(Pos(1, pos), Pos(1, pos));
                        if let Some(s) = self.symbols.get(w) {
                            Some((*s, w.to_string(), pos_span))
                        } else if let Some(num) = self.num && w.chars().next().unwrap().is_ascii_digit() {
                            Some((num, w.to_string(), pos_span))
                        } else if let Some(id) = self.id && w.chars().next().unwrap().is_ascii_alphabetic() {
                            Some((id, w.to_string(), pos_span))
                        } else {
                            self.mode = MakeStreamMode::Error(
                                format!("invalid character {w:?} at {pos_span}"),
                                pos_span
                            );
                            None
                        }
                    } else {
                        self.mode = MakeStreamMode::Eof;
                        None
                    }
                }
                MakeStreamMode::Error(msg, pos_span) => {
                    let parser_token = (TokenId::MAX, std::mem::take(msg), std::mem::take(pos_span));
                    self.mode = MakeStreamMode::Token;
                    Some(parser_token)
                }
                MakeStreamMode::Eof => None,
            };
            if self.verbose {
                if let Some((token, str, pos)) = &next {
