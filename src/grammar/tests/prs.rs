...
fn rts_prodrule_from() {
    let tests = vec![
        ...
        ], vec![2048, 4097], vec![None, Some(0)]),
        (105, vec![
            r#"a -> a_1"#,                                      // parent_+_or_*
            r#"b -> C"#,                                        //
            r#"a_1 -> b A b B A a_1 | b A b B A"#,              // child_+_or_* | plus
        ], vec![2048, 0, 4097], vec![None, None, Some(0)]),
        (106, vec![
...
fn prs_ll1_from() {
    let tests = vec![
        ...
        ], vec![2048, 4129, 64], vec![None, Some(0), Some(1)]),
        (106, vec![
            // a -> (A (b ",")* ";")* C
            // b -> B
            r#"a -> a_2 C"#,                                    // parent_+_or_*
        ...
        ], vec![2048, 0, 4129, 6177, 64, 64], vec![None, None, Some(3), Some(0), Some(2), Some(3)]),
        (200, vec![
            // a -> A (<L=i> B)* C
        ...
        ], vec![2048, 4257, 64], vec![None, Some(0), Some(1)]),
        (208, vec![
            // a -> (<L=i> A (<L=j> b ",")* ";")* C
            // b -> B
            r#"a -> i C"#,                                      // parent_+_or_*
        ...
        ], vec![2048, 6305, 4257, 0, 64, 64], vec![None, Some(0), Some(1), None, Some(1), Some(2)]),
        (300, vec![
            // a -> "?" a | "!"
            r#"a -> "?" a | "!""#,                              // right_rec
        ], vec![2], vec![None]),
        ...
        (811, vec![
            ...
            r#"a_1 -> A a_2"#,                                  // child_+_or_* | parent_left_fact | plus
            r#"a_2 -> a_1 | ε"#,                                // child_left_fact
        ], vec![2050, 4129, 64], vec![None, Some(0), Some(1)]),
        (812, vec![
            // a -> (A a)*
            r#"a -> a_1"#,                                      // parent_+_or_*
        ...
        (821, vec![
            ...
            r#"a_2 -> a_1 C a_2 | ε"#,                          // child_left_rec
            r#"a_3 -> a_1 | ε"#,                                // child_left_fact
        ], vec![2560, 4129, 4, 64], vec![None, Some(0), Some(0), Some(1)]),
        (822, vec![
            // a -> (a A)* | B
        ...
        ], vec![2048, 4129, 96, 64, 64], vec![None, Some(0), Some(1), Some(2), Some(2)]),
        (842, vec![
            // a -> A* B* | A* C*
        ...
        ], vec![2048, 4129, 4129, 4129, 4129, 64, 64, 64, 64], vec![None, Some(0), Some(0), Some(0), Some(0), Some(1), Some(2), Some(3), Some(4)]),
        (860, vec![
            // a -> A B a | A C a | D
        ...
        (901, vec![
            ...
            r#"alt_item -> alt_item_1"#,                                                                                                // parent_+_or_*
            ...
            r#"char_set -> "[" char_set_1 "]" | "." | FixedSet"#,                                                                       // parent_+_or_*
         ...
         vec![2048, 0, 0, 0, 2048, 32, 2048, 0, 0, 2048, 2048, 32, 34, 2048, 32, 1, 1, 1, 1, 4129, 4129, 64, 96, 64, 64, 64, 64, 64, 64],
         vec![
             None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, Some(0), Some(4), Some(6), Some(9),
             Some(10), Some(13), Some(5), Some(11), Some(12), Some(14), Some(19), Some(20), Some(22), Some(22)]),
        /* template:
        (1, vec![
        ], vec![], vec![]),
        */
...
fn prs_calc_table() {
    let tests = vec![
        ...
        (902, 0, false, vec![
            // - 0: program -> decl_i inst_i
            // - 1: decl_i -> decl decl_i
            // - 2: decl_i -> ε
            // - 3: inst_i -> inst inst_i_1
            // - 4: decl -> Type Id id_i ";"
            // - 5: decl -> "typedef" Type Id ";"
            // - 6: id_i -> "," Id id_i
            // - 7: id_i -> ε
            // - 8: inst -> "let" Id "=" expr ";"
            // - 9: inst -> "print" expr ";"
            // - 10: expr -> expr_2 expr_1
            ...
            // expr_2   | 16  15   .    .   p     .     .   .    .   14   p   .
            // inst_i_1 |  .   .   .    .   .     .    17   .   17    .   .  18
             19,  19,   0,  19,  19,   0,   0,  19,   0,  19,  19,  20,
             19,  19,   1,  19,  19,   1,   2,  19,   2,  19,  19,  19,
             19,  19,  19,  19,  19,  19,   3,  19,   3,  19,  19,  20,
             19,  19,   4,  19,  19,   5,  20,  19,  20,  19,  19,  19,
             19,  19,  19,   6,   7,  19,  19,  19,  19,  19,  19,  19,
             19,  19,  19,  19,  19,  19,   8,  19,   9,  19,  19,  20,
             10,  10,  19,  19,  20,  19,  19,  19,  19,  10,  19,  19,
             19,  19,  19,  19,  13,  19,  19,  19,  19,  12,  11,  19,
             16,  15,  19,  19,  20,  19,  19,  19,  19,  14,  20,  19,
             19,  19,  19,  19,  19,  19,  17,  19,  17,  19,  19,  18,
        ]),
        (903, 0, false, vec![
            // - 0: program -> stmt_i
            // - 1: stmt_i -> stmt stmt_i
            // - 2: stmt_i -> ε
            // - 3: stmt -> decl
            // - 4: stmt -> inst
            // - 5: decl -> Type Id decl_1 ";"
            // - 6: decl -> "typedef" Type Id ";"
            // - 7: inst -> Id "=" expr ";"
            // - 8: inst -> "print" expr ";"
            // - 9: expr -> expr_2 expr_1
            // - 10: decl_1 -> "," Id decl_1
            // - 11: decl_1 -> ε
            // - 12: expr_1 -> "+" expr_2 expr_1
            // - 13: expr_1 -> "-" expr_2 expr_1
            // - 14: expr_1 -> ε
            // - 15: expr_2 -> "-" expr_2
            // - 16: expr_2 -> Id
            // - 17: expr_2 -> Num
            //
            //         | Num Id  Type  ,   ;  typedef  =  print  -   +   $
            // --------+----------------------------------------------------
            // program |  .   0   0    .   .     0     .    0    .   .   0
            // stmt_i  |  .   1   1    .   .     1     .    1    .   .   2
            // stmt    |  .   4   3    .   .     3     .    4    .   .   p
            ...
            // expr_1  |  .   .   .    .  14     .     .    .   13  12   .
            // expr_2  | 17  16   .    .   p     .     .    .   15   p   .
             18,   0,   0,  18,  18,   0,  18,   0,  18,  18,   0,
             18,   1,   1,  18,  18,   1,  18,   1,  18,  18,   2,
             18,   4,   3,  18,  18,   3,  18,   4,  18,  18,  19,
             18,  19,   5,  18,  18,   6,  18,  19,  18,  18,  19,
             18,   7,  19,  18,  18,  19,  18,   8,  18,  18,  19,
              9,   9,  18,  18,  19,  18,  18,  18,   9,  18,  18,
             18,  18,  18,  10,  11,  18,  18,  18,  18,  18,  18,
             18,  18,  18,  18,  14,  18,  18,  18,  13,  12,  18,
             17,  16,  18,  18,  19,  18,  18,  18,  15,  19,  18,
        ]),
        /* template:
        (, 0, false, vec![]),
        */
