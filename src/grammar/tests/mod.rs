...
impl TestRules {
    ...
    fn make_rts_general(&mut self) {
        ...
        let specs = match self.test_id {
            ...
            // copy of Lexi's original grammar
            901 => vec![
                r#"token"#,
            ...
            982 => vec![
                r#"a -> s p; s -> vs ns xs; p -> vp np xp;"#,
                r#"vs -> A (<L=ivs> B)*;        ns -> "A" (<L=ins> "B")*;               xs -> A (<L=ixs> x)*;"#,
                r#"vp -> A (<L=ivp> B)+;        np -> "A" (<L=inp> "B")+;               xp -> A (<L=ixp> x)+;"#,
                r#"x -> "X";"#,
            ],
            983 => vec![
                r#"a -> s p; s -> vs ns xs; p -> vp np xp;"#,
            ...
            984 => vec![
                r#"a -> vp np xp;"#,
                r#"vp -> A (B C / ",")+;        np -> "A" ("B" "C" / ",")+;             xp -> A (x y / ",")+;"#,
                r#"x -> "X"; y -> "Y"; "#,
            ],
            985 => vec![
                r#"a -> vp np xp;"#,
                r#"vp -> A (<L=ivp> B C / ",")+; np -> "A" (<L=inp> "B" "C" / ",")+;    xp -> A (<L=ixp> x y / ",")+;"#,
                r#"x -> "X"; y -> "Y"; "#,
            ],
