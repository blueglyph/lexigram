...
use crate::{escape_string, gnode, CollectJoin, General, SymbolTable, TokenId, LL1, VarId};
...
fn regexgen_stability() {
    ...
    for opt in [LexerType::Normalized, LexerType::Optimized] {
        ...
        let (tokens, texts): (Vec<TokenId>, Vec<String>) = lexer
            .tokens()
            .filter_map(|(tok, ch, text, _pos_span)| if ch == 0 {
        ...
        let (tokens2, texts2): (Vec<TokenId>, Vec<String>) = lexer
            .tokens()
            .filter_map(|(tok, ch, text, _pos_span)| if ch == 0 { Some((tok, text)) } else { None })
            .unzip();
...
fn lexiparser_source() {
    ...
    if VERBOSE {
        if !rules.get_log().is_empty() {
            println!("messages PRS<General>: {}", rules.get_log().get_messages_str());
        }
